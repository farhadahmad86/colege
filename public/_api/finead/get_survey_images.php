...
try {
    ...
    if (isset($jwt)) {
        ...
        $surveyId = isset($_GET['id']) ? $database->escape_value($_GET['id']) : '';

        $surveyDetailsQuery = "SELECT type, shop_name, shop_address FROM survey WHERE id = $surveyId LIMIT 1;";
        $surveyDetailsResult = $database->query($surveyDetailsQuery);

        if ($surveyDetailsResult) {
            $s = $database->fetch_assoc($surveyDetailsResult);
            $surveyType = $s['type'];
            $shopName = $s['shop_name'];
            $shopAddress = $s['shop_address'];
        }

        $query = "SELECT id, complete_url as url FROM images_before WHERE survey_id = $surveyId;";
        ...
        if ($result) {
            ...
            while ($data = $database->fetch_array($result)) {
                ...
                $list[] = array('id' => $data['id'], 'url' => $data['url']);
            ...
            $response->data = array('type' => $surveyType, 'shop_name' => $shopName, 'address' => $shopAddress, 'list' => $list);
//            $response->data = $list;
            $response->message = "Survey images List.";

        } else {
            $response->code = NOT_OK;
            $response->message = "Unable to get Survey images.";
        }
