...
try {
    ...
    if (isset($jwt)) {
        ...
        $search = isset($_GET['s']) ? $database->escape_value($_GET['s']) : '';
        $companyId = isset($_GET['cid']) ? $database->escape_value($_GET['cid']) : 0;
        $userId = isset($_GET['uid']) ? $database->escape_value($_GET['uid']) : 0;

        $searchFilter = " WHERE id > 0 AND execution = 'NO'";

        if ($q != '') {
            $searchFilter .= " AND shop_name LIKE '%$q%' ";
        }

        if ($search != '') {
            $searchFilter .= " AND (shop_name LIKE '%$search%' OR shop_address LIKE '%$search%' OR load_number LIKE '%$search%' OR whats_app_number LIKE '%$search%') ";
        }

        if ($companyId > 0) {
            $searchFilter .= " AND company_id = $companyId ";
        }

        if ($userId > 0) {
            $searchFilter .= " AND created_by = $userId ";
        }

        $query = "SELECT id, shop_name, shop_address FROM survey $searchFilter order by id;";
        ...
        if ($result) {
            ...
            while ($data = $database->fetch_array($result)) {
                ...
                $list[] = array('id' => $data['id'], 'shop_name' => $data['shop_name'], 'shop_address' => $data['shop_address']);
            ...
            $response->message = "Unable to get Survey List.";
        }
