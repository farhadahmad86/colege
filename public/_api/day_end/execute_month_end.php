...
$error = false;
//************************************************************************
//                         DB Variables in database.php START
//************************************************************************
...
$tellerId = TELLER;
//************************************************************************
//                         DB Variables in database.php END
//************************************************************************
...
*/
if (!$error && !$rollBack) {
    $query = "UPDATE financials_day_end SET de_datetime_status = 'PROCESSING' WHERE de_clg_id = $loginClgId AND de_id = $currentDayEndId AND de_month_status = 'PENDING'
    ...
    if (count($child) > 0) {
        ...
        foreach ($child as $item) {
            ...
            if ($entry['level'] === 3) {
                ...
                $childEntry .= trialEntry($item, $id . $childCounter); // smaj nai aii
            }
        }
        $childEntry .= '
                </table>
...
if ($createTrial || ($createPnL || $createBalanceSheet || $createPnlDistribution)) {
    ...
    if (!$error) {
        ...
        while ($topLevelData = $database->fetch_assoc($topLevelResult)) {
            ...
            // Fetching Level 2
            $parentQuery = "SELECT coa_code, coa_head_name, coa_level FROM financials_coa_heads WHERE coa_clg_id = $loginClgId AND coa_parent = $controlCode;";
            ...
            while ($parentData = $database->fetch_assoc($parentResult)) {
                ...
                // Fetching Level 3
                $childQuery = "SELECT coa_code, coa_head_name, coa_level FROM financials_coa_heads WHERE coa_clg_id = $loginClgId AND coa_parent = $parentCode;";
...
//                            array('parent' => $childCode, 'code' => $entryCode, 'name' => "$entryName", 'level' => 4, 'type' => $balanceType, 'opening' => $opening, 'inwards' => $inwards, 'outwards' => $outwards, 'balance' => $balanceBF);
                    ...
                    // comment to view empty parent, child heads
                    if (count($entryAccounts) > 0) { $trialViewList[$controlCode]['child'][$parentCode]['child'][$childCode] =
                        array('parent' => $parentCode, 'code' => $childCode, 'name' => "$childName", 'level' => 3, 'child' => array(), 'type' => '', 'opening' => 0.00, 'inwards' => 0.00, 'outwards' => 0.00, 'balance' => 0.00);
                ...
                foreach ($child2 as $entry3) {
                    ...
                    foreach ($child3 as $entry4) {
                        if ($entryCode == $assetsTopLevelUID || $entryCode == $expensesTopLevelUID) {
                            // echo $entryCode.'=='.$assetsTopLevelUID.'OR'.$entryCode.'=='.$expensesTopLevelUID.$entry4['type'].'Bal:'.$entry4['balance'].'Ent4Bal: ';
                            if ($entry4['type'] == 'DR') {
                    ...
                    // print_r($trialViewList[$entryCode]);
                    if ($entryCode == $assetsTopLevelUID || $entryCode == $expensesTopLevelUID) {
                        if ($entry4Type == 'DR') {
        ...
        foreach ($trialViewList as $control) {
            ...
            //WHERE account_clg_id = $loginClgId AND
            if ($controlType == "DR") {
...
if ($monthlyCompleteReport) {
    if (!$error) {
        ...
        // print_r($trialViewList[EXPENSES]['child']);
        // echo '<br>';
        if (array_key_exists(CGS_EXPENSE_GROUP_UID, $trialViewList[EXPENSES]['child'])) {
            ...
            foreach ($cgsExpensesParent as $cgsExp) {
                ...
                foreach ($child as $exp) {
                    ...
                    $totalCGSExpenses += $exp['balance']; // Muzammil 24-09-2022
//                    print_r($cgsExpenseChildList);
                }
            }
...
// echo $cgsParentCode.':'.$cgsCode.':'.$cgsName.':'.$cgsLevel.':'.$cgsBalance.':'.$cgsChild;
        ...
        if ($pnlTempExpensesBalanceType == 'CR') {
            ...
            // echo $pnlTempExpensesBalanceType;
        }
...
if ($monthlyCompleteReport) {
    if (!$error) {
        ...
        // **********************************
//print_r($bsAssetEntries['balance'] );
        ...
        foreach ($bsLiabilityChild as $bsLiabilityChild1) {
            ...
            foreach ($bsLiability2Child as $bsLiabilityChild2) {
                ...
                foreach ($bsLiability3Child as $bsLiabilityChild3) {
                    ...
                    $bsLiabilityOCBQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
...
//        echo " Equity Liability"."</br>";
//echo (double)$bsLiabilityBalance .'+'. (double)$bsEquityBalance .'+' .$netProfitOrLossAmount;
        // total liability + equity
        $bsTotalLiabilityEquityBalance = (double)$bsLiabilityBalance + (double)$bsEquityBalance + (double)$netProfitOrLossAmount;

        $bsTotalLiabilityEquityBalanceDisplay=0; // mustafa
        ...
        if ($capResult && $numRows > 0) {
            ...
            if ($numRows == 1) {
                ...
                if ($netProfitOrLoss == "Profit") {
                    ...
                    $type = "Credit";
                    $pnlOpOutwards += $netProfitOrLossAbsoluteAmount;// mustafa
                ...
                if ($newProfitAndLossAccountBalance >= 0) {// mustafa
                ...
                if (number_format($undistributedProfitAndLossAmount, 2, '.', '') != 0.00) {
                    ...
                    if ($netProfitOrLoss == "Profit") {
                        ...
                        $type = "Credit";
                        $pnlOpOutwards += $undistributedProfitAndLossAmount; //mustafa
                        $newUndistributedProfitLossAccountBalance = $undistributedProfitLossAccountBalance + $undistributedProfitAndLossAmount; // Muzammil - to + hello
                    } else {
                        $drBalance = $undistributedProfitAndLossAmount;
                        $type = "Debit";
                        $pnlOpInwards += $undistributedProfitAndLossAmount;
                        $newUndistributedProfitLossAccountBalance = $undistributedProfitLossAccountBalance - $undistributedProfitAndLossAmount; // Muzammil + to -
                    }
                    ...
                    if ($undistributedProfitLossAccountBalance >= 0) { //mustafa
                        $newUndistributedProfitLossAccountBalanceType = 'CR';
                    } else {
                        $newUndistributedProfitLossAccountBalanceType = 'DR';
                    }
...
//    $database->commit();
    ...
    $dayEndReportURLQuery = "UPDATE financials_day_end SET de_month_end_report_url = '$dayEndHTMLReportCompleteURL', de_last_day_of_month = 1, de_month_status = 'EXECUTED' WHERE de_id = $currentDayEndId LIMIT 1;";
    ...
    try {
        ...
        $query = "INSERT INTO financials_day_end (de_datetime_status, de_datetime, de_first_day_of_month, de_createdby, de_current_datetime, de_ip_adrs, de_brwsr_info,de_clg_id)
                        VALUES ('OPEN', '$newDate', 1, $loginUserId, '$dbTimeStamp', '$ipAddress', '$type','$loginClgId');";
