...
$cssContent = file_get_contents(__DIR__ . '/src/css/day_end.css') or "";
$cssContent = str_replace( "../", $mailContentSrcPath, $cssContent);
    ...
    if ($pendingMonthEnd->found == true) {
        ...
        try {
            ...
            $fields = array(
                ...
                'clg_id' => urlencode(isset($_REQUEST['clg_id']) ? $database->escape_value($_REQUEST['clg_id']) : 0),
 ...
 * ******************************************************************************************************************************
 *
 * Header Section including Company Logo and Day End Date and Error
 *
 * ******************************************************************************************************************************
 */
$headerErrorSection = '
 ...
 * ******************************************************************************************************************************
 */
if ($currentDayEndIsLocked) {

    if ($type == "android") {
        $lockErrorMessage = "Current Day End is Locked for Execution, Please Unlock the Current Day End first!";
    } else {
        $lockErrorMessage = "Current Day End is Locked for Execution, Please close this popup window and go to Settings <img src='" . $srcUrl . "/images/settings.png' height='22' class='settings-dd-image' /> (Available on top Right) to Unlock the Current Day End!";
    }

    $dieReport = $htmlHeader;
    $dieReport .= $headerSection;
    $dieReport .= error("Day End Locked", $lockErrorMessage);
    $dieReport .= $htmlFooter;
...
if ($cashCheck || $isThisLastDayOfMonth) {
    $getCashAccountsQuery = "SELECT account_uid as UID, account_name as name FROM financials_accounts WHERE account_clg_id = $loginClgId AND account_parent_code = $cashParentUID;";
 ...
 *******************************************************************************************************************************
 *
 * Depreciation Account Error
 *
 * ******************************************************************************************************************************
 */
$depreciationErrorReport = "";

//if ($bankCheck || $isThisLastDayOfMonth) {
$getDepAccountsQuery = "SELECT fa_id, fa_account_name, fa_link_account_uids, fa_price, fa_residual_value,
                        ...
                        WHERE fa_clg_id =$loginClgId AND fa_status = 0 and fa_disabled= 0;";

$getDepAccountsResult = $database->query($getDepAccountsQuery);

if ($getDepAccountsResult && $database->num_rows($getDepAccountsResult) > 0) {

    $tempDepreciationErrorReport = '
        <!-- Cash Accounts Error -->
        <section class="invoice_cntnt_sec"><!-- invoice content section start here -->
            <div class="invoice_cntnt_title_bx">
                <h2 class="gnrl-mrgn-pdng invoice_cntnt_title">
                    Depreciation Accounts Error
                </h2>
            </div>
            <table class="table day_end_table">
                <thead>
    ...
    while ($depreciationAccounts = $database->fetch_array($getDepAccountsResult)) {

        $depAccounts = explode(',', $depreciationAccounts['fa_link_account_uids']);
        $accountUID = $depAccounts[0];
        $accountName = $depreciationAccounts['fa_account_name'];
        $price = $depreciationAccounts['fa_price'];
    ...
    $tempDepreciationErrorReport .= '
			...
			<p class="alert-well">
                Depreciation account Create Fixed Asset Voucher!
            </p>
        </section><!-- invoice content section end here -->
    ';

    if($depreciationError > 0) {
        $depreciationErrorReport .= $tempDepreciationErrorReport;
    }
}
//}
...
if ($warehouseCheck || $isThisLastDayOfMonth) {
    $getProductsQuery = "SELECT pro_p_code, pro_title, wh_title, whs_stock
                        FROM financials_warehouse_stock
                            ...
                            join financials_warehouse on wh_id = whs_warehouse_id WHERE pro_clg_id=$loginClgId;";
    $getProductsResult = $database->query($getProductsQuery);
    if ($getProductsResult) {
...
if ($error || $rollBack) {
    ...
    $dieReport .= $depreciationErrorReport;
...
*/
if (!$error && !$rollBack) {
    $query = "UPDATE financials_day_end SET de_datetime_status = 'PROCESSING' WHERE de_clg_id = $loginClgId AND de_id = $currentDayEndId AND de_datetime_status = 'OPEN'
    ...
    if(!$result || $database->affected_rows() != 1) {
        ...
        $dieReport = $htmlHeader;
        $dieReport .= $headerSection;
        $dieReport .= error("Day End Processing Error", "Unable to start Day End processing!");
        $dieReport .= $htmlFooter;
...
$dailyDepreciationError = false;
$dailyDepreciationErrorMessage = "";
...
if (!$error) {
    ...
    if ($dailyDepResult && $database->num_rows($dailyDepResult) > 0) {
        ...
        while ($dep = $database->fetch_array($dailyDepResult)) {
            ...
            $accDepAssetAccountBalance = getAccountClosingBalance($depAccDepAssetAccountUID,$loginClgId)->balance;
            ...
            /*
             *
             * Save daily depreciation entries and hit accounts ledger accordingly
             *
             */
            $remarks = "Auto generate daily depreciation expense on $currentDate using $methodName @ $depPercentage%";
            $devInsertQuery = "INSERT INTO financials_dep_expense_voucher(dev_exp_uid, dev_acc_dep_uid, dev_remarks, dev_day_end_id, dev_day_end_date, dev_user_id, dev_current_date_time,dev_clg_id)
                                VALUES ($depDepExpenseAccountUID, $depAccDepAssetAccountUID, '$remarks', $currentDayEndId, '$currentDayEndDate', $loginUserId, '$dbTimeStamp',$loginClgId);";

            $devInsertResult = $database->query($devInsertQuery);

            if ($devInsertResult) {

                $depInsertedId = $database->inserted_id();

                $fixedAssetUpdateQuery = "UPDATE financials_fixed_asset SET fa_book_value = $newBookValue, fa_dep_entries = fa_dep_entries + 1 WHERE fa_id = $fixedAssetEntryId AND fa_dep_clg_id=$loginClgId LIMIT 1";

                $fixedAssetUpdateResult = $database->query($fixedAssetUpdateQuery);

                if ($fixedAssetUpdateResult && $database->affected_rows() == 1) {

                    $depVoucherCode = DEPRECIATION_EXPENSE_VOUCHER_CODE . $depInsertedId;

                    $depDepExpenseAccountBalance = getAccountClosingBalance($depDepExpenseAccountUID,$loginClgId)->balance;
                    $depAccDepAssetAccountBalance = getAccountClosingBalance($depAccDepAssetAccountUID,$loginClgId)->balance;

                    $newDepDepExpenseAccountBalance = $depDepExpenseAccountBalance + $todaysDepExp;
                    $newDepAccDepAssetAccountBalance = $depAccDepAssetAccountBalance - $todaysDepExp;

                    $depBalanceQuery = "INSERT INTO financials_balances (bal_account_id, bal_transaction_type, bal_remarks, bal_dr, bal_cr, bal_total, bal_transaction_id, bal_day_end_id, bal_day_end_date, bal_detail_remarks, bal_voucher_number, bal_user_id, bal_datetime,bal_clg_id)
                            VALUES ($depDepExpenseAccountUID, 'DEPRECIATION EXPENSE VOUCHER', '$remarks', $todaysDepExp, 0, $newDepDepExpenseAccountBalance, 0, $currentDayEndId, '$currentDayEndDate', '$remarks', '$depVoucherCode', $loginUserId, '$dbTimeStamp',$loginClgId),
                                   ($depAccDepAssetAccountUID, 'DEPRECIATION EXPENSE VOUCHER', '$remarks', 0, $todaysDepExp, $newDepAccDepAssetAccountBalance, 0, $currentDayEndId, '$currentDayEndDate', '$remarks', '$depVoucherCode', $loginUserId, '$dbTimeStamp',$loginClgId);";

                    $depBalanceResult = $database->query($depBalanceQuery);

                    if (!$depBalanceResult) {
                        $error = true;
                        $rollBack = true;
                        $dailyDepreciationError = true;
                        $dailyDepreciationErrorMessage .= "Unable to save Depreciation Expense into ledger account of " . $dep['fa_account_name'];
                        break;
                    }
                    ...
                    $rollBack = true;
                    $dailyDepreciationError = true;
                    $dailyDepreciationErrorMessage .= "Unable to reduce asset value of " . $dep['fa_account_name'];
                    break;
                }
                ...
                $rollBack = true;
                $dailyDepreciationError = true;
                $dailyDepreciationErrorMessage .= "Unable to save Depreciation Expense Voucher of " . $dep['fa_account_name'];
                break;
            }
        }
        ...
        if ($dailyDepreciationError) {
            $dailyDepreciationHTMPReport .= shortError($dailyDepreciationErrorMessage);
        } else {
            $dailyDepreciationHTMPReport .= shortSuccess("Daily Depreciation occur Successfully.");
        }
...
if ($error || $rollBack) {
    ...
    $dieReport .= error("Day End Execution Stop", "Day End Stop and unable to process until the above Error(s) resolved!");
    $dieReport .= $htmlFooter;
...
$monthlyDepreciationError = false;
$monthlyDepreciationErrorMessage = "";
...
//                }
                ...
                /*
                 *
                 * Save monthly depreciation entries and hit accounts ledger accordingly
                 *
                 */
                if ($depEntries == 0) {
                    $remarks = "Auto generate monthly depreciation expense of $depDays days on $currentDate using $methodName @ $depPercentage%";
                } else {
                    $remarks = "Auto generate monthly depreciation expense on $currentDate using $methodName @ $depPercentage%";
                }
                $devInsertQuery = "INSERT INTO financials_dep_expense_voucher(dev_exp_uid, dev_acc_dep_uid, dev_remarks, dev_day_end_id, dev_day_end_date, dev_user_id, dev_current_date_time,dev_clg_id)
                                VALUES ($depDepExpenseAccountUID, $depAccDepAssetAccountUID, '$remarks', $currentDayEndId, '$currentDayEndDate', $loginUserId, '$dbTimeStamp',$loginClgId);";
                ...
                if ($devInsertResult) {
                    ...
                    if ($fixedAssetUpdateResult && $database->affected_rows() == 1) {
                        ...
                        $newDepDepExpenseAccountBalance = $depDepExpenseAccountBalance + $monthlyDepExp;
                        $newDepAccDepAssetAccountBalance = $depAccDepAssetAccountBalance - $monthlyDepExp;

                        $depBalanceQuery = "INSERT INTO financials_balances (bal_account_id, bal_transaction_type, bal_remarks, bal_dr, bal_cr, bal_total, bal_transaction_id, bal_day_end_id, bal_day_end_date, bal_detail_remarks, bal_voucher_number, bal_user_id, bal_datetime,bal_clg_id)
                            VALUES ($depDepExpenseAccountUID, 'DEPRECIATION EXPENSE VOUCHER', '$remarks', $monthlyDepExp, 0, $newDepDepExpenseAccountBalance, 0, $currentDayEndId, '$currentDayEndDate', '$remarks', '$depVoucherCode', $loginUserId, '$dbTimeStamp','$loginClgId'),
                                   ($depAccDepAssetAccountUID, 'DEPRECIATION EXPENSE VOUCHER', '$remarks', 0, $monthlyDepExp, $newDepAccDepAssetAccountBalance, 0, $currentDayEndId, '$currentDayEndDate', '$remarks', '$depVoucherCode', $loginUserId, '$dbTimeStamp','$loginClgId');";
                        ...
                        if (!$depBalanceResult) {
                            $error = true;
                            $rollBack = true;
                            $monthlyDepreciationError = true;
                            $monthlyDepreciationErrorMessage .= "Unable to save Depreciation Expense into ledger account of " . $dep['fa_account_name'];
                            break;
                        }
                        ...
                        $rollBack = true;
                        $monthlyDepreciationError = true;
                        $monthlyDepreciationErrorMessage .= "Unable to reduce asset value of " . $dep['fa_account_name'];
                        break;
                    }
                    ...
                    $rollBack = true;
                    $monthlyDepreciationError = true;
                    $monthlyDepreciationErrorMessage .= "Unable to save Depreciation Expense Voucher of " . $dep['fa_account_name'];
                    break;
                }
            }
            ...
            if ($monthlyDepreciationError) {
                $monthlyDepreciationHTMPReport .= shortError($monthlyDepreciationErrorMessage);
            } else {
                $monthlyDepreciationHTMPReport .= shortSuccess("Monthly Depreciation occur Successfully.");
            }
...
if (!$error) {
    ...
    $opLevelQuery = "SELECT coa_code, coa_head_name, coa_level FROM financials_coa_heads WHERE coa_level = 1 AND coa_clg_id =$loginClgId;";
    ...
    while ($topLevelData = $database->fetch_assoc($topLevelResult)) {
        ...
        $parentQuery = "SELECT coa_code, coa_head_name, coa_level FROM financials_coa_heads WHERE coa_parent = $controlCode AND coa_clg_id=$loginClgId;";
        ...
        while ($parentData = $database->fetch_assoc($parentResult)) {
            ...
            $childQuery = "SELECT coa_code, coa_head_name, coa_level FROM financials_coa_heads WHERE coa_parent = $parentCode AND coa_clg_id=$loginClgId;";
...
//                        array('parent' => $parentCode, 'code' => $childCode, 'name' => "$childName", 'level' => 3, 'child' => array(), 'type' => '', 'opening' => 0.00, 'inwards' => 0.00, 'outwards' => 0.00, 'balance' => 0.00);
                ...
                $entryQuery = "SELECT
                                    ...
                                    WHERE account_parent_code = $childCode AND account_clg_id=$loginClgId;";
...
//                        $entryTodayOpeningBalance = $entryData['account_balance'];//mustafa
...
//                        if ($createPnL || $createBalanceSheet || $createPnlDistribution) {
                    ...
                    if ($entryCode == $stockAccountUID) {
                        if ($currentDayEndId == 1) {
                            $stockOpeningBalance = getAccountOpeningBalance($stockAccountUID,$loginClgId);
...
//                            array('parent' => $childCode, 'code' => $entryCode, 'name' => "$entryName", 'level' => 4, 'type' => $balanceType, 'opening' => $opening, 'inwards' => $inwards, 'outwards' => $outwards, 'balance' => $balanceBF);
    ...
    if ($dailyCompleteReport) {
        ...
        foreach ($trialViewList as $control) {
            ...
            if ($controlType == "DR") {
                $controlTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', 0, $controlCode, '$controlName', $controlLevel, $controlBalance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            } elseif ($controlType == "CR") {
                $controlTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', 0, $controlCode, '$controlName', $controlLevel, NULL, $controlBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            } else {
                if (startsWith($controlCode, ASSETS) || startsWith($controlCode, EXPENSES)) { // debit zero
                    $controlTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', 0, $controlCode, '$controlName', $controlLevel, 0, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                } else { // credit zero
                    $controlTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', 0, $controlCode, '$controlName', $controlLevel, NULL, 0, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                }
            }
            ...
            foreach ($parents as $parent) {
                ...
                if ($parentType == "DR") {
                    $parentTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $controlCode, $parentCode, '$parentName', $parentLevel, $parentBalance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                } elseif ($parentType == "CR") {
                    $parentTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $controlCode, $parentCode, '$parentName', $parentLevel, NULL, $parentBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                } else {
                    if (startsWith($parentCode, ASSETS) || startsWith($parentCode, EXPENSES)) { // debit zero
                        $parentTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $controlCode, $parentCode, '$parentName', $parentLevel, 0, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    } else { // credit zero
                        $parentTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $controlCode, $parentCode, '$parentName', $parentLevel, NULL, 0, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    }
                }
                ...
                foreach ($children as $child) {
                    ...
                    if ($childType == "DR") {
                        $childTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $parentCode, $childCode, '$childName', $childLevel, $childBalance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    } elseif ($childType == "CR") {
                        $childTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $parentCode, $childCode, '$childName', $childLevel, NULL, $childBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    } else {
                        if (startsWith($childCode, ASSETS) || startsWith($childCode, EXPENSES)) { // debit zero
                            $childTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $parentCode, $childCode, '$childName', $childLevel, 0, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp'),'$loginClgId');";
                        } else { // credit zero
                            $childTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $parentCode, $childCode, '$childName', $childLevel, NULL, 0, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                        }
                    }
                    ...
                    foreach ($entries as $entry) {
                        ...
                        if ($entryType == "DR") {
                            $entryTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $childCode, $entryCode, '$entryName', $entryLevel, $entryBalance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                        } elseif ($entryType == "CR") {
                            $entryTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $childCode, $entryCode, '$entryName', $entryLevel, NULL, $entryBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                        } else {
                            if (startsWith($entryCode, ASSETS) || startsWith($entryCode, EXPENSES)) { // debit zero
                                $entryTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $childCode, $entryCode, '$entryName', $entryLevel, 0, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                            } else { // credit zero
                                $entryTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                                    VALUES ('$closingType', $childCode, $entryCode, '$entryName', $entryLevel, NULL, 0, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                            }
                        }
    ...
    // save trial total
    if ($dailyCompleteReport) {
        $totalTBQuery = "INSERT INTO financials_trial_balance (tb_type, tb_parent_uid, tb_account_uid, tb_account_name, tb_account_level, tb_total_debit, tb_total_credit, tb_day_end_id, tb_day_end_date, tb_datetime,tb_clg_id)
                          VALUES ('$closingType', 0, 0, 'Total', 0, $trialTotalDebit, $trialTotalCredit, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
...
if (!$error) {
    ...
    // print_r($stockAtPurchaseRate);
    // print_r($stockAtAverageRate);
    // exit();
...
if ($dailyCompleteReport) {
    if (!$error) {
        ...
        $salesPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                            VALUES ('$closingType', $salesParentCode, $salesCode, '$salesName', $salesLevel, '', NULL, NULL, $salesAmount, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($salesChild as $child1) {
            ...
            $sales2PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                            VALUES ('$closingType', $sales2ParentCode, $sales2Code, '$sales2Name', $sales2Level, '', NULL, $sales2Amount, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            if (array_key_exists('child', $child1)) {
                ...
                foreach ($sales2Child as $child2) {
                    ...
                    $sales3PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                            VALUES ('$closingType', $sales3ParentCode, $sales3Code, '$sales3Name', $sales3Level, '', $sales3Amount, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $cgsPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $cgsParentCode, $cgsCode, '$cgsName', $cgsLevel, '', NULL, NULL, $cgsBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($cgsChild as $cgsChild1) {
            ...
            $cgs2PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $cgs2ParentCode, $cgs2Code, '$cgs2Name', $cgs2Level, '', NULL, $cgs2Balance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            if (array_key_exists('child', $cgsChild1)) {
                ...
                foreach ($cgs2Child as $cgsChild2) {
                    ...
                    $cgs3PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $cgs3ParentCode, $cgs3Code, '$cgs3Name', $cgs3Level, '', $cgs3Balance, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $grPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $grParentCode, $grCode, '$grName', $grLevel, '', NULL, NULL, $grBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $expPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $expParentCode, $expCode, '$expName', $expLevel, '$expType', NULL, NULL, $expBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($expChild as $expChild1) {
            ...
            $exp2PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $exp2ParentCode, $exp2Code, '$exp2Name', $exp2Level, '$exp2Type', NULL, $exp2Balance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            foreach ($exp2Child as $expChild2) {
                ...
                $exp3PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $exp3ParentCode, $exp3Code, '$exp3Name', $exp3Level, '$exp3Type', $exp3Balance, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                ...
                foreach ($exp3Child as $expChild3) {
                    ...
                    $exp4PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $exp4ParentCode, $exp4Code, '$exp4Name', $exp4Level, '$exp4Type', $exp4Balance, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $nopPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $nopParentCode, $nopCode, '$nopName', $nopLevel, '', NULL, NULL, $nopBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $revPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $revParentCode, $revCode, '$revName', $revLevel, '$revType', NULL, NULL, $revBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($revChild as $revChild1) {
            ...
            $rev2PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $rev2ParentCode, $rev2Code, '$rev2Name', $rev2Level, '$rev2Type', NULL, $rev2Balance, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            foreach ($rev2Child as $revChild2) {
                ...
                $rev3PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $rev3ParentCode, $rev3Code, '$rev3Name', $rev3Level, '$rev3Type', $rev3Balance, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                ...
                foreach ($rev3Child as $revChild3) {
                    ...
                    $rev4PnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', $rev4ParentCode, $rev4Code, '$rev4Name', $rev4Level, '$rev4Type', $rev4Balance, NULL, NULL, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $nplPnLQuery = "INSERT INTO financials_income_statement(
                                        is_closing_type, is_parent_uid, is_account_uid, is_account_name, is_level, is_type, is_amount1, is_amount2, is_amount3, is_day_end_id, is_day_end_date, is_current_datetime,is_clg_id)
                        VALUES ('$closingType', 0, 0, '$nplName', 0, '', NULL, NULL, $netProfitOrLossAbsoluteAmount, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
...
$allRegisteredProfitAndLossAccountsQuery = "SELECT group_concat(cr_profit_loss_account_uid) as pnlAccounts FROM financials_capital_register WHERE cr_clg_id=$loginClgId;";
...
if ($allRegisteredProfitAndLossAccountsResult && $database->num_rows($allRegisteredProfitAndLossAccountsResult) > 0) {
    ...
    $dieReport = $htmlHeader;
    $dieReport .= $headerSection;
    $dieReport .= error("Day End Execution Stop", "No registered capital found for Balance Sheet and Profit / Loss distribution!");
    $dieReport .= $htmlFooter;
...
//if ($createPnL || $createBalanceSheet || $createPnlDistribution) {
//if ($dailyCompleteReport) {
//    $accClosingResetQuery = "UPDATE financials_accounts SET
//                               account_today_opening_type = '',
//                               account_today_opening = 0,
//                               account_today_debit = 0,
//                               account_today_credit = 0,
//                               account_monthly_opening_type = '',
//                               account_monthly_opening = 0,
...
$accClosingResetQuery = "UPDATE financials_accounts SET
                               ...
                               account_today_debit = 0,
                               account_today_credit = 0
                            WHERE account_id > 0 AND account_clg_id=$loginClgId;";
$accClosingResetResult = $database->query($accClosingResetQuery);
if (!$accClosingResetResult) {
    $error = true;
    $rollBack = true;
...
if ($dailyCompleteReport) {
    if (!$error) {
        ...
        $bsAssetsQuery = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsAssetsParentCode, $bsAssetsCode, '$bsAssetsName', $bsAssetsLevel, '$bsAssetsType', $bsAssetsBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($bsAssetsChild as $bsAssetsChild1) {
            ...
            $bsAssets2Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsAssets2ParentCode, $bsAssets2Code, '$bsAssets2Name', $bsAssets2Level, '$bsAssets2Type', $bsAssets2Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            foreach ($bsAssets2Child as $bsAssetsChild2) {
                ...
                $bsAssets3Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsAssets3ParentCode, $bsAssets3Code, '$bsAssets3Name', $bsAssets3Level, '$bsAssets3Type', $bsAssets3Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                ...
                foreach ($bsAssets3Child as $bsAssetsChild3) {
                    ...
                    $bsAssets4Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsAssets4ParentCode, $bsAssets4Code, '$bsAssets4Name', $bsAssets4Level, '$bsAssets4Type', $bsAssets4Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    ...
                    $bsAssetsOCBQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                                        VALUES ($bsAssets4Code, '$bsAssets4Name', '$bsAssets4OpeningType', $bsAssets4OpeningBalance, $bsAssets4Inwards, $bsAssets4Outwards, $bsAssets4Balance, '$bsAssets4Type', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
        ...
        $bsAssets5Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', 0, 0, 'Total Assets', 0, '', $bsAssetBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $bsLiabilityQuery = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsLiabilityParentCode, $bsLiabilityCode, '$bsLiabilityName', $bsLiabilityLevel, '$bsLiabilityType', $bsLiabilityBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($bsLiabilityChild as $bsLiabilityChild1) {
            ...
            $bsLiability2Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsLiability2ParentCode, $bsLiability2Code, '$bsLiability2Name', $bsLiability2Level, '$bsLiability2Type', $bsLiability2Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            foreach ($bsLiability2Child as $bsLiabilityChild2) {
                ...
                $bsLiability3Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsLiability3ParentCode, $bsLiability3Code, '$bsLiability3Name', $bsLiability3Level, '$bsLiability3Type', $bsLiability3Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                ...
                foreach ($bsLiability3Child as $bsLiabilityChild3) {
                    ...
                    $bsLiability4Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsLiability4ParentCode, $bsLiability4Code, '$bsLiability4Name', $bsLiability4Level, '$bsLiability4Type', $bsLiability4Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    ...
                    $bsLiabilityOCBQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,bs_clg_id)
                                        VALUES ($bsLiability4Code, '$bsLiability4Name','$bsLiability4OpeningType', $bsLiability4OpeningBalance, $bsLiability4Inwards, $bsLiability4Outwards, $bsLiability4Balance, '$bsLiability4Type', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
        ...
        $bsEquityQuery = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsEquityParentCode, $bsEquityCode, '$bsEquityName', $bsEquityLevel, '$bsEquityType', $bsEquityBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        foreach ($bsEquityChild as $bsEquityChild1) {
            ...
            $bsEquity2Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsEquity2ParentCode, $bsEquity2Code, '$bsEquity2Name', $bsEquity2Level, '$bsEquity2Type', $bsEquity2Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
            ...
            foreach ($bsEquity2Child as $bsEquityChild2) {
                ...
                $bsEquity3Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsEquity3ParentCode, $bsEquity3Code, '$bsEquity3Name', $bsEquity3Level, '$bsEquity3Type', $bsEquity3Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                ...
                foreach ($bsEquity3Child as $bsEquityChild3) {
                    ...
                    $bsEquity4Query = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', $bsEquity4ParentCode, $bsEquity4Code, '$bsEquity4Name', $bsEquity4Level, '$bsEquity4Type', $bsEquity4Balance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
                    ...
                    $bsEquityOCBQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                                        VALUES ($bsEquity4Code, '$bsEquity4Name', '$bsEquity4OpeningType', $bsEquity4OpeningBalance, $bsEquity4Inwards, $bsEquity4Outwards, $bsEquity4Balance, '$bsEquity4Type', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
        ...
        if ($netProfitOrLossAbsoluteAmount > 0) {
            ...
            $bsCurrentPoLQuery = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', 0, 0, '$cPoL', 0, '', $netProfitOrLossAbsoluteAmount, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
...
//            $bsLiabilityBalance = $bsLiabilityBalance * (-1); // comment by muzammil
        }
        ...
        if($bsTotalLiabilityEquityBalance > 0){$bsTotalLiabilityEquityBalanceDisplay= number_format($bsTotalLiabilityEquityBalance,2);}else{$abc =abs
        ($bsTotalLiabilityEquityBalance);
            $bsTotalLiabilityEquityBalanceDisplay='('.number_format($abc,2).')';
        }
        $balanceSheetHTMLReport .= '
            <tr class="normalHover">
                <th class="text-left tbl_txt_59">
                ...
                <th class="text-right tbl_srl_18 gnrl-bg">
                    ' . $bsTotalLiabilityEquityBalanceDisplay . '
                </th>
            </tr>
        ';
        ...
        $bsLiabilityEquityQuery = "INSERT INTO financials_balance_sheet(
                                        bs_closing_type, bs_parent_uid, bs_account_uid, bs_account_name, bs_level, bs_type, bs_amount, bs_day_end_id, bs_day_end_date, bs_current_datetime,bs_clg_id)
                        VALUES ('$closingType', 0, 0, 'Total (Liabilities + Equity)', 0, '', $bsTotalLiabilityEquityBalance, $currentDayEndId, '$currentDayEndDate', '$dbTimeStamp','$loginClgId');";
        ...
        $bsDiff = abs((double)number_format($bsAssetBalance, 2, '.', '') - (double)number_format($bsTotalLiabilityEquityBalance, 2, '.', ''));// Muzammil Added + instead of -
...
if ($dailyCompleteReport) {
    if (!$error && $netProfitOrLossAbsoluteAmount > 0) {
        ...
        $capQuery = "SELECT cr_parent_account_uid, cr_capital_account_uid, cr_profit_loss_account_uid, cr_drawing_account_uid, cr_reserve_account_uid, cr_user_id, cr_partner_nature,
                    cr_is_custom_profit, cr_is_custom_loss, cr_fixed_profit_per, cr_fixed_loss_per, cr_remaning_profit_division, cr_remaning_loss_division FROM financials_capital_register WHERE cr_clg_id=$loginClgId;";
        ...
        if ($capResult && $numRows > 0) {
            ...
            if ($numRows == 1) {
                ...
                if ($netProfitOrLoss == "Profit") {
                    ...
                    $newProfitAndLossAccountBalance = $profitAndLossAccountBalance - $netProfitOrLossAbsoluteAmount;//+=- mustafa
                } else {
                ...
                $adminPnLBalanceQuery = "INSERT INTO financials_balances(
                                bal_account_id, bal_transaction_type, bal_dr, bal_cr, bal_total, bal_remarks, bal_transaction_id, bal_day_end_id, bal_day_end_date, bal_detail_remarks, bal_voucher_number, bal_user_id,bal_clg_id)
                                VALUES (
                ...
                if ($newProfitAndLossAccountBalance >= 0) {
                    // $newProfitAndLossAccountBalanceType = "CR"; // arbaz
                    $newProfitAndLossAccountBalanceType = "DR"; // mustafa
                } else {
                    // $newProfitAndLossAccountBalanceType = "DR"; // arbaz
                    $newProfitAndLossAccountBalanceType = "CR"; // mustafa
                }
                ...
                $adminPnLOCBalanceQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                        VALUES ($profitAndLossAccountUID, '$capAccountName', '$pnlOpType', $pnlOpBalance, $pnlOpInwards, $pnlOpOutwards, $newProfitAndLossAccountBalance, '$newProfitAndLossAccountBalanceType', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
                ...
                foreach ($printProfitDiv as $eachProfit) {
                    ...
                    $profitAndLossAccount = getAccountClosingBalance($eachProfitAndLossAccountUID);
                    ...
                    $adminPnLOCBalanceQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                        VALUES ($eachProfitAndLossAccountUID, '$capAccountName', '$pnlOpType', $pnlOpBalance, $pnlOpInwards, $pnlOpOutwards, $newProfitAndLossAccountBalance, '$newProfitAndLossAccountBalanceType', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
                ...
                if (number_format($undistributedProfitAndLossAmount, 2, '.', '') != 0.00) {
                    ...
                    $undistributedProfitLossAccount = getAccountClosingBalance($undistributedProfitLossAccountUID);
                    ...
                    $newUndistributedProfitLossAccountOCBalancesQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                        VALUES ($undistributedProfitLossAccountUID, 'Undistributed Profit & Loss', '$pnlOpType', $pnlOpBalance, $pnlOpInwards, $pnlOpOutwards, $newUndistributedProfitLossAccountBalance, '$newUndistributedProfitLossAccountBalanceType', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
                    ...
                    $undistributedProfitLossAccountBalancesQuery = "INSERT INTO financials_account_opening_closing_balance (aoc_account_uid, aoc_account_name, aoc_op_type, aoc_op_balance, aoc_inwards, aoc_outwards, aoc_balance, aoc_type, aoc_closing_type, aoc_day_end_id, aoc_day_end_date,aoc_clg_id)
                        VALUES ($undistributedProfitLossAccountUID, 'Undistributed Profit & Loss', '$pnlOpType', $pnlOpBalance, $pnlOpInwards, $pnlOpOutwards, $newUndistributedProfitLossAccountBalance, '$newUndistributedProfitLossAccountBalanceType', '$closingType', $currentDayEndId, '$currentDayEndDate','$loginClgId');";
...
if ($error || $rollBack) {
    ...
    $query = "UPDATE financials_day_end SET de_datetime_status = 'CLOSE', de_update_datetime = '$dbTimeStamp', de_createdby = $loginUserId, de_ip_adrs = '$ipAddress', de_brwsr_info = '$type'
                WHERE de_datetime_status = 'PROCESSING' AND de_id = $currentDayEndId AND de_clg_id=$loginClgId  ORDER BY de_id DESC LIMIT 1;";
    ...
    $lastDayOfMonth = $isThisLastDayOfMonth ? 1 : 0;

    $query = "UPDATE financials_day_end
                SET de_datetime_status = 'CLOSE', de_update_datetime = '$dbTimeStamp', de_createdby = $loginUserId, de_ip_adrs = '$ipAddress', de_brwsr_info = '$type', de_last_day_of_month = $lastDayOfMonth
                WHERE de_datetime_status = 'PROCESSING' AND de_id = $currentDayEndId AND de_clg_id=$loginClgId ORDER BY de_id DESC LIMIT 1;";
    ...
    if(!$result || $database->affected_rows() != 1) {
        ...
        $dieReport .= $balanceSheetHTMLReport;
        $dieReport .= $profitLossDistributionHTMLReport;
        $dieReport .= error("Day End Execution Stop", "Day End Stop and unable to process until the above Error(s) resolved! " . $dayClosingErrorMessage);
        $dieReport .= $htmlFooter;
...
//        if (!$isThisLastDayOfMonth) {
        if ($dailyCompleteReport) {
            ...
            try {
                ...
                $firstDayOfMonth = $isThisLastDayOfMonth ? 1 : 0;

                $date = new DateTime($currentDayEndDate);
                $nDate = $date->modify('+1 day');
                $newDate = $nDate->format(DB_DATE_STAMP_FORMAT);

                $query = "INSERT INTO financials_day_end (de_datetime_status, de_datetime, de_first_day_of_month, de_createdby, de_current_datetime, de_ip_adrs, de_brwsr_info,de_clg_id)
                        VALUES ('OPEN', '$newDate', $firstDayOfMonth, $loginUserId, '$dbTimeStamp', '$ipAddress', '$type','$loginClgId');";
            ...
            if (!$isThisLastDayOfMonth) {
                ...
                try {
                    ...
                    $query = "INSERT INTO financials_day_end (de_datetime_status, de_datetime, de_createdby, de_current_datetime, de_ip_adrs, de_brwsr_info,de_clg_id)
                        VALUES ('OPEN', '$newDate', $loginUserId, '$dbTimeStamp', '$ipAddress', '$type','$loginClgId');";
...
$successMessage = "";
if (!$error && !$rollBack) {
    $successMessage = successMessage("Day End Executed Successfully", "Day End Executed and report generated Successfully.");
}
 ...
 *******************************************************************************************************************************
 *
 * Day End executed successfully, now committing all changes
 *
 * ******************************************************************************************************************************
*/
$successMessage = "";
...
if (!$error && !$rollBack) {
    ...
    try {
        ...
        $fp = fopen($dayEndReportFolder . "/" . $currentDayEndIdMD5 . ".html","wb");
        ...
        fclose($fp);
        $dayEndHTMLReportCompleteURL = $subDomain . $dayEndFolderPath . "view_report.php?t=" . TYPE_DAY_END . "&n=" . $currentDayEndIdMD5;
    ...
    mailTo($userEmail, "Day End Report of $currentUserReadableDate", $mailContent);

    //----- Mail to Support -----
    $supportEmail ='reports@softagics.com';
    mailTo($supportEmail, "Day End Report of $currentUserReadableDate", $mailContent);

    $monthStatus = '';
    if ($isThisLastDayOfMonth && !$dailyCompleteReport) {
        $monthStatus = 'PENDING';
    }

    // save day end report url
    $dayEndReportURLQuery = "UPDATE financials_day_end SET de_report_url = '$dayEndHTMLReportCompleteURL', de_month_status = '$monthStatus' WHERE de_id = $currentDayEndId AND de_clg_id=$loginClgId LIMIT 1;";
