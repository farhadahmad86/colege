...
try {
    ...
    if (isset($jwt)) {
        ...
        $query = "SELECT user_role_id, user_role_title FROM financials_user_role order by user_role_title;";
        ...
        if ($result && $database->num_rows($result) > 0) {
            ...
            while ($role = $database->fetch_assoc($result)) {

                $dataArray[] = array('id' => (int)$role['user_role_id'], 'title' => $role['user_role_title']);

            }

            $response->data = $dataArray;
            $response->code = OK;
            $response->message = "User roles";
