...
$invoice_id = isset($_GET["cloud_id"]) ? $database->escape_value($_GET["cloud_id"]) : 0;
$return_type = isset($_GET["return_type"]) ? $database->escape_value($_GET["return_type"]) : 0;

if ($return_type == 0) {
	$query = "SELECT * FROM financials_sale_invoice WHERE si_id='$invoice_id'";
} elseif ($return_type == 1) {
	$query = "SELECT * FROM financials_sale_saletax_invoice WHERE ssi_id='$invoice_id'";
}
...
if ($return_type == 0) {
	$query_product = "SELECT * FROM financials_sale_invoice_items WHERE sii_invoice_id='$invoice_id'";
} elseif ($return_type == 1) {
	$query_product = "SELECT * FROM financials_sale_saletax_invoice_items WHERE ssii_invoice_id='$invoice_id'";
}
