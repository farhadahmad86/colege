...
try {
    ...
    if (isset($jwt)) {
        ...
        $query = "SELECT mg_id, mg_title FROM financials_modular_groups order by mg_title;";
        ...
        if ($result && $database->num_rows($result) > 0) {
            ...
            while ($mGroup = $database->fetch_assoc($result)) {

                $dataArray[] = array('id' => (int)$mGroup['mg_id'], 'title' => $mGroup['mg_title']);

            }

            $response->data = $dataArray;
            $response->code = OK;
            $response->message = "Modular groups.";

        } else {
            $response->data = array();
            $response->code = DATA_EMPTY;
            $response->message = "No modular groups found!";
        }
