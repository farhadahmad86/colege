...
if(empty($text)) {
    $query = "SELECT pro_id as id, pro_title as title, pro_remarks as remarks, c.cat_id as cid, c.cat_title as ctitle, c.cat_remarks as cremarks
          FROM product 
...
if($result) {
    ...
    while ($region = $database->fetch_array($result)) {
        ...
        $category = array('id' => $region['cid'], 'title' => $region['ctitle'], 'remarks' => $region['cremarks']);

        $response->data[] = array('id' => $region['id'], 'title' => $region['title'], 'remarks' => $region['remarks'], 'category' => $category);
    }
