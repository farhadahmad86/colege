...
if($catId != 0) {
    $query = "SELECT pro_id as id, pro_title as title FROM product WHERE  pro_category_id = $catId ORDER BY pro_title;";
} else {
    $query = "SELECT pro_id as id, pro_title as title FROM product ORDER BY pro_title;";
}
...
if($result) {
    ...
    while ($region = $database->fetch_array($result)) {
        $response->data[] = array('id' => $region['id'], 'title' => $region['title']);
    }
