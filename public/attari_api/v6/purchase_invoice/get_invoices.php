...
if(!empty($queryDate)) {
    $searchQuery .= " AND (puri_datetime >= '$queryStartDate' AND puri_datetime <= '" . str_replace("59:59:59","23:59:59", $queryEndDate) . "')";
}
...
$query = "SELECT 
              ...
              puri_product_id as pro_id,
              pro_title as product, 
              puri_category_id as cat_id,
              cat_title as category, 
              puri_party_id as party_id,
              prty_title as party, 
            ...
            $searchQuery
            ORDER BY puri_datetime DESC LIMIT $limit OFFSET $offset;";
...
if($result) {
    ...
    while ($inv = $database->fetch_array($result)) {
        ...
        $product = array('id' => $inv['pro_id'], 'title' => $inv['product'], 'type' => $inv['type']);
        $category = array('id' => $inv['cat_id'], 'title' => $inv['category']);
        $party = array('id' => $inv['party_id'], 'title' => $inv['party']);
    ...
    $sumQuery = "SELECT sum(puri_total_purchase_price) as total_purchase 
                    FROM purchase_invoice puri
                    JOIN product p ON p.pro_id = puri.puri_product_id
                    JOIN category c ON c.cat_id = puri.puri_category_id
                    JOIN party pr ON pr.prty_id = puri.puri_party_id 
    ...
    if(empty($response->data)) {
        ...
        $response->code = OK;
        $response->message = "Total Purchase: " . number_format($totalPurchaseAmount, 2, '.', ',');
    }
