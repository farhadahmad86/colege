...
if(empty($text)) {
    $query = "SELECT prty_id as id, prty_title as title, prty_remarks as remarks, prty_phone as phone, prty_address as address FROM party ORDER BY prty_title LIMIT $limit OFFSET $offset;";
} else {
    $query = "SELECT prty_id as id, prty_title as title, prty_remarks as remarks, prty_phone as phone, prty_address as address FROM party  
...
if($result) {
    ...
    while ($region = $database->fetch_array($result)) {
        $response->data[] = array('id' => $region['id'], 'title' => $region['title'], 'remarks' => $region['remarks'], 'phone' => $region['phone'], 'address' => $region['address']);
    }
