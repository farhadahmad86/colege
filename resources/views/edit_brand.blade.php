        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text">Update Brand</h4>
                        </div>
                    </div>
                </div><!-- form header close -->

                <form name="f1" class="f1" id="f1" action="update_brand" onsubmit="return checkForm()" method="post">
                    @csrf
                    <div class="row">
...
config('fields_info.about_form_fields.party_registration.region.name.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.region.name.validations') }}</p>">
                                            ...
                                            Brand Title</label>
                                        <input type="text" name="brand_name" id="brand_name" class="inputs_up form-control"
                                               data-rule-required="true" data-msg-required="Please Enter Brand Title"
                                               placeholder="Brand Title" autofocus value="{{$request->title}}" autocomplete="off"/>
...
config('fields_info.about_form_fields.party_registration.general_fields.remarks.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.general_fields.remarks.validations') }}</P>">
                                ...
                                <input value="{{$request->brand_id}}" type="text" name="brand_id">
    ...
    <script type="text/javascript">
        ...
        function form_validation()
        {
            ...
            if(brand_name.trim() == "")
            {
                ...
                if (focus_once == 0) { jQuery("#brand_name").focus(); focus_once = 1;}
                flag_submit = false;
            }else{
