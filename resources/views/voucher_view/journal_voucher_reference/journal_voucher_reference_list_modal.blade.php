...
{{--                </th>--}}
                <th scope="col"  class="text-center tbl_txt_35">
                    Account Name</th>
                <th scope="col"  class="text-center tbl_txt_32">
                    Remarks
                </th>
...
{{--                    <td>--}}
{{--                        {{ $item->jvri_account_id }}--}}
{{--                    </td>--}}
                    <td>
                        {!! $item->jvri_account_name !!}
                    </td>
                    <td>
                        {!! $item->jvri_remarks !!}
                    </td>
                    <td align="right" class="align_right text-right">
                        @php
                            $DR_val = ($item->jvri_type === "Dr") ? $item->jvri_amount : "";
                            $DR = ($item->jvri_type === "Dr") ? $DR_val : "0";
                            $ttl_dr = +$ttl_dr + +$DR;
                        @endphp
                    ...
                    <td align="right" class="align_right text-right">
                        @php
                            $CR_val = ($item->jvri_type === "Cr") ? $item->jvri_amount : "";
                            $CR = ($item->jvri_type === "Cr") ? $CR_val : "0";
                            $ttl_cr = +$ttl_cr + +$CR;
                        @endphp
        ...
        @if( $type === 'grid')
            <div class="itm_vchr_rmrks">
                <a href="{{ route('journal_voucher_reference_items_view_details_pdf_SH',['id'=>$jrnl->jvr_id]) }}" class="align_right text-center btn btn-sm btn-info" style="float: left;margin-top:
                7px;">
