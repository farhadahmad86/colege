...
{{--                Account No.--}}
{{--            </th>--}}
            <th scope="col"  class="text-left align_left tbl_txt_26">
                Account Name
            </th>
            <th scope="col"  class="text-left align_left tbl_txt_20">
                Posting Reference
            </th>
            <th scope="col"  class="text-left align_left tbl_txt_26">
                Remarks
        ...
        @foreach( $items as $item )
            ...
            @php
                $vchr_id = $item->cri_salary_payment_voucher_id;

                $DR_val = ($vchr_id === $item->cri_salary_payment_voucher_id) ? $item->cri_amount : "";
            ...
            @if( $vchr_id !== $item->cri_salary_payment_voucher_id)
                <tr class="even pointer">
                    <th>
                        {{ $i }}
                    </th>
                    <td>
                        {{ $cr_acnt_nme->account_uid }} - {!! $cr_acnt_nme->account_name !!}
                    </td>
{{--                    <td>--}}
...
{{--                    </td>--}}
                    ...
                    <td align="right" class="align_right text-right">
                        {{ (!empty($csh_rcpt->cr_total_amount)) ? number_format($csh_rcpt->cr_total_amount,2) : "" }}
                    </td>
                    <td align="right" class="align_right text-right">
                    </td>
                </tr>
...
{{--                </td>--}}
    ...
    @if( $type === 'grid')
        <div class="itm_vchr_rmrks">
            ...
            <a href="{{ route('cash_receipt_items_view_details_pdf_SH',['id'=>$csh_rcpt->cr_id]) }}" class="align_right text-center btn btn-sm btn-info" style="float: left;margin-top: 7px;">
