...
@foreach ($fee_vouchers as $fee_voucher)
    ...
    $number = $fee_voucher->afv_total_amount;
    $locale = 'en_US';
            ...
            <div class="size col-xs-4" style="margin-top:12px;margin-left: 40px; border-right: 2px solid black;" >
                <div class="row" id="logo-row">
                    ...
                    <div class="" style="text-align: center; margin-top:10px;">
                        ...
                        <h5><strong>{{ $fee_voucher->branch_name }}</strong></h5>
                        <h5><strong>BANK COPY</strong></h5>
                        <h5><strong>(CASH DEPOSIT SLIP)</strong></h5>
                        <h5><strong>{{ date('d-M-y', strtotime(str_replace('/', '-', $fee_voucher->afv_created_datetime))) }}</strong></h5>
                    </div>
                </div>
...
{{--                            <th class="th-1">--}}
{{--                                {{ date('d-M-y', strtotime(str_replace('/', '-', $fee_voucher->fv_issue_date))) }}--}}
{{--                            </th>--}}
...
{{--                    </table>--}}
                    ...
                    <h2 class="FID_Title">FID: {{ $fee_voucher->afv_v_no }}</h2>
                ...
                <div class="row" style="line-height: 0px">
                    ...
                    <h4 class="name_font">
                        Father Name: {{ $fee_voucher->father_name }}</h4>
                    <p><strong>Admission #: {{ $fee_voucher->registration_no }}</strong>
                    </p>
                    <p><strong>Class:</strong> {{ $fee_voucher->class_name }} - {{$fee_voucher->cs_name}}
                    </p>
                    <div style="width:95%; padding:0 0 2% 0">
                        <table style="line-height:15px; border:2px solid;">
                            ...
                            <tr></tr>
                            <tr></tr>
                            @if($fee_voucher->afv_t_fee > 0)
                                <tr>
                                    <td>Advance Tution Fee</td>
                                    <td>{{ $fee_voucher->afv_t_fee }}</td>
                                </tr>
                            @endif

                            @if($fee_voucher->afv_fund > 0)
                                <tr>
                                    <td>Advance Fund</td>
                                    <td>{{ $fee_voucher->afv_fund }}</td>
                                </tr>
                            @endif
                            ...
                            <tr>
                                ...
                                <td><strong>{{ $fee_voucher->afv_total_amount }}</strong></td>
