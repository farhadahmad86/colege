    ...
    <div id="" class="table-responsive">
        ...
        <table class="table table-bordered table-sm">
            ...
            <tr class="headings vi_tbl_hdng">
                <th scope="col" class="text-center align_center tbl_srl_4">Sr.</th>
                {{--            <th scope="col" class="text-center align_center tbl_amnt_10">Account No.</th>--}}
                <th scope="col" class="text-left align_left tbl_txt_26">Account Name</th>
                <th scope="col" class="text-left align_left tbl_txt_20">Posting Reference</th>
                <th scope="col" class="text-left align_left tbl_txt_26">Remarks</th>
                <th scope="col" class="text-center align_center tbl_amnt_12">Dr.</th>
                <th scope="col" class="text-center align_center tbl_amnt_12">Cr.</th>
            </tr>
            </thead>
            ...
            @foreach( $items as $item )
                ...
                @php
                    $vchr_id = $item->cpi_salary_payment_voucher_id;

                    $DR_val = ($vchr_id === $item->cpi_salary_payment_voucher_id) ? $item->cpi_amount : "";
                    $DR = (!empty($DR_val)) ? $DR_val : "0";
                    $grand_total = +(+$grand_total + +$DR);

                    if( $item->cpi_deduct_amount > 0 ):
                        $adv_amnt = $item->cpi_deduct_amount;
                    endif;
                ...
                @if( $vchr_id !== $item->cpi_salary_payment_voucher_id)
                    <tr class="even pointer">
                        <th>
                            {{ $i }}
                        </th>
                        <td>
                            {{ $cp_acnt_nme->account_uid }} - {!! $cp_acnt_nme->account_name !!}
                        </td>
                        {{--                    <td>--}}
                        {{--                        {!! $cp_acnt_nme->account_name !!}--}}
                        {{--                    </td>--}}
                        <td>

                        </td>
                        <td>
                            {!! $csh_pymnt->cp_remarks !!}
                        </td>
                        <td align="right" class="align_right text-right">
                        </td>
                        <td align="right" class="align_right text-right">
