    ...
    <div id="" class="table-responsive">
        ...
        <table class="table table-bordered table-sm">
            ...
            @foreach( $items as $item )
                ...
                @php
                    $vchr_id = $loan_rcpt->lrv_id;

                    //$CR_val = ($vchr_id === $item->lvi_voucher_id) ? $item->account_amount : "";
                    $DR_val =  $item->account_amount;
                    $CR = (!empty($CR_val)) ? $CR_val : "0";
                    $grand_total = +(+$grand_total + +$CR);

                    $ttl_dr = +$ttl_dr + +$DR_val;
                ...
                <tr class="even pointer">
                    ...
                    <td>
                        {!! $item->posting_reference_name !!}
                    </td>
                    <td>
                        {!! $item->account_remarks !!}
                    </td>
                    <td align="right" class="align_right text-right">
                        {{ (!empty($item->account_amount)) ? number_format($item->account_amount,2) : ""  }}
                    </td>
                    <td align="right" class="align_right text-right">
            ...
            <tr class="even pointer">
                ...
                <td align="right" class="align_right text-right">
                    {{ (!empty($loan_rcpt->lrv_total_amount)) ? number_format($loan_rcpt->lrv_total_amount,2) : "" }}
                </td>
            </tr>
            ...
            <tr class="border-0">
                ...
                <td class="text-right align_right border-left-0 border-right-0 border-top-2 border-bottom-3" align="right">
                    {{ number_format($loan_rcpt->lrv_total_amount,2) }}
                </td>
            </tr>
        ...
        @if( $type === 'grid')
            <div class="itm_vchr_rmrks">
                ...
                <a href="{{ route('loan_receipt_items_view_details_pdf_SH',['id'=>$loan_rcpt->lrv_id]) }}" class="align_right text-center btn btn-sm btn-info" style="float: left;margin-top: 7px;">
