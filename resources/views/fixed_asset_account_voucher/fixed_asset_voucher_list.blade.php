        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Fixed Asset Voucher List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ ( !empty($search) || !empty($search_to) || !empty($search_from) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('fixed_asset_voucher_list') }}" name="form1" id="form1" method="post">
                        @csrf
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx"><!-- start input box -->
                                            ...
                                            <select tabindex="4" name="fixed_account" class="inputs_up form-control" id="fixed_account">
                                                <option value="">Select Fixed Asset Account</option>
                                                @foreach($fixedAssetAccountList as $fixed_account)
                                                    <option value="{{$fixed_account['code']}}" {{ $fixed_account['code'] == $search_fixed_account ? 'selected="selected"' :
                                                            ''}}>{{$fixed_account['name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12 mt-lg-4 text-right">
                                        ...
                                        <x-add-button tabindex="9" href="{{ route('fixed_asset_voucher') }}"/>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $voucher)
                            @php $per_page_ttl_amnt = +$voucher->fav_total_amount + +$per_page_ttl_amnt; @endphp
                            ...
                            <tr>
                                ...
                                <td class="view" data-id="{{$voucher->fav_id}}">
                                    FAV-{{$voucher->fav_id}}
                                </td>
                                <td>
                                    {{$voucher->fav_remarks}}
                                ...
                                <td>
                                    {!! str_replace("&oS;",'<br />', $voucher->fav_detail_remarks) !!}
                                </td>
                                <td class="align_right text-right">
                                    {{$voucher->fav_total_amount !=0 ? number_format($voucher->fav_total_amount,2):''}}
                                </td>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content base_clr">
                <div class="modal-header">
                    <h4 class="modal-title text-black">Fixed Asset Voucher Detail</h4>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('fixed_asset_voucher_list') }}',
            url;
    ...
    <script>
        jQuery("#cancel").click(function () {
            ...
            $("#fixed_account").select2().val(null).trigger("change");
            $("#fixed_account > option").removeAttr('selected');
        });

        jQuery("#fixed_account").select2();
    </script>
