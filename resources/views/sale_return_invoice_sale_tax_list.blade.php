 ...
 <div class="row">
        <div class="container-fluid search-filter form-group form_manage" id="">
            ...
            <div class="search_form m-0 p-0">
                <form class="highlight prnt_lst_frm" action="{{ route('sale_tax_sale_return_invoice_list') }}" name="form1" id="form1" method="post">
                    @csrf
                    <div class="row">
...
<x-add-button tabindex="9" href="{{ route('simple_sale_tax_return_invoice') }}"/>
            ...
            <div class="table-responsive" id="printTable">
                <table class="table table-bordered table-sm" id="fixTable">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="text-center tbl_amnt_5">
                            Cash Paid
                        </th>
                    ...
                    @forelse($datas as $invoice)
                        ...
                        <tr>
                            ...
                            <td class="view" data-id="{{$invoice->srsi_id}}">
                                STSRI-{{$invoice->srsi_id}}
                            </td>

                            @php
                                $party_name = $invoice->srsi_party_name;
                        ...
                        $party_name = $invoice->account_urdu_name;
                            ...
                            <td class="fonti">
                                {{$invoice->srsi_remarks}}
                            </td>
                            <td class="fonti">
                                {!! str_replace("&oS;",'<br />', $invoice->srsi_detail_remarks) !!}
                            </td>
                            @php
                                $ttlPrc = +($invoice->srsi_grand_total) + +$ttlPrc;
                            @endphp
                            <td class="text-right ">
                                {{$invoice->srsi_grand_total !=0 ? number_format($invoice->srsi_grand_total,2):''}}
                            </td>

                            @php
                                $cashPaid = +($invoice->srsi_cash_received) + +$cashPaid;
                            @endphp
                            <td class="text-right">
                                {{$invoice->srsi_cash_received !=0 ? number_format($invoice->srsi_cash_received,2):''}}
                            </td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('sale_tax_sale_return_invoice_list') }}',
            url;
