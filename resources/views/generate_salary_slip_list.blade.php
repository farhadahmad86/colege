        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Generate Salary Slip Voucher List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ ( !empty($search) || !empty($search_to) || !empty($search_from)|| !empty($search_month) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('generate_salary_slip_voucher_list') }}" name="form1" id="form1" method="post">
                        @csrf
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-2 col-md-12 col-sm-12 col-xs-12 text-right mt-4">
                                        ...
                                        <!-- Call add button component -->
                                            <x-add-button tabindex="9" href="{{ route('generate_salary_slip_voucher_with_load') }}"/>
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $voucher)
                            ...
                            <tr>
                                ...
                                @php
                                    $ip_browser_info= ''.$voucher->gss_ip_adrs.','.str_replace(' ','-',$voucher->gss_brwsr_info).'';
                                @endphp
        ...
        <div class="modal-dialog modal-lg mdl_wdth">
            <div class="modal-content base_clr">
                <div class="modal-header">
                    <h4 class="modal-title text-black">Generate Salary Slip Voucher Detail</h4>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('generate_salary_slip_voucher_list') }}',
            url;
    ...
    <script>
        ...
        jQuery(".view").click(function () {
            ...
            $(".modal-body").load('{{ url('generate_salary_slip_items_view_details/view/') }}/' + id, function () {
                // jQuery(".pre-loader").fadeToggle("medium");
            ...
            {{--jQuery.ajax({--}}
            {{--    url: "{{ route('salary_slip_items_view_details') }}",--}}
            {{--    data: {id: id},--}}
            {{--    type: "POST",--}}
            {{--    cache: false,--}}
            {{--    dataType: 'json',--}}
            ...
            {{--            var amnt = (value['ssi_amount'] !== "") ? value['ssi_amount'] : 0;--}}
            {{--            ttlAmnt = amnt + ttlAmnt;--}}

            {{--            jQuery("#table_body").append(--}}
            {{--            '<tr>' +--}}
            {{--            '<td class="align_left wdth_5">' + value['ssi_account_id'] + '</td>' +--}}
            {{--            '<td class="align_left wdth_2"> <div class="max_txt"> ' + value['ssi_account_name'] + '</div> </td>' +--}}
            {{--            '<td class="align_left text-right wdth_5">' + value['ssi_amount'] + '</td>' +--}}
            {{--            '</tr>');--}}
