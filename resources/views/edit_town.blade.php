            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                    <div class="form_header"><!-- form header start -->
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 class="text-white get-heading-text">Edit Town</h4>
                            </div>
                            <div class="list_btn">
                                <a class="btn list_link add_more_button" href="{{ route('town_list') }}" role="button">
                    ...
                    <form name="f1" class="f1" id="f1" action="{{ route('update_town') }}" onsubmit="return checkForm()" method="post" autocomplete="off">
                        @csrf
                        <div class="row">
...
config('fields_info.about_form_fields.party_registration.region.name.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.region.name.validations') }}</p>">
                                                        ...
                                                        <a class="add_btn"  id="refresh_region" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-html="true" data-content="{{config('fields_info.about_form_fields.refresh.description')}}">
                                                            <l class="fa fa-refresh"></l>
                                                        </a>
                                                    </label>
                                                    <select name="region_name" class="inputs_up form-control" id="region_name"
...
config('fields_info.about_form_fields.party_registration.area.area_title.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.area.area_title.validations') }}</p>">
                                                    ...
                                                    <select name="area_name" class="inputs_up form-control" id="area_name"
                                                            data-rule-required="true" data-msg-required="Please Enter Area"
                                                    >
...
config('fields_info.about_form_fields.party_registration.sector.sector_title.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.sector.sector_title.validations') }}</p>">
                                                    ...
                                                    <select name="sector_name" class="inputs_up form-control" id="sector_name"
                                                            data-rule-required="true" data-msg-required="Please Enter Sector"
                                                    >
                                        ...
                                        <h6>Example</h6><p>{{config('fields_info.about_form_fields.town.example')}}</p>">
                                                        ...
                                                        Town Name</label>
                                                    <input type="text" name="town_name" id="town_name" class="inputs_up form-control"
                                                           data-rule-required="true" data-msg-required="Please Enter Town"
                                                           placeholder="Town Name" value="{{$request->town_name}}">
                                    ...
                                    <input type="hidden" name="town_id" id="town_id" value="{{$request->town_id}}">
...
config('fields_info.about_form_fields.party_registration.general_fields.remarks.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.general_fields.remarks.validations') }}</P>">
                                                        ...
                                                        Remarks</label>
                                                    <textarea name="remarks" id="remarks" class="inputs_up remarks form-control" placeholder="Remarks">{{$request->town_remarks}}</textarea>
...
@section('scripts')
    ...
    <script>
        jQuery("#region_name").change(function () {
            ...
            var get_area_id = '{{$request->area_id}}';
        ...
        jQuery("#area_name").change(function () {
            ...
            var sector_id = '{{$request->sector_id}}';
            ...
            jQuery.ajax({
                url: "get_sector",
                data: {area_id: area_id, sector_id: sector_id},
                type: "POST",
                cache: false,
                dataType: 'json',
                success: function (data) {
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            var reg_id = '{{$request->region_id}}';
            var area_id = '{{$request->area_id}}';
            var sector_id = '{{$request->sector_id}}';
            ...
            jQuery.ajax({
                url: "get_area",
                data: {reg_id: reg_id, area_id: area_id},
                type: "POST",
                cache: false,
                dataType: 'json',
                success: function (data) {
