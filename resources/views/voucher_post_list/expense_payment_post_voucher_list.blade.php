        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Post Expense Payment Voucher List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ ( !empty($search) || !empty($search_to) || !empty($search_from) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('expense_payment_voucher_list') }}" name="form1" id="form1" method="post">
                        @csrf
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx"><!-- start input box -->
                                            ...
                                            <select tabindex="4" name="account" class="inputs_up form-control" id="account">
                                                <option value="">Select Account</option>
                                                @foreach($accounts as $account)
                                                    <option value="{{$account->account_uid}}" {{ $account->account_uid == $search_account ? 'selected="selected"' :
                                    ...
                                    <!-- Call add button component -->
                                        <x-add-button tabindex="9" href="{{ route('expense_payment_voucher') }}"/>
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $voucher)
                            ...
                            <tr data-toggle="tooltip" data-placement="top" title="" data-original-title="View Journal Voucher">
                                ...
                                <td data-id="{{$voucher->ep_id}}" class="view">
                                    EPV-{{$voucher->ep_id}}
                                </td>
                                <td>
                                    {{$voucher->ep_remarks}}
                                ...
                                <td>
                                    {!! str_replace("&oS;",'<br />', $voucher->ep_detail_remarks) !!}
                                </td>
                                ...
                                <td class="usr_prfl text-center" title="Click To Post Voucher">
                                    <form method="post" action="{{ route('post_expense_payment_voucher',$voucher->ep_id) }}">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $voucher->ep_id }}">
                ...
                <div class="modal-footer">
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="form_controls">
                            <button type="button" class="btn btn-default cancel_button btn btn-sm btn-secondary" data-dismiss="modal">
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('expense_payment_post_voucher_list') }}',
            url;
    ...
    <script>
        ...
        jQuery(".view").click(function () {
            ...
            {{--jQuery.ajax({--}}
            {{--    url: "{{ route('expense_payment_items_view_details') }}",--}}
            {{--    data: {id: id},--}}
            {{--    type: "POST",--}}
            {{--    cache: false,--}}
            {{--    dataType: 'json',--}}
            ...
            {{--            var amnt = (value['epi_amount'] !== "") ? value['epi_amount'] : 0;--}}
            {{--                ttlAmnt = ttlAmnt + amnt;--}}

            {{--            jQuery("#table_body").append(--}}
            {{--            '<tr>' +--}}
            {{--            '<td class="align_left wdth_5">' + value['epi_account_id'] + '</td>' +--}}
            {{--            '<td class="align_left wdth_2"> <div class="max_txt"> ' + value['epi_account_name'] + '</div> </td>' +--}}
            {{--            '<td class="align_left text-right wdth_5">' + value['epi_amount'] + '</td>' +--}}
            {{--            '</tr>');--}}
            {{--        //--}}
            {{--        });--}}
