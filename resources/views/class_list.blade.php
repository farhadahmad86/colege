...
{{--                <div class="search_form {{ ( !empty($search) || !empty($search_class) ) ? '' : 'search_form_hidden' }}">--}}
{{--                    <div class="row">--}}
                <div class="search_form">

                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">

                            <form class="prnt_lst_frm" action="{{ route('class_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1" method="post">
                                <div class="row">
                                    @csrf
                                    ...
                                    <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                                        <div class="form_controls text-left">
                                            ...
                                            <a tabindex="4" class="save_button form-control" href="{{ route('add_class') }}" role="button">
                                                <l class="fa fa-plus"></l> Class
                                            </a>
                            ...
                            <form name="edit" id="edit" action="{{ route('edit_class') }}" method="post">
                                @csrf
                                <input tabindex="5" name="name" id="name" type="hidden">
                                <input tabindex="6"name="remarks" id="remarks" type="hidden">
                                <input tabindex="7" name="class_id" id="class_id" type="hidden">

                            </form>

                            <form name="delete" id="delete" action="{{ route('delete_class') }}" method="post">
                                @csrf
                                <input tabindex="8" name="cla_id" id="cla_id" type="hidden">
                            </form>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-sm" id="fixTable">
                        ...
                        @forelse($datas as $data)
                            ...
                            <tr data-title="{{$data->cla_title}}" data-remarks="{{$data->cla_remarks}}" data-class_id="{{$data->cla_id}}">
                                ...
                                @php
                                    $ip_browser_info= ''.$data->cla_ip_adrs.','.str_replace(' ','-',$data->cla_brwsr_info).'';
                                @endphp
                                ...
                                <td class="align_right text-right hide_column tbl_amnt_6">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($data->cla_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $data->cla_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$data->cla_id}}"
                                            {{ $data->cla_disabled == 0 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>

                                <td class="align_right text-right hide_column tbl_srl_6">
                                    <a data-class_id="{{$data->cla_id}}" class="delete"  data-toggle="tooltip" data-placement="left"  title="" data-original-title="Are you sure?">
                                        <i class="fa fa-{{$data->cla_delete_status == 1 ? 'undo':'trash'}}"></i>
                                    </a>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="11">
                                    <center><h3 style="color:#554F4F">No Class</h3></center>
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
        ...
        $(document).ready(function () {
            $('.enable_disable').change(function () {
                ...
                $.ajax({
                    ...
                    data: {'status': status, 'cla_id': pubId},
