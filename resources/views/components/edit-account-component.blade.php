<div class="invoice_col_bx">
    ...
    <div class="invoice_col_input refresh-addition">
        ...
        <div class="invoice_col_short">
            ...
            <a href="{{ route($href) }}" target="_blank" class="col_short_btn add" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-html="true" data-content="{{config('fields_info.about_form_fields.add.description')}}">
                <i class="fa fa-plus"></i>
            </a>
            <a id="refresh_account_name" class="col_short_btn refresh" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-html="true" data-content="{{config('fields_info.about_form_fields.refresh.description')}}" onclick="refresh_account('{{$id}}','{{$title}}','{{$array_index}}','{{$class}}')">
        ...
        <select tabindex="{{$tabindex}}" name="{{$name}}" class="inputs_up form-control {{$class}}" id="{{$id}}" data-rule-required="true" data-msg-required="Please Enter {{$title}}">
            ...
            @foreach($accounts as $account)
                <option value="{{$account->account_uid}}" {{$value == $account->account_uid ? 'selected=selected':''}}>{{$account->$account_show}}</option>
            @endforeach
        </select>
        ...
        jQuery.ajax({
            ...
            , success: function(data) {
                ...
                $.each(data, function(index, value) {
                    option += "<option value='" + value.account_uid + "'>" + value.account_name + "</option>";
                });
                $.each(data, function(index, value) {
