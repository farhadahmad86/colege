    ...
    <div class="row">
            <div class="container-fluid search-filter form-group form_manage" id="">
                <div class="form_header">
                    ...
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Service Tax Invoice List</h4>
                        </div>
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('service_tax_invoice_list') }}" name="form1"
                            ...
                            @forelse($datas as $invoice)
                                <tr>
                                    ...
                                    <td class="align_left text-left tbl_txt_13">
                                        {{ $invoice->sesi_remarks }}
                                    </td>
                                    <td class="align_left text-left tbl_txt_27">
                                        {{--                                            {!! str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'<br />', $invoice->sesi_detail_remarks) !!} --}}
                                        {!! str_replace('&oS;', '<br />', $invoice->sesi_detail_remarks) !!}
                                    </td>
                                    @php
                                        $ttlPrc = +$invoice->sesi_total_price + +$ttlPrc;
                                    @endphp
                                    <td align="right" class="align_right text-right tbl_amnt_5">
                                        {{ $invoice->sesi_total_price != 0 ? number_format($invoice->sesi_total_price, 2) : '' }}
                                    </td>
                                    @php
                                        $cashPaid = +$invoice->sesi_cash_received + +$cashPaid;
                                    @endphp
                                    <td align="right" class="align_right text-right tbl_amnt_5">
                                        {{ $invoice->sesi_cash_received != 0 ? number_format($invoice->sesi_cash_received, 2) : '' }}
                                    </td>

                                    @php
                                        $ip_browser_info = '' . $invoice->sesi_ip_adrs . ',' . str_replace(' ', '-', $invoice->sesi_brwsr_info) . '';
                                    @endphp
        ...
        <div class="modal-dialog modal-lg mdl_wdth">
            <div class="modal-content base_clr">
                <div class="modal-header">
                    <h4 class="modal-title text-black">Service Tax Invoice Detail</h4>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('service_tax_invoice_list') }}',
            url;
    ...
    <script>
        ...
        jQuery(".view").click(function() {
            ...
            $('.modal-body').load('{{ url('services_tax_items_view_details/view/') }}' + '/' + id, function() {
                $("#myModal").modal({
                    show: true
                });
            });
            ...
            //
            // jQuery.ajax({
            //     url: "services_items_view_details",
            //     data: {id: id},
            //     type: "POST",
            //     cache: false,
            //     dataType: 'json',
            ...
            //             }
            //
            //             jQuery("#table_body").append('<tr><td class="align_center">' + value['seii_service_code'] + '</td><td class="align_left">' + value['seii_service_name'] + '</td><td class="align_right">' + qty + '</td><td class="align_right">' + rate + '</td><td class="align_right">' +  discount + '</td><td class="align_right">' + saletax + '</td><td class="align_right">' + amount + '</td><td class="align_left">' + value['seii_remarks'] + '</td></tr>');
            //
            //         });
            //     },
