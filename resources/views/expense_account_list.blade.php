        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            ...
            <!-- <div class="search_form {{ ( !empty($search) || !empty($search_first_head) || !empty($search_second_head) || !empty($search_third_head) || !empty($search_group) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('expense_account_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1"
                          method="post">
                        @csrf
                        <div class="row">
                            ...
                            <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                <div class="input_bx"><!-- start input box -->
                                    ...
                                    <select tabindex="3" class="inputs_up form-control cstm_clm_srch" name="second_head_code" id="second_head_code">
                            ...
                            <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                <div class="input_bx"><!-- start input box -->
                                    ...
                                    <select tabindex="4" class="inputs_up form-control cstm_clm_srch" name="third_head_code" id="third_head_code">
                            ...
                            <div class="col-lg-12 col-md-3 col-sm-12 col-xs-12 mt-4 form_controls text-right">
                                @include('include.clear_search_button')
                                <!-- Call add button component -->
                                    <x-add-button tabindex="9" href="{{ route('expense_account_registration') }}"/>
                    ...
                    <form name="edit" id="edit" action="{{ route('edit_account') }}" method="post">
                        @csrf
                        <input name="acc_id" id="acc_id" type="hidden">
                    </form>
                ...
                <div class="table-responsive" id="printTable">
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @php
                            ...
                            $countSeg = (!empty($segmentSr)) ? $segmentSr : 0; $bal = 0;
                        @endphp
                        @forelse($datas as $index=> $account)
                            @if($account->account_type==0)
                                <tr data-id="{{$account->account_id}}">
                                    ...
                                    <td>
                                        <a data-id="{{$account->account_uid}}" data-name="{{$account->account_name}}" class="ledger" data-toggle="tooltip" data-placement="top" title=""
                                           data-original-title="Ledger Report of {{$account->account_name}}">
                                    ...
                                    @if($account->account_type==0)
                                        ...
                                        @if(in_array($account->account_uid,explode(',',config('global_variables.default_account_list'))))
                                            <td class="text-center "><a>-</a></td>
                                        @else
                                            <td class="text-center hide_column ">
                                                <a data-id="{{$account->account_uid}}" data-parent="{{$account->account_parent_code}}" class="delete" data-toggle="tooltip" data-placement="left"
                        ...
                        <tfoot>
                        <tr>
                            <th colspan="7" class="text-right border-0"> Per Page Total</th>
                            <td class="text-right border-0"> {{ number_format($bal,2) }} </td>
                        </tr>
                        </tfoot>
                    </table>
                ...
                <span
                    class="hide_column">  {{ $datas->appends(['segmentSr' => $countSeg, 'search'=>$search, 'first_head'=>$search_first_head, 'second_head_code'=>$search_second_head, 'third_head_code'=>$search_third_head, 'group'=>$search_group ])->links() }}</span>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('expense_account_list') }}',
            url;
    ...
    <script>
        jQuery("#cancel").click(function () {
            ...
            $("#second_head_code").select2().val(null).trigger("change");
            $("#second_head_code > option").removeAttr('selected');

            $("#third_head_code").select2().val(null).trigger("change");
            $("#third_head_code > option").removeAttr('selected');
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            jQuery("#third_head_code").select2();
