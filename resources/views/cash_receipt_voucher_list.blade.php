        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header">
                    ...
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Cash Receipt Voucher List</h4>
                        </div>
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('cash_receipt_voucher_list') }}" name="form1"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx">
                                            ...
                                            <select tabindex="4" name="cash_account"
                                                    class="inputs_up form-control" id="cash_account">
                                                <option value="">Select Cash Account</option>
                                                @foreach ($cash_accounts as $cash_account)
                                                    <option value="{{ $cash_account->account_uid }}"
                                                        {{ $cash_account->account_uid == $search_cash_account ? 'selected="selected"' : '' }}>
                                                        {{ $cash_account->account_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12 mt-lg-4 text-right">
                                        ...
                                        <x-add-button tabindex="9" href="{{ route('cash_receipt_voucher') }}"/>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $voucher)
                            @php $per_page_ttl_amnt = +$voucher->cr_total_amount + +$per_page_ttl_amnt; @endphp
                            ...
                            <tr>
                                ...
                                <td class="view" data-id="{{ $voucher->cr_id }}">
                                    CRV-{{ $voucher->cr_id }}
                                </td>
                                <td>
                                    {{ $voucher->cr_remarks }}
                                ...
                                <td>
                                    {{--                                        {!! str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'<br />', $voucher->cr_detail_remarks) !!} --}}
                                    {!! str_replace('&oS;', '<br />', $voucher->cr_detail_remarks) !!}
                                </td>
                                <td>
                                    {{ $voucher->cr_total_amount != 0 ? number_format($voucher->cr_total_amount, 2) : '' }}
                                </td>
                                ...
                                <td class="text-center usr_prfl {{ $voucher->cr_status == 'post' ? : '' }}">

                                    @if ($voucher->cr_status == 'post')
                                        <span class="btn btn-sm btn-success">POSTED<span>
                                                @elseif ($voucher->cr_status == 'deleted')
                                                    <span class="btn btn-sm btn-info">Deleted<span>
                                        @elseif ($voucher->cr_status == 'park')
                                                                <a class="btn btn-sm btn-info"
                                                                   href="{{ route('edit_cash_receipt_voucher', $voucher->cr_id) }}"
                                                                   title="Click To Edit Voucher"><i
                                                                        class="fa fa-edit"></i></a> |
                                                                <a class="btn btn-sm btn-danger"
                                                                   href="{{ route('delete_cash_receipt_voucher', $voucher->cr_id) }}"
                                                                   title="Click To Delete Voucher"
                ...
                <div class="row">
                    ...
                    <div class="col-md-9 text-right">
                        <span
                            class="hide_column">{{ $datas->appends(['segmentSr' => $countSeg, 'search' => $search, 'to' => $search_to, 'from' => $search_from, 'cash_account' => $search_cash_account])->links() }}</span>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('cash_receipt_voucher_list') }}',
            url;
