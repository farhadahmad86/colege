        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Trade Claim Stock Issue to Party List</h4>
                        </div>
                ...
                {{--                {{ ( !empty($search) || !empty($search_to) || !empty($search_from) ) ? '' : 'search_form_hidden' }}--}}
                <div class="search_form m-0 p-0 ">
                    <form class="highlight prnt_lst_frm" action="{{ route('trade_claim_stock_issue_to_party_list') }}" name="form1" id="form1" method="post">
                        @csrf
                        <div class="row">
                                ...
                                <div class="row">
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx"><!-- start input box -->
                                            ...
                                            <input tabindex="2" type="text" name="to" id="to" class="inputs_up form-control datepicker1" autocomplete="off" <?php if(isset($search_to)){?>
                                    ...
                                    <!-- Call add button component -->
                                        <x-add-button tabindex="9" href="{{ route('add_claim_stock_issue_to_party') }}"/>
                    ...
                    <form name="edit" id="edit" action="{{ route('approval_journal_voucher_edit') }}" method="post">
                        @csrf
                        <input name="ajv_id" id="ajv_id" type="hidden">
                    </form>

                    <form name="approval_jv_confirm" id="approval_jv_confirm" action="{{ route('approval_journal_voucher_confirm') }}" method="post">
                        @csrf
                        <input name="ajv_confirm_id" id="ajv_confirm_id" type="hidden">
                    </form>

                    <form name="approval_jv_response" id="approval_jv_response" action="{{ route('approval_journal_voucher_response') }}" method="post">
                        @csrf
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="tbl_amnt_6">Voucher#</th>
                            <th scope="col" class="tbl_txt_20">Remarks</th>
                            <th scope="col" class="tbl_txt_40">Detail Remarks</th>
                        ...
                        @php
                            ...
                            $per_page_ttl_amnt = 0;
                        @endphp
                        @forelse($partyClaims as $partyClaim)
                            ...
                            <tr>
                                ...
                                <td class="usr_prfl" data-usr_prfl="{{ $partyClaim->pc_user_id }}" data-user_info="{!! $ip_browser_info !!}"
                ...
                <div class="row">
                    <div class="col-md-3">
                        <span>Showing {{$partyClaims->firstItem()}} - {{$partyClaims->lastItem()}} of {{$partyClaims->total()}}</span>
                    </div>
                    <div class="col-md-9 text-right">
                        <span class="hide_column">{{ $partyClaims->appends(['segmentSr' => $countSeg, 'search'=>$search ?? '', 'to'=>$search_to ?? '', 'from'=>$search_from ?? '' ])->links() }}</span>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content base_clr">
                <div class="modal-header">
                    <h4 class="modal-title text-black">Trade Claim Stock Issue To Party Detail</h4>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('trade_claim_stock_issue_to_party_list') }}',
            url;
    ...
    <script>
        ...
        jQuery(".view").click(function () {
            ...
            $(".modal-body").load('{{ url('trade_claim_stock_issue_to_party_items_view_details/view/') }}/' + id, function () {
                // jQuery(".pre-loader").fadeToggle("medium");
    ...
    <script>
        jQuery("#cancel").click(function () {
            ...
            $("#cash_account").select2().val(null).trigger("change");
            $("#cash_account > option").removeAttr('selected');
        });
