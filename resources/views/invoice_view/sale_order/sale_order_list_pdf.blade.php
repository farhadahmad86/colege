...
<div id="" class="table-responsive" style="z-index: 9;">
    ...
    <table class="table table-bordered table-sm">
        ...
        <tr class="bg-transparent">
            ...
            <td colspan="{{ ($som->so_order_type === 2) ? '6' : '4' }}" class="p-0 border-0 text-right" >
                <h2 class="invoice_hdng">
                    Sale {{ ($som->so_status !== 'RETURN') ? '' : 'Return' }} order
                </h2>
                ...
                <p class="invoice_para m-0 pt-5">
                    <b> Date: </b>
                    {{date('d-M-y', strtotime(str_replace('/', '-', $som->so_day_end_date)))}}
                </p>
            </td>
        </tr>

        <tr>
            <td colspan="{{ ($som->so_order_type === 2) ? '8' : '6' }}"  class="p-0 border-0">
                <table class="table p-0 m-0 bg-transparent" >
                    <tr class="bg-transparent" >
                        <td colspan="1"  class="p-0 border-0">
                            <h3 class="invoice_sub_hdng mb-0 mt-0">
                                Billed {{ $som->so_status == 'PURCHASE' ? 'To' : 'From' }}
                            </h3>
                            <p class="invoice_para m-0 pt-5">
                                <b> Name: </b>
    ...
    <table class="table table-bordered table-sm">
        ...
        <tr class="headings vi_tbl_hdng">
            ...
            <th scope="col" align="center" class="align_left text-left {{ ($som->so_order_type === 2) ? 'pur_in_wdth_2' : 'pur_in_wdth_3' }}">Product Name</th>
            <th scope="col" align="center" class="pur_in_wdth_4 text-center align_center">QTY</th>
            @if($som->so_order_type === 1)
                <th scope="col" align="center" class="pur_in_wdth_4 text-center align_center">Net Rate</th>
            @endif
            @if($som->so_order_type !== 1)
                <th class="pur_in_wdth_4 align_center text-center"> Rate </th>
            @endif
            <th scope="col" align="center" class="pur_in_wdth_8 text-center align_center">Excluded {{ ($som->so_order_type === 2) ? 'Sale Tax' : 'Discount' }} </th>
            <th scope="col" align="center" class="pur_in_wdth_4 text-center align_center">Dis.</th>
            @if($som->so_order_type === 2)
        ...
        @foreach( $soims as $soim )
            @php
                $dis = $soim->soi_discount;
                $ttlDis = +$dis + +$ttlDis;

                $amnt = $soim->soi_amount;
                $ttlAmnt = +$amnt + +$ttlAmnt;
            @endphp
            <tr class="even pointer">

                <td class="pur_in_wdth_1 align_center text-center"> {{ $i }} </td>
                <td class="align_left text-left {{ ($som->so_order_type === 2) ? 'pur_in_wdth_2' : 'pur_in_wdth_3' }}">
                    {!! str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'<br />', $soim->soi_product_name) !!}
                    @if(!empty($soim->soi_remarks) && isset($soim->soi_remarks))
                        <blockquote>
                            {!! str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'<br />', $soim->soi_remarks) !!}
                        </blockquote>
                    @endif
                ...
                @if($som->so_order_type === 1)
                    @php
                        $percent = (($soim->soi_rate * $soim->soi_saletax) / 100);
                        $net_rate = +$soim->soi_rate + +$percent;
                        $excluSaleTaxRate = $net_rate * $soim->soi_qty;
                ...
                @if($som->so_order_type !== 1)
                    @php
                        $rate = $soim->soi_rate;
                        $excluSaleTaxRate = $rate * $soim->soi_qty;
                        $ttlExcluSaleTaxRate = +$excluSaleTaxRate + +$ttlExcluSaleTaxRate;
                        $ttlRate = +$rate + +$ttlRate;
                    @endphp
                    <td class="pur_in_wdth_4 align_center text-center">
                        {{ number_format($soim->soi_rate,2) }}
                    </td>
                @endif
                ...
                @if($som->so_order_type === 2)
                    @php
                        $ttl_pro = $soim->soi_rate * $soim->soi_qty;
                        $deductDis = +$ttl_pro - +$soim->soi_discount;
                        $ttlDeductDis1 = +$deductDis + +$ttlDeductDis1;
                        $percent = (($deductDis * $soim->soi_saletax) / 100);
                ...
                <td class="pur_in_wdth_5 align_right text-right">
                    {!! number_format($soim->soi_amount,2) !!}
                </td>
        ...
        <tfoot>
        <tr class="border-0">
            <td colspan="{{ ($som->so_order_type === 2) ? '10' : '7' }}" valign="top" class="p-0 border-0" >
                    ...
                    <tr>
                            ...
                            <table  class="m-0 p-0 table">
                                <tr>
                                    ...
                                    <td class="text-right fontS-12">
                                        {{ number_format($som->so_expense,2) }}
                                    </td>
                                </tr>
                                {{--                                    @if($som->so_invoice_type === 2)--}}
                                {{--                                        <tr>--}}
                                {{--                                            <td class="vi_tbl_hdng">--}}
                                {{--                                                <label class="total-items-label text-right">Sale Tax:-</label>--}}
                                {{--                                            </td>--}}
                                {{--                                            <td class="text-right">--}}
                                {{--                                                {{ number_format($som->so_sales_tax,2) }}--}}
                                {{--                                            </td>--}}
                                {{--                                        </tr>--}}
                                {{--                                    @endif--}}
                            </table>
                        ...
                        <td class="pl-0 pb-0 pr-0 border-0 pts-10">
                            <table  class="m-0 p-0 table">
                                <tr>
                                    ...
                                    <td class="text-right fontS-12">
                                        {{ number_format($som->so_trade_disc,2) }}
                                    </td>
                                </tr>
                            </table>
                        </td>
                        ...
                        <td class="pl-0 pb-0 pr-0 border-0 pts-10">
                            <table  class="m-0 p-0 table">
                                ...
                                {{--                                        </th>--}}
                                {{--                                        <td class="text-right">--}}
                                {{--                                            {{$som->so_total_price !=0 ? number_format($som->so_total_price,2):'0'}}--}}
                                {{--                                        </td>--}}
                                {{--                                    </tr>--}}
                                <tr>
                                    <th class="vi_tbl_hdng fontS-12 text-right">
                                    ...
                                    <td class="text-right fontS-12">
                                        {{ number_format($som->so_grand_total,2) }}
                                    </td>
                                </tr>
                            </table>
                        </td>
        ...
        <tr>
            <td colspan="{{ ($som->so_order_type === 2) ? '10' : '7' }}" class="border-0 p-0">
                <div class="wrds_con">
                    <div class="wrds_bx">
