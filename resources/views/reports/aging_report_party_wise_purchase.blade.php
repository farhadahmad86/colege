        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text">Party Wise Purchase Aging Report</h4>
                        </div>
                ...
                <div class="search_form m-0 p-0 {{ ( !empty($search) || !empty($search_to) || !empty($search_from) ) ? '' : '' }}">
                    ...
                    <form class="highlight prnt_lst_frm" action="{{ route('aging_report_party_wise_purchase') }}" name="form1" id="form1" method="post">
                        @csrf
                        <div class="row">
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $invoice)
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="view"
                                       data-transcation_id="{{ ($invoice->type === "pur_in") ? config('global_variables.PURCHASE_VOUCHER_CODE').$invoice->pi_id : config('global_variables.PURCHASE_SALE_TAX_VOUCHER_CODE').$invoice->pi_id}}"
                                       data-toggle="modal" data-target="#myModal"
                                       style="cursor:pointer; color: #0099ff;">
                                        @if($invoice->type === "pur_in")
                                            {{config('global_variables.PURCHASE_VOUCHER_CODE').$invoice->pi_id }}
                                        @elseif($invoice->type === "pur_sltx_in")
                                            {{config('global_variables.PURCHASE_SALE_TAX_VOUCHER_CODE').$invoice->pi_id }}
                                        @endif
                                    </a>
                                </td>
                                <td>
                                    {{(strtotime($current_date) - strtotime($invoice->pi_day_end_date)) / 86400}}
                                </td>
                                <td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('aging_report_party_wise_purchase') }}',
            url;
