        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Bank Account List</h4>
                            {{--{{ $accounts ->links() }}--}}
                        </div>
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('bank_account_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1"
                          method="post">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-lg-12 col-md-3 col-sm-12 col-xs-12 mt-4 form_controls">
                                @include('include.clear_search_button')
                                <!-- Call add button component -->
                                    <x-add-button tabindex="9" href="{{ route('bank_account_registration') }}"/>
                    ...
                    <form name="delete" id="delete" action="{{ route('delete_account') }}" method="post">
                        @csrf
                        <input name="account_id" id="del_account_id" type="hidden">
                        <input name="parent_account_id" id="del_parent" type="hidden">
                    </form>
                </div>
                        ...
                        @forelse($datas as $index=> $account)
                            ...
                            @if($account->account_type==0)
                                <tr data-id="{{$account->account_id}}">
                                    ...
                                    <td class="edit">
                                        @php if(substr($account->account_uid,0,1)==1){ echo 'Asset'; }
                                            elseif (substr($account->account_uid,0,1)==2){ echo 'Liability'; }
                                            elseif (substr($account->account_uid,0,1)==3){ echo 'Revenue'; }
                                            elseif (substr($account->account_uid,0,1)==4){ echo 'Expense'; }
                                            else{ echo 'Equity'; }
                                        @endphp
                                    </td>
                                    <td class="edit">
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('bank_account_list') }}',
            url;
