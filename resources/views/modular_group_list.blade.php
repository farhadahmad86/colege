        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Modular Group List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ ( !empty($search) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('modular_group_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1"
                          method="post">
                        @csrf
                        <div class="row">
                            ...
                            <!-- Call add button component -->
                                <x-add-button tabindex="9" href="{{ route('add_modular_group') }}"/>
                    ...
                    <form name="edit" id="edit" action="{{ route('edit_modular_group') }}" method="post">
                        @csrf
                        <input tabindex="-1" name="group_id" id="group_id" type="hidden">
                    </form>

                    <form name="delete" id="delete" action="{{ route('delete_modular_group') }}" method="post">
                        @csrf
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $group)
                            ...
                            <tr data-group_id="{{$group->mg_id}}">
                                ...
                                @php
                                    $ip_browser_info= ''.$group->mg_ip_adrs.','.str_replace(' ','-',$group->mg_brwsr_info).'';
                                @endphp
                                ...
                                <td class="text-center hide_column ">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($group->mg_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $group->mg_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$group->mg_id}}"
                                            {{ $group->mg_disabled == 0 ? 'checked' : '' }}>
                                ...
                                <td class="text-center hide_column">
                                    <a data-group_id="{{$group->mg_id}}" class="delete" style="cursor:pointer;" data-toggle="tooltip" data-placement="left" title=""
                                       data-original-title="Are you sure?">
                                        <i class="fa fa-{{$group->mg_delete_status
 == 1 ? 'undo':'trash'}}"></i>
                                    </a>
                                </td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('modular_group_list') }}',
            url;
        ...
        $(document).ready(function () {
            $('.enable_disable').change(function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('enable_disable_modular_group') }}',
                    data: {'status': status, 'mg_id': mgId},
