            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-9 col-xl-9">
                        ...
                        <div class="col-md-6 col-sm-12">
                            <nav aria-label="breadcrumb" role="navigation">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">PSO Credit Card Sale Invoice</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                    ...
                    <div class="form_header"><!-- form header start -->
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 class="text-white">PSO Credit Card Sale Invoice</h4>
                            </div>
                            <div class="list_btn">
                                <a class="btn list_link add_more_button" href="{{ route('pso_sale_invoice_list') }}" role="button">
                    ...
                    <form name="f1" class="f1" id="f1" action="{{ route('submit_pso_sale_invoice_credit') }}" onsubmit="return popvalidation()" method="post">
                    @csrf
                    <!-- main row ends here --> <!-- first row ends here -->
                                    ...
                                    <div class="row">  <!--  new row starts here -->
                                        ...
                                        <div class="form-group col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                            <div class="input_bx"><!-- start input box -->
                                                ...
                                                <select name="machine" class="inputs_up form-control" id="machine">
                                                    <option value="0">Select Machine</option>
                                                    @foreach($machines as $machine)
                                                        <option value="{{$machine->ccm_id}}"
                                                                data-bank="{{$machine->ccm_bank_code}}" data-bank_name="{{$machine->account_name}}"
                                                                data-percentage="{{$machine->ccm_percentage}}">{{$machine->ccm_title}}</option>
                                                    @endforeach
                                                </select>
                                            </div><!-- end input box -->
                                        </div>
                                            ...
                                            <div class="input_bx"><!-- start input box -->
                                                ...
                                                <select name="nozzle" class="inputs_up form-control" id="nozzle">
                                                    ...
                                                    @foreach($nozzles as $nozzle)
                                                        <option value="{{$nozzle->noz_id}}" data-tank="{{$nozzle->noz_tank_id}}" data-pro_code="{{$nozzle->t_product_code}}">
                                                            {{$nozzle->noz_name}}</option>
                                                    @endforeach
                                                </select>
                                ...
                                <div class="row m-0" style="">
                                    ...
                                    <div class="table_div table-responsive table-max-height-500px">
                                        <table class="table table-striped table-bordered m-0" id="category_dynamic_table">
                                            ...
                                            <tr>
                                                <th scope="col" class=" wdth_5">Machine</th>
                                                ...
                                                <th scope="col" class="text-right wdth_5">Sale Person</th>
                        ...
                        <input type="hidden" name="credit_card" id="credit_card" value="1">
                    </form>
...
<script>
    jQuery("#nozzle").change(function () {
        ...
        jQuery("#product_name > option").each(function () {
            jQuery('#product_name option[value="' + pro_code + '"]').prop('selected', true);
        });
...
<script>
    ...
    //    var product_name = document.getElementById("product_name").value;
        ...
        if (flag_submit1) {
            ...
            jQuery("#machine").select2("destroy");
            jQuery("#product_name").select2("destroy");
            jQuery("#nozzle").select2("destroy");
            jQuery("#sale_person").select2("destroy");

            var checkedValue = [];

            var product_name = jQuery("#product_name option:selected").text();
            var selected_code_value = jQuery("#product_name option:selected").val();
            var qty = jQuery("#quantity").val();
            ...
            var sale_person_name = jQuery("#sale_person option:selected").text();

            var machine_name = jQuery("#machine option:selected").text();
            var account = jQuery('option:selected', '#machine').attr('data-bank');
            var account_name = jQuery('option:selected', '#machine').attr('data-bank_name');
            var percentage = jQuery('option:selected', '#machine').attr('data-percentage');
            ...
            sales[counter] = [selected_code_value, product_name, selected_remarks, qty, selected_rate, selected_amount, nozzle , tank , account, account_name, sale_person, machine , machine_name,
                percentage];

            numberofsales = Object.keys(sales).length;

            jQuery("#table_body").prepend('<tr id=' + counter + '><td >' + machine_name + '</td><td > <div class="max_txt">' + product_name + '</div></td><td>' + selected_remarks + '</td><td' +
                ' >' + qty + '</td><td >' + selected_rate + '</td><td>' + selected_amount + '</td><td class="text-right">' + nozzle_name + '</td><td class="text-right">' + sale_person_name + '</td><td align="right"><a class="edit_link ' +
            ...
            jQuery('#sale_person option[value="' + 0 + '"]').prop('selected', true);
    ...
    function delete_sale(current_item) {
        ...
        // jQuery("#account option[value=" + temp_sales[8] + "]").attr("disabled", false);
    ...
    function edit_sale(current_item) {
        ...
        jQuery("#machine").val(temp_sales[11]);
        ...
        jQuery("#sale_person").val(temp_sales[10]);
