        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text">Parent Account List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ ( !empty($search) || !empty($search_first_level_account) ) ? '' : 'search_form_hidden' }}"> -->


                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('second_level_chart_of_account_list') .((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}"
                        ...
                        <div class="row">
                            ...
                            <!-- Call add button component -->
                                <x-add-button tabindex="9" href="{{ route('add_second_level_chart_of_account') }}"/>
                    ...
                    <form name="edit" id="edit" action="{{ route('edit_second_level_chart_of_account') }}" method="post">
                        @csrf
                        <input tabindex="-1" name="account_id" id="account_id" type="hidden">
                    </form>

                    <form name="delete" id="delete" action="{{ route('delete_second_level_chart_of_account') }}" method="post">
                        @csrf
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $account)
                            ...
                            <tr data-account_id="{{$account->coa_id}}">
                                ...
                                <td class="text-center hide_column ">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($account->coa_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $account->coa_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$account->coa_id}}"
                                            {{ $account->coa_disabled == 0 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                {{--    add code by mustafa end --}}
                                <td class="text-center hide_column">
                                    <a data-account_code="{{$account->coa_code}}" data-account_id="{{$account->coa_id}}" class="delete" data-toggle="tooltip" data-placement="left" title=""
                                       data-original-title="Are you sure?">
                                        <i class="fa fa-{{$account->coa_delete_status == 1 ? 'undo':'trash'}}"></i>
                ...
                <div class="row">
                    ...
                    <div class="col-md-9 text-right">
                        <span class="hide_column"> {{ $datas->appends(['segmentSr' => $countSeg, 'search'=>$search, 'first_level'=>$search_first_level_account ])->links() }}</span>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('second_level_chart_of_account_list') }}',
            url;
