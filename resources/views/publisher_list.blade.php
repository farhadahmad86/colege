        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            ...
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                ...
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Publisher List</h4>
                        </div>
                ...
                <!-- <div class="search_form {{ ( !empty($search) || !empty($search_publisher) ) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form">
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            ...
                            <form class="prnt_lst_frm" action="{{ route('publisher_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1" method="post">
                                <div class="row">
                                    @csrf
                                    ...
                                    <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                                        <div class="form_controls text-left">
                                            ...
                                            <a tabindex="4" class="save_button form-control" href="{{ route('add_publisher') }}" role="button">
                            ...
                            <form name="edit" id="edit" action="{{ route('edit_publisher') }}" method="post">
                                @csrf
                                <input tabindex="5" name="name" id="name" type="hidden">
                                <input tabindex="6" name="remarks" id="remarks" type="hidden">
                                <input tabindex="7" name="publisher_id" id="publisher_id" type="hidden">

                            </form>

                            <form name="delete" id="delete" action="{{ route('delete_publisher') }}" method="post">
                                @csrf
                                <input tabindex="8" name="pub_id" id="pub_id" type="hidden">
                            </form>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-sm" id="fixTable">
                        ...
                        @forelse($datas as $data)
                            ...
                            <tr data-title="{{$data->pub_title}}" data-remarks="{{$data->pub_remarks}}" data-publisher_id="{{$data->pub_id}}">
                                ...
                                @php
                                    $ip_browser_info= ''.$data->pub_ip_adrs.','.str_replace(' ','-',$data->pub_brwsr_info).'';
                                @endphp
                                ...
                                <td class="align_right text-right hide_column tbl_amnt_6">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($data->pub_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $data->pub_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$data->pub_id}}"
                                            {{ $data->pub_disabled == 0 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>

                                <td class="align_right text-right hide_column tbl_srl_6">
                                    <a data-publisher_id="{{$data->pub_id}}" class="delete"  data-toggle="tooltip" data-placement="left"  title="" data-original-title="Are you sure?">
                                        <i class="fa fa-{{$data->pub_delete_status == 1 ? 'undo':'trash'}}"></i>
                                    </a>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="11">
                                    <center><h3 style="color:#554F4F">No Publisher</h3></center>
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
...
@section('scripts')
        ...
        $(document).ready(function () {
            $('.enable_disable').change(function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('enable_disable_publisher') }}',
                    data: {'status': status, 'pub_id': pubId},
    ...
    <script>
        ...
        $('.delete').on('click', function (event) {
            ...
            var publisher_id = jQuery(this).attr("data-publisher_id");
            ...
            Swal.fire({
                ...
                if (result.value) {
                    jQuery("#pub_id").val(publisher_id);
