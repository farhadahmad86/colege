            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                ...
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                    ...
                    <div class="form_header"><!-- form header start -->
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 class="text-white get-heading-text">Edit Sector</h4>
                            </div>
                        </div>
                    </div><!-- form header close -->

                    <form name="f1" class="f1" id="f1" action="{{ route('update_sector') }}" onsubmit="return checkForm()" method="post">
                        @csrf
                        <div class="row">
...
config('fields_info.about_form_fields.party_registration.region.name.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.region.name.validations') }}</p>">
                                                    ...
                                                    <select name="region_name" class="inputs_up form-control" id="region_name" autofocus data-rule-required="true" data-msg-required="Please Enter Region Title">
...
config('fields_info.about_form_fields.party_registration.area.area_title.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.area.area_title.validations') }}</p>">
                                                    ...
                                                    <select name="area_name" class="inputs_up form-control" id="area_name" autofocus data-rule-required="true" data-msg-required="Please Enter Area Title">
                                                        <option value="">Select Area</option>
                                                    {{--@foreach($areas as $area)--}}
                                                        {{--<option value="{{$area->area_id}}" {{ $area->area_id == $request->area_id? 'selected="selected"' : '' }}>{{$area->area_title}}</option>--}}
                                                    {{--@endforeach--}}
                                                    </select>
...
config('fields_info.about_form_fields.party_registration.sector.sector_title.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.sector.sector_title.validations') }}</p>">
                                                    ...
                                                    <input type="text" name="sector_name" id="sector_name" class="inputs_up form-control"
                                                           data-rule-required="true" data-msg-required="Please Enter Sector"
                                                           placeholder="Sector Title" value="{{$request->sector_title}}" autocomplete="off">
                                                    <input value="{{$request->sector_id}}" type="hidden" name="sector_id">
...
config('fields_info.about_form_fields.party_registration.general_fields.remarks.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.general_fields.remarks.validations') }}</P>">
                                                        ...
                                                        Remarks</label>
                                                    <textarea name="remarks" id="remarks" class="inputs_up inputs_up remarks form-control">{{$request->remarks}}</textarea>
