        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                <div class="form_header"><!-- form header start -->
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">Currency List</h4>
                        </div>
...
{{--                <div class="search_form {{ ( !empty($search) || !empty($search_currency) ) ? '' : 'search_form_hidden' }}">--}}
                ...
                <div class="search_form">
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            ...
                            <form class="prnt_lst_frm" action="{{ route('currency_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1" method="post">
                                <div class="row">
                                    @csrf
                                    ...
                                    <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                                        <div class="form_controls text-left">
                                            ...
                                            <a tabindex="4" class="save_button form-control" href="{{ route('add_currency') }}" role="button">
                            ...
                            <form name="edit" id="edit" action="{{ route('edit_currency') }}" method="post">
                                @csrf
                                <input tabindex="4" name="name" id="name" type="hidden">
                                <input tabindex="5" name="remarks" id="remarks" type="hidden">
                                <input tabindex="6" name="currency_id" id="currency_id" type="hidden">

                            </form>

                            <form name="delete" id="delete" action="{{ route('delete_currency') }}" method="post">
                                @csrf
                                <input tabindex="7" name="cur_id" id="cur_id" type="hidden">
                            </form>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-sm" id="fixTable">
                        ...
                        @forelse($datas as $data)
                            ...
                            <tr data-title="{{$data->cur_title}}" data-remarks="{{$data->cur_remarks}}" data-currency_id="{{$data->cur_id}}">
                                ...
                                @php
                                    $ip_browser_info= ''.$data->cur_ip_adrs.','.str_replace(' ','-',$data->cur_brwsr_info).'';
                                @endphp
                                ...
                                <td class="align_right text-right hide_column tbl_amnt_6">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($data->cur_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $data->cur_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$data->cur_id}}"
                                            {{ $data->cur_disabled == 0 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>

                                <td class="align_right text-right hide_column tbl_srl_6">
                                    <a data-currency_id="{{$data->cur_id}}" class="delete"  data-toggle="tooltip" data-placement="left"  title="" data-original-title="Are you sure?">
                                        <i class="fa fa-{{$data->cur_delete_status == 1 ? 'undo':'trash'}}"></i>
                                    </a>
                                </td>
                            </tr>
                            @php
                        ...
                        @empty
                            <tr>
                                <td colspan="11">
                                    <center><h3 style="color:#554F4F">No Currency</h3></center>
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
...
@section('scripts')
        ...
        $(document).ready(function () {
            $('.enable_disable').change(function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('enable_disable_currency') }}',
                    data: {'status': status, 'cur_id': curId},
    ...
    <script>
        ...
        $('.delete').on('click', function (event) {
            ...
            var currency_id = jQuery(this).attr("data-currency_id");
