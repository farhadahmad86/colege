            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                ...
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage">
                    ...
                    <div class="form_header"><!-- form header start -->
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 tabindex="-1" class="text-white get-heading-text">Update Surveyor</h4>
                            </div>
                        </div>
                    </div><!-- form header close -->

                    <form name="f1" class="f1" id="f1" action="{{route('surveyor.update')}}" onsubmit="return checkForm()" method="post">
                        @csrf
                        <div class="row">
...
config('fields_info.about_form_fields.party_registration.region.name.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.region.name.validations') }}</p>">
                                                ...
                                                Name</label>
                                            <input tabindex="1" autofocus type="text" name="surveyor_name" id="surveyor_name" class="inputs_up form-control"
                                                   data-rule-required="true" data-msg-required="Please Enter Name"
                                                   placeholder="Region Title" autofocus value="{{$request->srv_name}}" autocomplete="off"/>
...
config('fields_info.about_form_fields.party_registration.general_fields.remarks.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.general_fields.remarks.validations') }}</P>">
                                                ...
                                                Password</label>
                                            <input tabindex="2" type="text" name="surveyor_password" id="surveyor_password" class="inputs_up form-control"
                                                   data-rule-required="true" data-msg-required="Please Enter Name"
                                                   placeholder="Surveyor Password" autofocus value="" autocomplete="off"/>
...
config('fields_info.about_form_fields.party_registration.general_fields.remarks.example')}}</p><h6>Validation</h6><p>{{config('fields_info.about_form_fields.party_registration.general_fields.remarks.validations') }}</P>">
                                    ...
                                    <input tabindex="4" value="{{$request->srv_id}}" type="hidden" name="surveyor_id">

                                </div>

                                <div class="form-group row">
                                    <div class="col-lg-12 col-md-12 col-sm-12 form_controls">
                                        <button tabindex="5" type="button" name="cancel" id="cancel" class="cancel_button form-control">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        {
            var surveyor_name  = document.getElementById("surveyor_name").value;
            var surveyor_password  = document.getElementById("surveyor_password").value;

            var flag_submit = true;
            var focus_once = 0;

            if(surveyor_name.trim() == "")
            {
                document.getElementById("demo1").innerHTML = "Required";
                if (focus_once == 0) { jQuery("#surveyor_name").focus(); focus_once = 1;}
                flag_submit = false;
            }else{
                document.getElementById("demo1").innerHTML = "";
            }

            // if(surveyor_password.trim() == "")
            // {
            //     document.getElementById("demo2").innerHTML = "Required";
            //     if (focus_once == 0) { jQuery("#surveyor_password").focus(); focus_once = 1;}
            //     flag_submit = false;
            // }else{
