                ...
                <div class="search_form">
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            ...
                            <form class="prnt_lst_frm" action="{{ route('board_type_list') . ((isset($restore_list) && $restore_list == 1) ? '?restore_list=1' : '') }}" name="form1" id="form1" method="post">
                                <div class="row">
                                    @csrf
...
{{--                                            <a tabindex="4" class="save_button form-control" href="{{ route('add_board_type') }}" role="button">--}}
...
{{--                                            </a>--}}
                            ...
                            <form name="edit" id="edit" action="{{ route('edit_board_type') }}" method="post">
                                @csrf
                            ...
                            <form name="delete" id="delete" action="{{ route('delete_board_type') }}" method="post">
                                @csrf
                                <input name="bt_id" id="bt_id" type="hidden">
                            </form>
...
{{--                            </th>--}}
                        ...
                        @forelse($datas as $data)
                            ...
                            <tr data-title="{{$data->bt_title}}" data-remarks="{{$data->bt_remarks}}" data-board_type_id="{{$data->bt_id}}">
                                ...
                                @php
                                    $ip_browser_info= ''.$data->bt_ip_adrs.','.str_replace(' ','-',$data->bt_brwsr_info).'';
                                @endphp
                                ...
                                <td class="align_right text-right hide_column tbl_amnt_6">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($data->bt_disabled == 0) {
                                            echo 'checked="true"' . ' ' . 'value=' . $data->bt_disabled;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?>  class="enable_disable" data-id="{{$data->bt_id}}"
                                            {{ $data->bt_disabled == 0 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>

{{--                                <td class="align_right text-right hide_column tbl_srl_6">--}}
{{--                                    <a data-board_type_id="{{$data->bt_id}}" class="delete"  data-toggle="tooltip" data-placement="left"  title="" data-original-title="Are you sure?">--}}
{{--                                        <i class="fa fa-{{$data->bt_delete_status == 1 ? 'undo':'trash'}}"></i>--}}
{{--                                    </a>--}}
{{--                                </td>--}}



                                {{--                                        <td><input type="checkbox" data-id="{{ $data->bt_id }}" name="status" class="js-switch" {{ $data->reg_disabled == 0 ? 'checked' : '' }}></td>--}}
                            </tr>
                            @php
                        ...
                        @empty
                            <tr>
                                <td colspan="11">
                                    <center><h3 style="color:#554F4F">No Board Type</h3></center>
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('board_type_list') }}',
            url;
        ...
        $(document).ready(function () {
            $('.enable_disable').change(function () {
                ...
                $.ajax({
                    ...
                    data: {'status': status, 'bt_id': regId},
    ...
    <script>
        ...
        $('.delete').on('click', function (event) {
            ...
            var board_type_id = jQuery(this).attr("data-board_type_id");
            ...
            Swal.fire({
                ...
                if (result.value) {
                    jQuery("#bt_id").val(board_type_id);
