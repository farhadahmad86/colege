                    ...
                    <div class="search_form {{ ( !empty($search) || !empty($search_account) || !empty($search_product) || !empty($search_to) || !empty($search_from) ) ? '' : 'search_form_hidden' }}">
                        <div class="row">
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                ...
                                <form class="prnt_lst_frm" action="{{ route('teller/sale_tax_sale_invoice_list') }}" name="form1" id="form1" method="post">
                                    @csrf
                                    <div class="row">
                        ...
                        <table class="table table-sm" id="fixTable">
                            ...
                            @forelse($datas as $invoice)
                                ...
                                <tr>
                                    ...
                                    <td class="align_center text-center view tbl_amnt_6" data-id="{{$invoice->ssi_id}}">
                                        STSI-{{$invoice->ssi_id}}
                                    </td>
                                    {{--                                    <td class="align_left" style="white-space:pre-wrap;">{{$invoice->si_detail_remarks}}</td>--}}
                                    {{--<td class="align_left">{{$invoice->si_party_code}}</td>--}}
                                    ...
                                    @php
                                        $ttlPrc = +($invoice->ssi_total_price) + +$ttlPrc;
                                    @endphp
                                    <td class="align_right text-right tbl_amnt_10">
                                        {{$invoice->ssi_total_price !=0 ? number_format($invoice->ssi_total_price,2):''}}
                                    </td>
                                    {{--                                    <td class="align_left">{{$invoice->si_expense !=0 ? number_format($invoice->si_expense,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_trade_disc_percentage !=0 ? number_format($invoice->si_trade_disc_percentage,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_trade_disc !=0 ? number_format($invoice->si_trade_disc,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_value_of_supply !=0 ? number_format($invoice->si_value_of_supply,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_sales_tax !=0 ? number_format($invoice->si_sales_tax,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_special_disc_percentage !=0 ? number_format($invoice->si_special_disc_percentage,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_special_disc !=0 ? number_format($invoice->si_special_disc,2):''}}</td>--}}
                                    {{--                                    <td class="align_left">{{$invoice->si_grand_total !=0 ? number_format($invoice->si_grand_total,2):''}}</td>--}}
                                    @php
                                        $cashPaid = +($invoice->ssi_cash_received) + +$cashPaid;
                                    @endphp
                                    <td class="align_right text-right tbl_amnt_10">
                                        {{$invoice->ssi_cash_paid !=0 ? number_format($invoice->ssi_cash_received,2):''}}
                                    </td>
...
<script type="text/javascript">
    var base = '{{ route('teller/sale_tax_sale_invoice_list') }}',
        url;
...
<script>
    ...
    jQuery(".view").click(function () {
        ...
        $(".modal-body").load('{{url("teller/sale_tax_sale_invoice_list/view/")}}' + '/' + id, function () {
