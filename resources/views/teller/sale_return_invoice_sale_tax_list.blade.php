            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                ...
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                    ...
                    <div class="form_header"><!-- form header start -->
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 class="text-white get-heading-text file_name">Sale Return Sale Tax Invoice List</h4>
                            </div>
                    ...
                    <div class="m-0 p-0 search_form {{ ( !empty($search) || !empty($search_account) || !empty($search_product) || !empty($search_to) || !empty($search_from) ) ? '' : 'search_form_hidden' }}">
                                <form class="highlight prnt_lst_frm" action="{{ route('teller/sale_tax_sale_return_invoice_list') }}" name="form1" id="form1" method="post">
                                    @csrf
                                    <div class="row">
...
{{--                                                            @endforeach--}}
                                                ...
                                                <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12 mt-lg-2">
                                                    <div class="form_controls text-center text-lg-left">
                                                        ...
                                                        <a class="save_button form-control" href="{{ route('teller/sale_return_invoice') }}" role="button">
                            ...
                            @forelse($datas as $invoice)
                                ...
                                <th scope="row" class="edit tbl_srl_4">
                                    ...
                                    <td nowrap class="tbl_amnt_9">
                                        {{date('d-M-y', strtotime(str_replace('/', '-', $invoice->srsi_day_end_date)))}}
                                    </td>
                                    <td class="align_center view text-center tbl_amnt_6" data-id="{{$invoice->srsi_id}}">
                                        STSRI-{{$invoice->srsi_id}}
                                    </td>
                                    {{--                                    <td class="align_center" style="white-space:pre-wrap;">{{$invoice->si_detail_remarks}}</td>--}}
                                    <td class="align_left text-left tbl_txt_21">
                                        {{$invoice->srsi_party_name}}
                                    </td>
                                    <td class="align_left text-left tbl_txt_34">
                                        {!! str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'<br />', $invoice->srsi_detail_remarks) !!}
                                    </td>
                                    @php
                                        $ttlPrc = +($invoice->srsi_total_price) + +$ttlPrc;
                                    @endphp
                                    <td class="align_right text-right tbl_amnt_10">
                                        {{$invoice->srsi_total_price !=0 ? number_format($invoice->srsi_total_price,2):''}}
                                    </td>
                                    ...
                                    @php
                                        $cashPaid = +($invoice->srsi_cash_paid) + +$cashPaid;
                                    @endphp
                                    <td class="align_right text-right tbl_amnt_10">
                                        {{$invoice->srsi_cash_paid !=0 ? number_format($invoice->srsi_cash_paid,2):''}}
                                    </td>

                                    @php
                                        $ip_browser_info= ''.$invoice->srsi_ip_adrs.','.str_replace(' ','-',$invoice->srsi_brwsr_info).'';
                                    @endphp
                                    ...
                                    {{--                                    <td class="align_right">{{$invoice->si_reference_id}}</td>--}}
    ...
    <div class="modal-dialog modal-lg mdl_wdth">
        <div class="modal-content base_clr">
            <div class="modal-header">
                <h4 class="modal-title text-black">Sale Return Invoice Detail</h4>
...
<script type="text/javascript">
    var base = '{{ route('teller/sale_return_invoice_list') }}',
        url;
...
<script>
    ...
    jQuery(".view").click(function () {
        ...
        $('.modal-body').load('{{ url("teller/sale_tax_sale_return_invoice_list/view/") }}' + '/' + id, function () {
