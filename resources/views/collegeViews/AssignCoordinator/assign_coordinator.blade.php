    ...
    <div class="row">
        <div class="container-fluid search-filter form-group form_manage">
            <div class="form_header">
                ...
                <div class="clearfix">
                    <div class="pull-left">
                        <h4 tabindex="-1" class="text-white get-heading-text">Assign Section To Coordinator</h4>
                    </div>
                    <div class="list_btn">
            ...
            <form name="f1" class="f1" id="f1" action="{{ route('store_assign_coordinator') }}" method="post"
                            ...
                            <select tabindex="1" autofocus name="coordinator_id" class="form-control required"
                                ...
                                @foreach ($allusers as $user)
                                    <option value="{{ $user->user_id }}"
                                        {{ $user->user_id == old('user') ? 'selected' : '' }}>
                                        {{ $user->user_name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select tabindex=42 autofocus name="class_id" class="form-control" data-rule-required="true"
                                ...
                                @foreach ($allclasses as $allclass)
                                    <option value="{{ $allclass->class_id }}"
                                        {{ $allclass->class_id == old('class_id') ? 'selected' : '' }}>
                                        {{ $allclass->class_name }}
                                    </option>
                                @endforeach
                            </select>
