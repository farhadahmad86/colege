    ...
    <div class="row">
        <div class="container-fluid search-filter form-group form_manage">
            <div class="form_header">
                ...
                <div class="clearfix">
                    <div class="pull-left">
                        <h4 tabindex="-1" class="text-white get-heading-text">Edit Upload Lecture</h4>
                    </div>
                    <div class="list_btn">
            ...
            <form name="f1" class="f1" id="f1" action="{{ route('update_upload_lecture') }}" method="post"
                            ...
                            <select tabindex=2 autofocus name="class" class="form-control" data-rule-required="true"
                                ...
                                @foreach ($classes as $class)
                                    <option value="{{ $class->class_id }}"
                                        {{ $class->class_id == $request->class_id ? 'selected' : '' }}>
                            ...
                            <select tabindex=2 autofocus name="group" class="form-control" data-rule-required="true"
                                ...
                                @foreach ($groups as $group)
                                    <option value="{{ $group->ng_id }}"
                                        {{ $group->ng_id == $request->group_id ? 'selected' : '' }}>
                                        {{ $group->ng_name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select tabindex=3 autofocus name="subject" class="form-control" data-rule-required="true"
                                ...
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->subject_id }}"
                                        {{ $subject->subject_id == $request->subject_id ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-lg-3 col-md-3 col-sm-12">
                        <div class="input_bx">
                            ...
                            <input tabindex="1" type="text" name="title" id="title" class="inputs_up form-control" placeholder="Title" autofocus data-rule-required="true"
                                   data-msg-required="Please Enter Title" autocomplete="off" value="{{ $request->title }}"/>
                        </div>
                    </div>
                    <div class="form-group col-lg-3 col-md-3 col-sm-12">
                        <div class="input_bx">
                            ...
                            <input tabindex="1" type="text" name="link" id="link" class="inputs_up form-control" placeholder="Link" autofocus data-rule-required="true"
                                   data-msg-required="Please Enter Link" autocomplete="off" value="{{ $request->link }}"/>
                        </div>
                    </div>
                </div>

                <input value="{{ $request->lec_id }}" type="hidden" name="lec_id">
