                    ...
                    <div class="gnrl-mrgn-pdng gnrl-blk invoice_bx_scrl"><!-- invoice scroll box start -->
                        <div class="gnrl-mrgn-pdng gnrl-blk invoice_cntnt"><!-- invoice content start -->
                            <form id="f1" action="{{ route('submit_generate_salary_slip_voucher') }}" method="post" onsubmit="return checkForm()">
                                ...
                                <div class="invoice_row"><!-- invoice row start -->
                                    ...
                                    <div class="invoice_col form-group col-lg-3 col-md-3 col-sm-12"><!-- invoice column start -->
                                        <div class="invoice_col_bx"><!-- invoice column box start -->
                                            ...
                                            <div class="invoice_col_input"><!-- invoice column input start -->
                                                <h4 id="total_salary_amount_display" style="color: red; text-align: center">0</h4>
                                                <input tabindex="-1" type="hidden" name="total_salary_amount" class="inputs_up text-right form-control" id="total_salary_amount"
                                                />
                                            </div><!-- invoice column input end -->
                                        </div><!-- invoice column box end -->
                                    </div><!-- invoice column end -->
                                ...
                                <input tabindex="-1" type="hidden" name="accountsval" id="jsonOutput" data-rule-required="true"
                                       data-msg-required="Please Json">
                                <div class="invoice_row"><!-- invoice row start -->
...
{{--                                                    <th class="tbl_srl_8"> Department</th>--}}
                                                    ...
                                                    <th tabindex="-1" class="tbl_txt_6"> Over Times Charges</th>
    ...
    <div class="modal fade" id="myModalAd" role="dialog">
        <div class="modal-dialog modal-lg">
            <div class="modal-content base_clr">
                <div class="modal-header">
                    <h4 class="modal-title text-black">Allowance & Deduction Detail</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">

                    <div id="table_body_ad">
...
@section('scripts')
    ...
    <script type="text/javascript">
        function checkForm() {
            ...
            $('input[name="employee_id[]"]').each(function (pro_index) {
                ...
                var month_days = $('#month_days' + employee_id).val();
                var attend_days = $('#attend_days' + employee_id).val();
                var salary_period = $('#salary_period' + employee_id).val();
                var basic_salary = $('#basic_salary' + employee_id).val();
                var gross_salary = $('#gross_salary' + employee_id).val();
                var allowances = $('#allowances' + employee_id).val();
                var deductions = $('#deductions' + employee_id).val();
                var over_time_days = $('#over_time_days' + employee_id).val();
                var over_time_amount = $('#over_time_amount' + employee_id).val();
                var net_salary = $('#net_salary' + employee_id).val();
                var loan_acount = $('#loan_acount' + employee_id).val();
                var total_loan_amount = $('#total_loan_amount' + employee_id).val();
                var loan_instalment_amount = $('#loan_instalment_amount' + employee_id).val();
                var advance_account = $('#advance_account' + employee_id).val();
                var advance_salary = $('#advance_salary' + employee_id).val();
                var payable_amount = $('#payable_amount' + employee_id).val();
                jsonData[pro_index] = {
                    ...
                    'loan_account_id': loan_acount,
                    'total_loan_amount': total_loan_amount,
                    'loan_installment': loan_instalment_amount,
    ...
    <script>
        ...
        function view_ad(id) {
            ...
            // jQuery(".pre-loader").fadeToggle("medium");
            jQuery("#table_body_ad").html("");
            var month_ = jQuery("#month").val();

            const words = month_.split(' ');
            var year = words[1];
            ...
            var month = '';
            if (get_month < 10) {
                month = year + '-0' + get_month + '-01';
            } else {
                month = year + '-' + get_month + '-01';
            }

            $("#table_body_ad").load('{{ url('allowance_deduction_items_view_details/view/') }}/' + id + '/' + month, function () {
                // jQuery(".pre-loader").fadeToggle("medium");
                $("#myModalAd").modal({show: true});
            });
        }
    </script>
    ...
    <script>
        $('#department').change(function () {
            $('#total_salary_amount').val('');
            $('#total_salary_amount_display').text(0);
            $('#total_amount').val('');
        ...
        function get_employee(department_id, department_text = null) {
            ...
            if(month != '' && department_id != ''){
                ...
                jQuery.ajax({
                    url: "get_salary_accounts_with_load",
                    data: {
                    ...
                    success: function (data) {
                        ...
                        if (data.generated == 0) {
                            ...
                            $.each(data.pay_accounts, function (index, value) {
                                ...
                                var total_absent_in_year = 0;
                                var total_short_leave_in_year = 0;
                                if (value.total_year_short_leave != null) {
                                    total_short_leave_in_year = value.total_year_short_leave / 2;
                                    total_absent_in_year = value.total_year_short_leave / 2
                                }
                                if (value.total_year_absent != null) {
                                    total_absent_in_year = +value.total_year_absent + +total_short_leave_in_year;
                                }
                                var absent_days = 0;
                                if (total_absent_in_year > 8) {
                                    var total_absent = +value.total_absent + (value.total_short_leave / 2);
                                    var calculate_absent = total_absent_in_year - 8;
                                    if (calculate_absent <= total_absent) {
                                        absent_days = calculate_absent;
                                    } else {
                                ...
                                var extra_lecture_payment = 0;
                                var extra_lecture = 0;
                                var extra_lecture_charges = 0;

                                if (value.total_extra_lecture != null) {
                                    extra_lecture = value.total_extra_lecture;
                                    extra_lecture_payment = extra_lecture * value.extra_load_amount;
                                    extra_lecture_charges =value.extra_load_amount;
                                }
                                if (value.total_extra_lecture != null) {
                                    extra_lecture_payment = value.total_extra_lecture * value.extra_load_amount;
                                    extra_lecture_charges =value.extra_load_amount;
                                }
                                let total_attend_days = 0;
                                if (value.total_present != null) {
                                    total_attend_days = value.total_present + +value.total_short_leave + +value.total_absent - absent_days;
                                }
                                if (value.total_short_leave != null) {
                                    total_attend_days = +value.total_present + +value.total_short_leave + +value.total_absent - absent_days;
                                }
                                if(value.total_present == null){
                                    total_attend_days = +value.total_present + +value.total_short_leave + +value.total_absent - absent_days;
                                ...
                                jQuery("#table_body").append(
                                    ...
                                    `<th> ${index} </th> <td class="tbl_srl_9" hidden> <input type="text"
                                name="employee_id[]" tabindex="-1" value="${value.si_user_id}" class="inputs_up_tbl form-control " readonly>
                                </td>
                                <td class="tbl_srl_9" hidden> <input type="hidden" name="department_id[]" id="department_id${value.si_user_id}" tabindex="-1" value="${department_id}" class="inputs_up_tbl
                                form-control " readonly>
                                </td>
                                <td class="tbl_srl_8" hidden> <input type="hidden" name="department_name[]" tabindex="-1" value="${department_text}" id="department_name${value.si_user_id}"
                                class="inputs_up_tbl form-control text-left"
                                readonly>${department_text} </td>
                                <td class="tbl_srl_9" hidden><input type="text" name="account_code[]" id="account_code${value.si_user_id}" tabindex="-1" value="${value.account_uid}" class="inputs_up_tbl form-control " readonly>
                                </td>
                                <td class="text-left tbl_txt_8"><input type="hidden" name="account_name[]" tabindex="-1" value="${value.account_name}" id="account_name${value.si_user_id}" class="inputs_up_tbl form-control text-left"
                                readonly> ${value.account_name} </td>
                                <td class="text-left tbl_txt_5"><input type="text" name="month_days[]" id="month_days${value.si_user_id}" class="inputs_up_tbl form-control text-left" tabindex=" ${tabindex++}" placeholder="Enter Month Days" value="${total_days}" onkeyup="gross_salary(${value.si_user_id})" onkeypress="return allow_only_number_and_decimals(this,event);" readonly> </td>
                                <td class="text-left tbl_txt_5">
                                <input type="text" name="attend_days[]" id="attend_days${value.si_user_id}" class="inputs_up_tbl form-control text-left" tabindex="${tabindex++}" placeholder="Enter Attend Days" value="${total_attend_days}" onkeyup="gross_salary(${value.si_user_id})" onkeypress="return allow_only_number_and_decimals(this,event);" ></td>
                                <td class="text-left tbl_txt_5" hidden>
                                <input type="hidden" name="salary_period[]" id="salary_period${value.si_user_id}" value="${value.si_basic_salary_period}" class="inputs_up_tbl form-control text-left"
                                tabindex="${tabindex++}" placeholder="Enter Hour Day Month" readonly>${period}</td>
                                <td class="text-left tbl_txt_7"> <input type="text" name="basic_salary[]" id="basic_salary${value.si_user_id}" value="${value.si_basic_salary}" class="inputs_up_tbl
                                 form-control text-left" tabindex="${tabindex++}" placeholder="BasicPackage Salary" onkeyup="gross_salary(${value.si_user_id})" onkeypress="return allow_only_number_and_decimals(this,event);" ></td>
                                <td class="text-left tbl_txt_2">
                                <input type="text" name="gross_salary[]" id="gross_salary${value.si_user_id}" class="inputs_up_tbl form-control text-left total_gross_salary" tabindex="${tabindex++}" placeholder="Gross Salary" onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-left tbl_txt_6 view" onclick="view_ad(${value.si_user_id})">
                                 <input type="text" name="allowances[]" id="allowances${value.si_user_id}" class="inputs_up_tbl form-control text-left" tabindex="${tabindex++}"
                                 placeholder="Allowance" value="${allowance}"onkeypress="return allow_only_number_and_decimals(this,event);" onkeyup="gross_salary( ${value.si_user_id} )" readonly></td>
                                 <td class="text-left tbl_txt_6 view" onclick="view_ad(${value.si_user_id})">
                                <input type="text" name="deductions[]" id="deductions${value.si_user_id}" class="inputs_up_tbl form-control text-left" tabindex="${tabindex++}"
                                placeholder="Deduction" value="${deduction}" onkeypress="return allow_only_number_and_decimals(this,event);" onkeyup="gross_salary(${value.si_user_id})" readonly>
                                </td>
                                <td class="text-left tbl_txt_6">
                                <input type="text" name="over_time_charges[]" id="over_time_charges${value.si_user_id}" value="${extra_lecture_charges}"
                                class="inputs_up_tbl form-control text-left tabindex="${tabindex++}" placeholder="Over Time Charges" readonly>
                               </td>

                                        <td class="text-left tbl_txt_6">
                                <input type="text" name="over_time_days[]" id="over_time_days${value.si_user_id}" value="${extra_lecture}" onkeyup="gross_salary(${value.si_user_id})"
                                class="inputs_up_tbl form-control text-left tabindex="${tabindex++}" placeholder="days"
                                onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-left tbl_txt_6">
                                <input type="text" name="over_time_amount[]" id="over_time_amount${value.si_user_id}" value="${extra_lecture_payment}" class="inputs_up_tbl form-control text-left"
                                tabindex="${tabindex++}" placeholder="Amount" onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-right tbl_srl_8"><input type="text"
                                tabindex="${tabindex++}" name="net_salary[]" id="net_salary${value.si_user_id}" placeholder="Enter Amount" class="inputs_up_tbl form-control text-right net_salary_amount" onkeyup="calculate_amount();" onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-right tbl_srl_8"><input type="text" tabindex="${tabindex++}" name="advance_salary[]" id="advance_salary${value.si_user_id}" placeholder="Enter Amount" value="${total_advance_salary}" class="inputs_up_tbl form-control sum_advance_salary text-right " onkeyup="calculate_amount();" onkeypress="return
                                allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-right tbl_srl_8" hidden> <input type="text" tabindex="${tabindex++}" name="total_loan_amount[]" id="total_loan_amount${value.si_user_id}"
                                placeholder="Enter Amount" class="inputs_up_tbl form-control text-right" value="${loan_amount}" onkeyup="calculate_amount();"
                                onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-right tbl_srl_8"> <input type="text" tabindex="${tabindex++}"
                                 name="loan_instalment_amount[]" id="loan_instalment_amount${value.si_user_id}" placeholder="Enter Amount" value="${instalment_amount}" class="inputs_up_tbl form-control text-right " onkeyup="calculate_amount();" onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>

                                <td class="text-right tbl_srl_10" hidden><input type="text" tabindex="${tabindex++}"
                                 name="loan_acount[]" id="loan_acount${value.si_user_id}" placeholder="Enter Amount" value="${value.si_loan_account_uid}"
                                class="inputs_up_tbl form-control text-right" onkeyup="calculate_amount();"  onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                <td class="text-right tbl_srl_10" hidden> <input type="text"  tabindex="${tabindex++}" name="advance_account[]" id="advance_account${value.si_user_id}"
                                placeholder="Enter Amount" value="${value.si_advance_salary_account_uid}" class="inputs_up_tbl
                                form-control text-right " readonly></td>
                                <td class="text-right tbl_srl_8" > <input type="text" tabindex="${tabindex++}" name="payable_amount[]" id="payable_amount${value.si_user_id}" placeholder="Enter Amount" class="inputs_up_tbl form-control text-right employee_salary_amount" onkeyup="calculate_amount();" onkeypress="return allow_only_number_and_decimals(this,event);" readonly></td>
                                </tr>`);
        ...
        function calculate_amount() {
            ...
            document.getElementById("total_salary_amount").value = total_salary.toFixed(2);
            document.getElementById("total_salary_amount_display").innerText = total_salary.toFixed(2);
        }
    ...
    <script>
        function gross_salary(id) {
            ...
            $('#over_time_amount' + id).val(over_time_amount);
    ...
    <script>
        $('#month').click(function () {
            ...
            jQuery("#total_salary_amount").val('');
            jQuery("#total_salary_amount_display").html(0);
        });
    </script>
