    ...
    <div class="row">
        <div class="container-fluid search-filter form-group form_manage">
            <div class="form_header">
                ...
                <div class="clearfix">
                    <div class="pull-left">
                        <h4 class="text-white get-heading-text file_name">School List</h4>
                    </div>
            ...
            <div class="search_form m-0 p-0">
                <form class="highlight prnt_lst_frm"
                    action="{{ route('school_list') . (isset($restore_list) && $restore_list == 1 ? '?restore_list=1' : '') }}"
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-8 col-md-7 col-sm-12 col-xs-12 text-right form_controls mt-2">
                            ...
                            <x-add-button tabindex="9" href="{{ route('add_school') }}" />
                ...
                <form name="edit" id="edit" action="{{ route('edit_school') }}" method="post">
                    @csrf
                    <input name="title" id="title" type="hidden">
                    {{-- <input name="remarks" id="remarks" type="hidden"> --}}
                    <input name="sch_id" id="sch_id" type="hidden">
                </form>
                <form name="delete" id="delete" action="{{ route('delete_school') }}" method="post">
                    @csrf
                    <input name="sch_id" id="sch_id" type="hidden">
                </form>
            </div>
                ...
                <table class="table table-bordered table-sm" id="fixTable">
                    ...
                    <tbody>
                        ...
                        @forelse($datas as $data)
                            <tr data-title="{{ $data->sch_name }}" data-sch_id="{{ $data->sch_id }}">
                                {{--                                        <td class="edit "> --}}
                                {{--                                            {{$sr}} --}}
                                {{--                                        </td> --}}
                                ...
                                @php
                                    $ip_browser_info = '' . $data->sch_ip_adrs . ',' . str_replace(' ', '-', $data->sch_brwsr_info) . '';
                                @endphp
                                ...
                                <td class="text-center hide_column ">
                                    <label class="switch">
                                        <input type="checkbox" <?php if ($data->sch_disable_enable == 1) {
                                            echo 'checked="true"' . ' ' . 'value=' . $data->sch_disable_enable;
                                        } else {
                                            echo 'value=DISABLE';
                                        } ?> class="enable_disable"
                                            data-id="{{ $data->sch_id }}"
                                            {{ $data->sch_disable_enable == 1 ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>

                                {{-- <td class="text-center hide_column ">
                                    <a data-sch_id="{{ $data->sch_id }}" class="delete" data-toggle="tooltip"
                                        data-placement="left" title="" data-original-title="Are you sure?">
                                        <i class="fa fa-{{ $data->sch_delete_status == 1 ? 'undo' : 'trash' }}"></i>
                                    </a>
                                </td> --}}


                                {{--                                        <td><input type="checkbox" data-id="{{ $data->sch_id }}" name="status" class="js-switch" {{ $data->sch_disabled == 0 ? 'checked' : '' }}></td> --}}
                            </tr>
                            @php
                                $sr++;
                        ...
                        @empty
                            <tr>
                                <td colspan="11">
                                    <center>
                                        <h3 style="color:#554F4F">No School</h3>
                                    </center>
                                </td>
                            </tr>
                        @endforelse
...
@section('scripts')
        ...
        $(document).ready(function() {
            $('.enable_disable').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('enable_disable_school') }}',
                    data: {
                        'status': status,
                        'sch_id': sch_id
                    },
    ...
    <script>
        jQuery(".edit").click(function() {
            ...
            var sch_id = jQuery(this).parent('tr').attr("data-sch_id");

            jQuery("#title").val(title);
            // jQuery("#remarks").val(remarks);
            jQuery("#sch_id").val(sch_id);
            jQuery("#edit").submit();
        });

        $('.delete').on('click', function(event) {

            var sch_id = jQuery(this).attr("data-sch_id");
