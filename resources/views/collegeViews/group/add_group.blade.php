                            ...
                            <select name="group_names" id="group_names" class="inputs_up form-control"
                                ...
                                @foreach ($groups as $group)
                                    <option value="{{ $group->ng_id }}"
                                        {{ $group->ng_id == old('group_names') ? 'selected' : '' }}>{{ $group->ng_name }}
                                    </option>
                                @endforeach
                            </select>
                        </div><!-- end input box -->
...
<script>
    $(document).ready(function () {
        ...
        function updateClassOptions() {
            ...
            var selectTermOptionAdded = false; // Flag to track if the "Select Class" option is added
            ...
            if (isSemester) {
                ...
                @foreach ($classesSemester as $class)
                    ...
                    classDropdown.append(`<option value="{{ $class->class_id }}">{{ $class->class_name }}</option>`);
                @endforeach
                @foreach ($semesters as $item)
                    ...
                    termDropdown.append(`<option value="{{ $item->semester_id }}">{{ $item->semester_name }}</option>`);
                @endforeach
                $('.semester-div').show();
            } else {
                $('#label').html('Annual');
                @foreach ($classesAnnual as $class)
                    // Add the "Select Annaul" option only if it has not been added yet
                    if (!selectClassOptionAdded) {
                        classDropdown.append('<option value="" selected>Select Class</option>');
                        selectClassOptionAdded = true; // Set the flag to true once the option is added
                    }
        ...
        $('input[type="radio"].annual, input[type="radio"].semester').on('change', updateClassOptions);
