        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="pd-20 bg-white border-radius-4 box-shadow mb-30 form_manage" id="">
                <div class="form_header">
                    ...
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-white get-heading-text file_name">{{$search_status == 'fee_voucher' ? 'Fee' : ($search_status == 'custom_voucher' ? 'Custom' : 'Transport')}}
                                Voucher Pending
                                List</h4>
                        </div>
            ...
            <!-- <div class="search_form {{ !empty($search) || !empty($search_to) || !empty($search_from) ? '' : 'search_form_hidden' }}"> -->
                ...
                <div class="search_form m-0 p-0">
                    <form class="highlight prnt_lst_frm" action="{{ route('fee_voucher_pending_list') }}" name="form1"
                                    ...
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx">
                                            ...
                                            <select tabindex="5" name="status" class="inputs_up form-control" id="status">
                                                ...
                                                <option value="fee_voucher" {{ 'fee_voucher' == $search_status ? 'selected="selected"' : '' }}>
                                                    Fee Voucher
                                                </option>
                                                <option value="custom_voucher" {{ 'custom_voucher' == $search_status ? 'selected="selected"' : '' }}>
                                                    Custom Voucher
                                                </option><option value="transport_voucher" {{ 'transport_voucher' == $search_status ? 'selected="selected"' : '' }}>
                                                    Transport Voucher
                                                </option>
                                            </select>
                                        </div>
                    ...
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        @forelse($datas as $voucher)
                            ...
                            <tr data-toggle="tooltip" data-placement="top" title=""
                                ...
                                <td>
                                    {{$search_status == 'fee_voucher' ? 'FV-' : ($search_status == 'custom_voucher' ? 'CV-' : 'TV-')}}{{ $voucher->v_no }}
                                </td>
                                <td>
                                ...
                                <form action="{{route('paid_single_fee_voucher',$voucher->id)}}" method="post">
                                    ...
                                    <td>
                                        <input type="text" name="amount" class="inputs_up form-control" value="{{$voucher->total_amount}}"
...
@section('scripts')
    ...
    <script type="text/javascript">
        var base = '{{ route('fee_voucher_pending_list') }}',
            url;
    ...
    <script>
        ...
        jQuery(".view").click(function () {
            ...
            $(".modal-body").load('{{ url('fee_items_view_details/view/') }}/' + id, function () {
                $("#myModal").modal({
                    show: true
                });
            });
