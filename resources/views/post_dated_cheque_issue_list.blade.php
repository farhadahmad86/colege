...
@section('content')
    ...
    @php
        $route='';
        if($title=='Confirmed Post Dated Cheque Issue List'){
        $route='approve_post_dated_cheque_issue_list';
        }elseif ($title=='Rejected Post Dated Cheque Issue List'){
        $route='reject_post_dated_cheque_issue_list';
        }else{
        $route='post_dated_cheque_issue_list';
        }
    @endphp
                                ...
                                <div class="row">
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx"><!-- start input box -->
                                            ...
                                            <select tabindex="2" name="from_accounts" class="inputs_up form-control" id="from_accounts">
                                                <option value="">Select Account</option>
                                                @foreach($from_accounts as $from_account)
                                                    <option value="{{$from_account->account_uid}}" {{ $from_account->account_uid == $search_account_from ? 'selected="selected"' : '' }} >
                                    ...
                                    <div class="form-group col-lg-2 col-md-3 col-sm-12 col-xs-12">
                                        <div class="input_bx"><!-- start input box -->
                                            ...
                                            <select tabindex="3" name="to_accounts" class="inputs_up form-control" id="to_accounts">
                                                <option value="">Select Account</option>
                                                @foreach($to_accounts as $to_account)
                                                    <option value="{{$to_account->account_uid}}" {{ $to_account->account_uid == $search_account_to ? 'selected="selected"' : '' }}>
...
<x-add-button tabindex="9"  href="{{ route('add_post_dated_cheque_issue') }}"/>
                ...
                <div class="table-responsive" id="printTable">
                    <table class="table table-bordered table-sm" id="fixTable">
                        ...
                        <tr>
                            ...
                            @if($title=='Rejected Post Dated Cheque Issue List')
                                <th scope="col" class="tbl_txt_34">
                                    Reason
                                </th>
                            @else
                                <th scope="col" class="{{ ($title=='Pending Post Dated Cheque Issue List') ? 'tbl_txt_34' : 'tbl_txt_38' }} ">
                                    Remarks
                                </th>
                            @endif
                            <th scope="col" class="tbl_amnt_10">
                            ...
                            @if($title=='Pending Post Dated Cheque Issue List')
                        ...
                        @forelse($datas as $cheque)
                            ...
                            <tr>
                                ...
                                <td class="usr_prfl" data-usr_prfl="{{ $cheque->user_id }}" data-user_info="{!! $ip_browser_info !!}" title="Click To See User Detail">
                                    {{ $cheque->user_name }}
                                </td>
                                ...
                                @if($title=='Pending Post Dated Cheque Issue List')
                                    <td class="hide_column">
                                        <a class="btn btn-sm btn-success approve rjct_acpt_btn" data-id="{{$cheque->pdc_id}}" data-send_by_account="{{$cheque->pdc_account_code}}"
                                           data-send_by_account_name="{{$cheque->from}}" data-receive_by_account="{{$cheque->pdc_party_code}}" data-receive_by_account_name="{{$cheque->to}}"
                                           data-amount="{{$cheque->pdc_amount}}" data-cheque_date="{{$cheque->pdc_cheque_date}}" data-remarks="{{$cheque->pdc_remarks}}">
                                            <i class="fa fa-check approve" style="padding: 0 0px; cursor: pointer"></i>
                                        </a>
                                        <a class="btn btn-sm btn-danger reject rjct_acpt_btn" data-id="{{$cheque->pdc_id}}" data-toggle="modal" data-target="#exampleModal">
                                            <i class="fa fa-ban reject" style="padding: 0 0px;cursor: pointer"></i>
                                        </a>
                                    </td>
                                @endif
    ...
    @if($title=='Pending Post Dated Cheque Issue List')
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{route("reject_post_dated_cheque_issue")}}" method="post">
                        @csrf
        ...
        <form name="approve_form" id="approve_form" action="{{route("approve_post_dated_cheque_issue")}}" method="post">
            @csrf
