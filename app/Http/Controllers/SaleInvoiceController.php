...
use App\Models\SaleServiceSaletaxInvoiceItemsModel;
...
class SaleInvoiceController extends Controller
{
    ...
    public function sale_invoice()
    {
        ...
        return view('Simple_Invoices/simple_sale_invoice', compact('sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'detail_remarks'));
    }
    ...
    public function submit_sale_invoice(Request $request)
    {
        ...
        foreach ($requested_arrays as $index => $requested_array) {
            ...
            $item_remarks = isset($request->product_remarks[$index]) ? $request->product_remarks[$index] : ''; //$request->product_remarks[$index];
        ...
        if (!empty($sales_array)) {
            ...
            foreach ($item as $value) {
                ...
                $detail_remarks .= $product_name . ', ' . $value[$sale_items_prefix . '_qty'] . '@' . number_format($pro_rate, 2) . ' = ' . number_format($pro_amount, 2) . config('global_variables.Line_Break');
            }
            ...
            if ($transaction1->save()) {
                ...
                $balance1 = $this->AssignAccountBalancesValues(
                    $balances1,
                    $transaction_id1,
                    $stock_in_hand,
                    $stock_amount,
            ...
            if ($transaction2->save()) {
                ...
                $balance2 = $this->AssignAccountBalancesValues(
                    $balances2,
                    $transaction_id2,
                    $sale_account,
                    $sale_amount,
            ...
            if ($invoice_type == 2 && $product_total_saletax > 0) {
                ...
                if ($transaction3->save()) {
                    ...
                    $balance3 = $this->AssignAccountBalancesValues(
                        $balances3,
                        $transaction_id3,
                        $sale_tax_account,
                        $product_total_saletax,
                        'Cr',
                        $request->remarks,
                        $notes,
                        $detail_remarks,
            ...
            if ($transaction4->save()) {
                ...
                $balance4 = $this->AssignAccountBalancesValues(
                    $balances4,
                    $transaction_id4,
                    $account_code,
                    $product_grand_total,
            ...
            if ($product_discount_amount != 0) {
                ...
                if ($transaction5->save()) {
                    ...
                    $balance5 = $this->AssignAccountBalancesValues(
                        $balances5,
                        $transaction_id5,
                        $product_discount_account,
            ...
            if ($product_round_off_discount_amount != 0) {
                ...
                if ($transaction6->save()) {
                    ...
                    $balance6 = $this->AssignAccountBalancesValues(
                        $balances6,
                        $transaction_id6,
                        $round_off_discount_account,
                        $product_round_off_discount_amount,
                        'Dr',
                        $request->remarks,
                        $notes,
                        $detail_remarks,
            ...
            if ($product_cash_discount_amount != 0) {
                ...
                if ($transaction7->save()) {
                    ...
                    $balance7 = $this->AssignAccountBalancesValues(
                        $balances7,
                        $transaction_id7,
                        $cash_discount_account,
                        $product_cash_discount_amount,
                        'Dr',
                        $request->remarks,
                        $notes,
                        $detail_remarks,
            ...
            if ($product_cash_paid > 0) {
                ...
                if ($transaction8->save()) {
                    ...
                    $balance8 = $this->AssignAccountBalancesValues(
                        $balances8,
                        $transaction_id8,
                        $cash_in_hand,
                        $product_cash_paid,
                        'Dr',
                        $cash_voucher_remarks,
                        $cash_voucher_notes,
                        $cash_voucher_detail_remarks,
                        $cash_voucher_voucher_code . $cash_voucher_id,
                        $request->posting_reference
                    );
                    ...
                    $balance9 = $this->AssignAccountBalancesValues(
                        $balances9,
                        $transaction_id8,
                        $account_code,
                        $product_cash_paid,
            ...
            if ($product_credit_paid != 0 && $request->machine > 0) {
                ...
                if ($transaction10->save()) {
                    ...
                    $balance10 = $this->AssignAccountBalancesValues(
                        $balances10,
                        $transaction_id10,
                        $credit_card_machine_account_code,
                        $product_credit_paid,
                        'Dr',
                        $request->remarks,
                        $notes,
                ...
                if ($transaction11->save()) {
                    ...
                    $balance11 = $this->AssignAccountBalancesValues(
                        $balances11,
                        $transaction_id11,
                        $account_code,
                        $product_credit_paid,
        ...
        if (!empty($services_array)) {
            ...
            $service_invoice = $this->AssignSaleInvoiceValues(
                $request,
                $service_invoice,
                $day_end,
                $user,
                ...
                $service_round_off_discount_amount,
                $cash_discount_per,
                $service_cash_discount_amount,
                $service_total_discount_amount,
                $service_inclusive_saletax,
                $service_exclusive_saletax,
                $service_total_saletax,
                $service_grand_total,
                $service_cash_paid
            );
            ...
            foreach ($service_item as $value) {
                ...
                $detail_remarks .= $value[$service_items_prefix . '_service_name'] . ', ' . $value[$service_items_prefix . '_qty'] . '@' . number_format($ser_rate, 2) . ' = ' . number_format($ser_amount, 2) . config('global_variables.Line_Break');
            }
            ...
            if ($transaction1->save()) {
                ...
                $balance1 = $this->AssignAccountBalancesValues(
                    ...
                    $service_notes,
                    $detail_remarks,
                    $service_voucher_code . $service_invoice_id,
                    $request->posting_reference
                );
            ...
            if ($service_cash_paid > 0) {
                ...
                $transactions8 = new TransactionModel();
                $transaction8 = $this->AssignTransactionsValues(
                    $transactions8,
                    $cash_in_hand,
                    $service_cash_paid,
                    $account_code,
                    $cash_voucher_notes,
                    $cash_voucher_transaction_type,
                    $cash_voucher_id
                );
            ...
            if ($service_credit_paid != 0 && $request->machine > 0) {
                ...
                $transaction10 = $this->AssignTransactionsValues(
                    $transactions10,
                    $credit_card_machine_account_code,
                    $service_credit_paid,
                    0,
        ...
        if ($rollBack) {
            ...
            return redirect()->back()->with(['si_id' => $sale_invoice_id]);
    ...
    public
    function AssignSaleInvoiceValues(
        $request,
        $sale_invoice,
        $day_end,
        $user,
    ...
    function sale_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('sale_invoice_list', compact('datas', 'search', 'party', 'accounts', 'sale_persons', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'search_sale_persons', 'urdu_eng', 'posting_references', 'search_posting_reference'));
        }
    }
    ...
    public
    function sale_register(Request $request, $array = null, $str = null)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        ...
        $check_desktop = $request->check_desktop;
        $search_by_user = (isset($request->search_by_user) && !empty($request->search_by_user)) ? $request->search_by_user : '';
        $prnt_page_dir = 'print.sale_invoice_list.sale_register';
        $pge_title = 'Sale Register';
        $srch_fltr = [];
        array_push($srch_fltr, $search, $search_account, $search_product, $search_sale_persons, $search_to, $search_from);
        ...
        if ($urdu_eng->rc_invoice_party == 0) {
            $query = DB::table('financials_sale_invoice')
                ->leftJoin('financials_si_dc_extension', 'financials_si_dc_extension.sde_sale_id', 'financials_sale_invoice.si_id')
                ->leftJoin('financials_users', 'financials_users.user_id', 'financials_sale_invoice.si_sale_person');
        } else {
            $query = DB::table('financials_sale_invoice')
                ->leftJoin('financials_si_dc_extension', 'financials_si_dc_extension.sde_sale_id', 'financials_sale_invoice.si_id')
                ->join('financials_accounts', 'financials_accounts.account_uid', '=', 'financials_sale_invoice.si_party_code')
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('sale_register', compact('datas', 'search', 'party', 'accounts', 'sale_persons', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'search_sale_persons', 'urdu_eng'));
        }
    }
    ...
    public
    function sale_items_view_details(Request $request)
    {
        $items = SaleInvoiceItemsModel::where('sii_invoice_id', $request->id)->orderby('sii_product_name', 'ASC')->get();

        return response()->json($items);
    }

    public
    function sale_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            ...
            $sim = DB::table('financials_sale_invoice')
                ...
                ->where('si_id', $id)
                ->select(
                    'financials_accounts.account_urdu_name as si_party_name',
                    'si_id',
                    'si_party_code',
                    'si_customer_name',
                    'si_remarks',
                    ...
                    'si_cash_disc_amount',
                    'si_total_discount',
                    'si_inclusive_sales_tax',
                    'si_exclusive_sales_tax',
                    'si_total_sales_tax',
                    'si_grand_total',
                    'si_cash_received',
                    'si_day_end_id',
                    ...
                    'si_createdby',
                    'si_sale_person',
                    'si_service_invoice_id',
                    'si_local_invoice_id',
                    'si_local_service_invoice_id',
                    'si_cash_received_from_customer',
                    'si_return_amount'
                )->first();
        }
    ...
    public
    function sale_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            $sim = SaleInvoiceModel::leftJoin('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_invoice.si_pr_id')->where('si_id', $id)->first();
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled' => true, 'isRemoteEnabled' => true]);
        $pdf->getDomPDF()->setHttpContext($options, $optionss);
        $pdf->loadView('invoice_view.sale_invoice.sale_invoice_list_modal', compact('siims', 'sim', 'seim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'cash_received', 'urdu_eng', 'invoice_nbr', 'invoice_date'));
        //        $pdf->setOptions($options);
    ...
    // update code by shahzaib start
    public
    function sale_tax_sale_invoice_list(Request $request, $array = null, $str = null)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        $heads = config('global_variables.payable_receivable_cash');
        $heads = explode(',', $heads);
        ...
        $accounts = AccountRegisterationModel::whereIn('account_parent_code', $heads)->orderBy('account_name', 'ASC')->get();
        $products = ProductModel::orderBy('pro_title', 'ASC')->get();
        $sale_persons = User::where('user_delete_status', '!=', 1)->where('user_id', '!=', 1)->orderBy('user_role_id', 'DESC')->orderBy('user_name', 'ASC')->get();
        $posting_references = PostingReferenceModel::all();
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('sale_invoice_sale_tax_list', compact('datas', 'search', 'party', 'accounts', 'sale_persons', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'search_sale_persons', 'urdu_eng', 'search_posting_reference', 'posting_references'));
        }
    }
    ...
    public
    function sale_sale_tax_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0) {
            ...
            $sim = DB::table('financials_sale_saletax_invoice')
                ...
                ->where('ssi_id', $id)
                ->select(
                    'financials_accounts.account_urdu_name as ssi_party_name',
                    'ssi_id',
                    'ssi_party_code',
                    'ssi_customer_name',
                    'ssi_remarks',
                    ...
                    'ssi_round_off_disc',
                    'ssi_cash_disc_per',
                    'ssi_cash_disc_amount',
                    'ssi_total_discount',
                    'ssi_inclusive_sales_tax',
                    'ssi_exclusive_sales_tax',
                    'ssi_total_sales_tax',
                    'ssi_grand_total',
                    'ssi_cash_received',
                    'ssi_day_end_id',
                    ...
                    'ssi_createdby',
                    'ssi_sale_person',
                    'ssi_service_invoice_id',
                    'ssi_local_invoice_id',
                    'ssi_local_service_invoice_id',
                    'ssi_cash_received_from_customer',
                    'ssi_return_amount'
                )->first();
        }
        ...
        if ($urdu_eng->rc_invoice == 0) {
            $services = DB::table('financials_service_saletax_invoice_items')
                ...
                ->select(
                    'sesii_service_name as name',
                    'sesii_remarks as remarks',
                    'sesii_qty as qty',
                    'sesii_rate as rate',
                    'sesii_discount_per as discount',
                    'sesii_discount_amount as discount_amount',
                    'sesii_after_dis_rate as after_discount',
                    'sesii_saletax_per as sale_tax',
                    'sesii_scale_size as scale_size',
                    'sesii_saletax_amount as sale_tax_amount',
                    'sesii_saletax_inclusive as inclu_exclu',
                    'sesii_amount as amount',
                    'sesii_saletax_inclusive as type'
                );
        ...
        return view('invoice_view.sale_sale_tax_invoice.sale_sale_tax_invoice_list_modal', compact(
            'siims',
            'sim',
            'accnts',
            'nbrOfWrds',
    ...
    public
    function sale_sale_tax_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            $sim = SaleSaletaxInvoiceModel::leftJoin('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_saletax_invoice.ssi_pr_id')->where('ssi_id', $id)->first();
        ...
        $pdf->loadView('invoice_view.sale_sale_tax_invoice.sale_sale_tax_invoice_list_modal', compact(
            'siims',
            'sim',
            'nbrOfWrds',
            'accnts',
        ...
        return $pdf->stream('Sale-Tax-Sale-Invoice.pdf');
    }

    public function get_sale_person(Request $request)
    {
        $sale_person = DB::table('financials_accounts')
            ->join('financials_users', 'financials_users.user_id', '=', 'financials_accounts.account_sale_person')
            ->where('account_uid', '=', $request->party)->select('financials_users.user_id', 'financials_users.user_name')->get();

        if (sizeof($sale_person) == 0) {
            $sale_person = User::where('user_id', '=', 3)->select('user_id', 'user_name')->get();
            return response()->json($sale_person);
        } else {
            return response()->json($sale_person);
        }
    }
