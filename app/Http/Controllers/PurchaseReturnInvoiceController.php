...
//        return view('purchase_return_invoice', compact('pro_code', 'pro_name', 'accounts', 'warehouses', 'posting_references'));
        return view('Simple_Invoices/simple_purchase_return_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_purchase_return_invoice(Request $request)
    {
...
//        }
    ...
    public function purchase_return_invoice_list(Request $request, $array = null, $str = null)
    {
...
//            $pdf->setOptions($options);
    ...
    public function purchase_return_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnInvoiceModel::where('pri_id', $id)->first();
        }else{
            $pim = DB::table('financials_purchase_return_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as pri_party_name','pri_id','pri_party_code','pri_customer_name','pri_remarks','pri_total_items','pri_total_price','pri_product_disc','pri_round_off_disc',
                    'pri_cash_disc_per','pri_cash_disc_amount','pri_total_discount','pri_inclusive_sales_tax','pri_exclusive_sales_tax','pri_total_sales_tax','pri_grand_total','pri_cash_paid','pri_day_end_id','pri_day_end_date','pri_createdby')->first();
        }
    ...
    public function purchase_return_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnInvoiceModel::where('pri_id', $id)->first();
        ...
        $pdf->loadView('invoice_view.purchase_return_invoice.purchase_return_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);
