...
class GoodsReceiptNotePurchaseController extends Controller
{
    public $grn_purchase_total_stock_amount=0;
    public function grn_purchase_invoice()
    {
...
//        $accounts = $this->get_account_query('purchase')[0];
        ...
        foreach ($products as $product) {
            ...
            $pro_code .= "<option value='$product->pro_p_code' data-parent='$product->pro_p_code' data-purchase_price='$product->pro_last_purchase_rate' data-sale_price='$product->pro_sale_price' data-unit='$product->unit_title' data-unit_scale_size='$product->unit_scale_size'> $product->pro_p_code</option>";
            $pro_name .= "<option value='$product->pro_p_code' data-parent='$product->pro_p_code' data-purchase_price='$product->pro_last_purchase_rate' data-sale_price='$product->pro_sale_price' data-unit='$product->unit_title' data-unit_scale_size='$product->unit_scale_size'> $pro_title</option>";
        }
...
//
        ...
        $products = $query
            ->groupBy('grn_id')->pluck('financials_goods_receipt_note.grn_id');


        return view('goods_receipt_note_purchase_invoice', compact('pro_code', 'pro_name','products'));
    }

    public function submit_grn_purchase_invoice(Request $request)
    {
...
//        $cash_in_hand_account_name = $this->get_account_name($cash_in_hand);
        ...
        if ($invoice_type == 1) {
            ...
            $voucher_code = config('global_variables.GOODS_RECEIPT_NOTE_PURCHASE_VOUCHER_CODE');
            ...
            $voucher_code = config('global_variables.GOODS_RECEIPT_NOTE_PURCHASE_VOUCHER_CODE_PURCHASE_SALE_TAX_VOUCHER_CODE');
