...
class TradeInternalClaimStockReceiveController extends Controller
{
    public function add_trade_claim_stock_receive()
    {
        ...
        return view('Trade_Invoices/add_trade_internal_claim_stock_transfer_warehouse_voucher', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_claim_stock_receive(Request $request)
    {
...
//        $partyClaim->pc_sale_return_invoice_num = '';
    ...
    public function trade_claim_stock_receive_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Claim Stock Receive';
        $srch_fltr = [];
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_claim_stock_receive_list', compact('partyClaims', 'ttl_amount', 'search', 'search_to', 'search_from'));
        }
    }
    ...
    public function trade_claim_stock_receive_items_view_details_SH(Request $request, $id)
    {
        ...
        $type = 'grid';
        $pge_title = 'Trade Claim Stock Receive Items';

        return view('trade_voucher_view.trade_party_claim.receive.trade_party_stock_receive_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'invoice_remarks'));
    }

    public function trade_claim_stock_receive_items_view_details_pdf_SH(Request $request, $id)
    {
        $jrnl = PartyClaimModel::where('pc_id', $id)->first();
        $items = PartyClaimItemModel::where('pci_pc_id', $id)->get();
        $nbrOfWrds = $this->myCnvrtNbr($jrnl->pc_total_amount);
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf = PDF::loadView('trade_voucher_view.trade_party_claim.receive.trade_party_stock_receive_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'type','invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);
