...
class TradePurchaseInvoiceController extends Controller
{
    public $invoice_total_value=0;

    public function trade_purchase_invoice()
    {
//        $user = Auth::user();
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Trade_Invoices/trade_purchase_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_purchase_invoice(Request $request)
    {
...
//            return redirect()->back()->with('success', 'Successfully Saved');
    ...
    public function trade_purchase_invoice_list(Request $request, $array = null, $str = null)
    {
...
//        $query = PurchaseInvoiceModel::query();
        ...
        if (!empty($search_by_user)) {
            $query->where('pi_createdby', $search_by_user);
        }
...
//            $query->whereBetween('pi_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('/Trade_Invoices/trade_purchase_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_code', 'pro_name', 'search_to', 'search_from', 'search_by_user', 'posting_references', 'search_posting_reference'));
        }
    ...
    public function trade_purchase_items_view_details(Request $request)
    {
    ...
    public function trade_purchase_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0){
            ...
            $pim = DB::table('financials_purchase_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as pi_party_name','pi_id','pi_party_code','pi_customer_name','pi_remarks','pi_total_items','pi_total_price','pi_product_disc','pi_round_off_disc',
                    'pi_cash_disc_per','pi_cash_disc_amount','pi_total_discount','pi_inclusive_sales_tax','pi_exclusive_sales_tax','pi_total_sales_tax','pi_grand_total','pi_cash_paid','pi_day_end_id','pi_day_end_date','pi_createdby','financials_posting_reference.pr_name')->first();
        }
...
//            ->select('pii_product_name as name', 'pii_remarks as remarks', 'pii_qty as qty', 'pii_rate as rate', 'pii_discount as discount', 'pii_saletax as sale_tax', 'pii_amount as amount')
    ...
    public function trade_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_invoice.pi_pr_id')->where('pi_id', $id)->first();
...
//            ->select('pii_product_name as name', 'pii_remarks as remarks', 'pii_qty as qty', 'pii_rate as rate', 'pii_discount as discount', 'pii_saletax as sale_tax', 'pii_amount as amount')
        ...
        $pdf->loadView('trade_invoice_view.trade_purchase_invoice.trade_purchase_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type',
            'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);

        return $pdf->stream('Trade-Purchase-Invoice.pdf');
    }
...
//            $query->whereBetween('pi_day_end_date', [$start, $end]);
        ...
        if ($urdu_eng->rc_invoice_party == 0){
            ...
            $pim = DB::table('financials_purchase_saletax_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as psi_party_name','psi_id','psi_party_code','psi_customer_name','psi_remarks','psi_total_items','psi_total_price',
                    ...
                    'psi_cash_disc_per','psi_cash_disc_amount','psi_total_discount','psi_inclusive_sales_tax','psi_exclusive_sales_tax','psi_total_sales_tax','psi_grand_total','psi_cash_paid','psi_day_end_id','psi_day_end_date','psi_createdby','financials_posting_reference.pr_name')->first();
        }
    ...
    public function sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.purchase_sale_tax_invoice.purchase_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng'));
        // $pdf->setOptions($options);
...
//            $query->whereBetween('pi_day_end_date', [$start, $end]);
        ...
        if ($urdu_eng->rc_invoice_party == 0){
            ...
            $pim = DB::table('financials_purchase_return_saletax_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as prsi_party_name','prsi_id','prsi_party_code','prsi_customer_name','prsi_remarks','prsi_total_items','prsi_total_price',
                    ...
                    'prsi_cash_paid','prsi_day_end_id','prsi_day_end_date','prsi_createdby','financials_posting_reference.pr_name')->first();
        }
    ...
    public function return_sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.purchase_return_sale_tax_invoice.purchase_return_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng'));
        // $pdf->setOptions($options);
