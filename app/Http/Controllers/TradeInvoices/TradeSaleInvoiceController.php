...
class TradeSaleInvoiceController extends Controller
{
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        ...
        return view('Trade_Invoices/trade_sale_invoice', compact( 'sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'detail_remarks'));
//        return view('Trade_Invoices/practice', compact('accounts', 'sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'warehouses'));
    }

    public function submit_trade_sale_invoice(Request $request)
    {
...
// update code by shahzaib start
    ...
    public function trade_sale_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Sale Invoice List';
        $srch_fltr = [];
...
//            $query->whereBetween('si_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_sale_invoice_list', compact('datas', 'search', 'party', 'accounts', 'sale_persons', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'search_sale_persons', 'urdu_eng', 'posting_references','search_posting_reference'));
        }
    }
...
// update code by shahzaib end
    ...
    public
    function trade_sale_items_view_details(Request $request)
    {
    ...
    public
    function trade_sale_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
...
//            ->select('sii_product_name as name', 'sii_remarks as remarks', 'sii_qty as qty', 'sii_rate as rate', 'sii_discount as discount', 'sii_saletax as sale_tax', 'sii_amount as amount')
        ...
        return view('trade_invoice_view.trade_sale_invoice.trade_sale_invoice_list_modal', compact('siims', 'sim', 'seim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title',
            'cash_received', 'urdu_eng'));
    }

    public function trade_sale_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $sim = SaleInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_invoice.si_pr_id')->where('si_id', $id)->first();
...
//            ->select('sii_product_name as name', 'sii_remarks as remarks', 'sii_qty as qty', 'sii_rate as rate', 'sii_discount as discount', 'sii_saletax as sale_tax', 'sii_amount as amount')
        ...
        $pdf->loadView('trade_invoice_view.trade_sale_invoice.trade_sale_invoice_list_modal', compact('siims', 'sim', 'seim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'cash_received', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);

        return $pdf->stream('Trade-Sale-Invoice.pdf');
    }
