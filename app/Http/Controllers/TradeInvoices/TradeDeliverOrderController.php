...
class TradeDeliverOrderController extends Controller
{
    public function trade_delivery_order()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled', '=', 0)->get();
        ...
        return view('Trade_Invoices/trade_delivery_order', compact( 'sale_persons', 'machines', 'service_code', 'service_name', 'packages'));
    }

    public function submit_trade_delivery_order(Request $request)
    {
...
//update code by shahzaib start
    public
    function trade_delivery_order_invoice_list(Request $request, $array = null, $str = null)
    {
...
//            $query->whereBetween('do_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_delivery_order_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user'));
        }
    }
...
// update code by shahzaib end
    ...
    public
    function trade_delivery_order_items_view_details(Request $request)
    {
    ...
    public
    function trade_delivery_order_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice == 0) {
            $siims = DB::table('financials_delivery_order_items')
                ...
                ->select('doi_product_name as name', 'doi_remarks as remarks', 'doi_qty as qty', 'doi_scale_size as scale_size', 'doi_due_qty as due_qty', 'doi_bonus_qty as bonus', 'doi_status as status')
                ->get();
        } else {
            $siims = DB::table('financials_delivery_order_items')
                ...
                ->where('doi_invoice_id', $id)
                ->orderby('doi_product_name', 'ASC')
                ->select('financials_products.pro_urdu_title as name', 'doi_remarks as remarks', 'doi_qty as qty', 'doi_scale_size as scale_size', 'doi_due_qty as due_qty', 'doi_bonus_qty as bonus', 'doi_status as status')
                ->get();
        }
...
//        $nbrOfWrds = $this->myCnvrtNbr($mainGrndTtl);
        ...
        $type = 'grid';
        $pge_title = 'Trade Delivery Order Invoice';

        return view('trade_invoice_view.trade_delivery_order.trade_delivery_order_invoice_list_modal', compact('siims', 'sim', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title'));
    }

    public
    function trade_delivery_order_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $sim = DeliveryOrderModel::where('do_id', $id)->first();
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('trade_invoice_view.trade_delivery_order.trade_delivery_order_invoice_list_modal', compact('siims', 'sim', 'accnts', 'type', 'pge_title','invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);

        return $pdf->stream('Trade-Deliver-Order-Invoice.pdf');
    }
