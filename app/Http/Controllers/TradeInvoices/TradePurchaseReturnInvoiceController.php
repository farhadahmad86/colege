...
class TradePurchaseReturnInvoiceController extends Controller
{
    public function trade_purchase_return_invoice()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Trade_Invoices/trade_purchase_return_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_purchase_return_invoice(Request $request)
    {
...
//        $cash_in_hand_account_name = $this->get_account_name($cash_in_hand);
        ...
        if ($invoice_type == 1) {
            ...
            $voucher_code = config('global_variables.TRADE_PURCHASE_RETURN_VOUCHER_CODE');
            ...
            $voucher_code = config('global_variables.TRADE_PURCHASE_RETURN_SALE_TAX_VOUCHER_CODE');
...
//            return redirect()->back()->with('success', 'Successfully Saved');
            return redirect()->back()->with(['pri_id' => $purchase_invoice_id]);
        }
    }

    // update code by shahzaib start
    public function trade_purchase_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $prnt_page_dir = 'print.purchase_return_invoice_list.purchase_return_invoice_list';
        $pge_title = 'Trade Purchase Return Invoice List';
        $srch_fltr = [];
...
//        $query = PurchaseInvoiceModel::query();
        $query = DB::table('financials_purchase_return_invoice')
            ->leftJoin('financials_users', 'financials_users.user_id', 'financials_purchase_return_invoice.pri_createdby')
            ->leftJoin('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_invoice.pri_pr_id');

        if (!empty($search)) {
            $query->where(function ($query) use ($search) {
                $query->where('pri_party_code', 'like', '%' . $search . '%')
                    ->orWhere('pri_party_name', 'like', '%' . $search . '%')
                    ->orWhere('pri_remarks', 'like', '%' . $search . '%')
                    ->orWhere('pri_id', 'like', '%' . $search . '%')
        ...
        if (!empty($search_account)) {
            $query->where('pri_party_code', $search_account);
        }
        if (!empty($search_posting_reference)) {
            $query->where('pri_pr_id', $search_posting_reference);
        }

        if (!empty($search_product)) {
            $get_p_id = PurchaseReturnInvoiceItemsModel::where('prii_product_code', $search_product)->pluck('prii_invoice_id')->all();
            $query->whereIn('pri_id', $get_p_id);
        }

        if (!empty($search_by_user)) {
            $query->where('pri_createdby', $search_by_user);
        }

        if ((!empty($search_to)) && (!empty($search_from))) {
            $query->whereBetween('pri_day_end_date', [$start, $end]);
        } elseif (!empty($search_to)) {
            $query->where('pri_day_end_date', $start);
        } elseif (!empty($search_from)) {
            $query->where('pri_day_end_date', $end);
        }

        $datas = $query->orderBy('pri_id', config('global_variables.query_sorting'))
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_purchase_return_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_name', 'pro_code', 'search_to',
                'search_from', 'search_posting_reference', 'posting_references'));
    ...
    public function trade_purchase_return_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_invoice.pri_pr_id')->where('pri_id', $id)->first();
        }else{
            $pim = DB::table('financials_purchase_return_invoice')
                ->join('financials_accounts', 'financials_accounts.account_uid', '=', 'financials_purchase_return_invoice.pri_party_code')
                ->Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_invoice.pri_pr_id')
                ->where('pri_id', $id)
                ->select('financials_accounts.account_urdu_name as pri_party_name','pri_id','pri_party_code','pri_customer_name','pri_remarks','pri_total_items','pri_total_price',
                    'pri_product_disc','pri_round_off_disc',
                    'pri_cash_disc_per','pri_cash_disc_amount','pri_total_discount','pri_inclusive_sales_tax','pri_exclusive_sales_tax','pri_total_sales_tax','pri_grand_total',
                    'pri_cash_paid','pri_day_end_id','pri_day_end_date','pri_createdby','financials_posting_reference.pr_name')->first();
        }

        $accnts = AccountRegisterationModel::where('account_uid', $pim->pri_party_code)->first();
        ...
        return view('trade_invoice_view.trade_purchase_return_invoice.trade_purchase_return_invoice_list_modal', compact('piims', 'pim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type',
            'pge_title', 'urdu_eng'));


    }

    public function trade_purchase_return_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_invoice.pri_pr_id')->where('pri_id', $id)->first();
        ...
        $pdf->loadView('trade_invoice_view.trade_purchase_return_invoice.trade_purchase_return_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);

        return $pdf->stream('Trade-Purchase-Return-Invoice.pdf');
    }
