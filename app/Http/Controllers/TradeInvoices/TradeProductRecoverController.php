...
class TradeProductRecoverController extends Controller
{
    public function trade_product_recover()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Trade_Invoices/trade_product_recover', compact('accounts', 'pro_code', 'pro_name'));
    }

    public function submit_trade_product_recover(Request $request)
    {
        $product_total_items = 0;
        $product_grand_total = 0;

        $product_loss_controller = new TradeProductLossController();
...
//            }
        ...
        $voucher_code = config('global_variables.TRADE_PRODUCT_RECOVER_VOUCHER_CODE');
        $transaction_type = config('global_variables.PRODUCT_RECOVER');

        $sale_notes = 'TRADE_PRODUCT_RECOVER_SR';
//        $sale_voucher_code = config('global_variables.SALE_VOUCHER_CODE');
        $sale_voucher_code = config('global_variables.TRADE_PRODUCT_RECOVER_VOUCHER_CODE');
...
//            return redirect()->back()->with('success', 'Successfully Saved');
            return redirect()->back()->with(['pr_id' => $product_loss_id]);
        }
    }

    // update code by shahzaib start
    public function trade_product_recover_list(Request $request, $array = null, $str = null)
    {
...
//        $search_account = $request->account;
        ...
        $pge_title = 'Trade Product Recover List';
        $srch_fltr = [];
...
//        }
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_product_recover_list', compact('datas', 'search', 'product', 'search_products', 'search_product', 'search_to', 'search_from', 'search_by_user'));
        }
