...
class TradeClaimStockIssueToPartyController extends Controller
{
    public function add_trade_claim_stock_issue_to_party()
    {
        ...
        return view('Trade_Invoices/add_trade_claim_stock_issue_to_party', compact('pro_code', 'pro_name'));

    }


    public function submit_trade_claim_stock_issue_to_party(Request $request)
    {
        $this->validate($request, [
            'party_claim_account' => ['required', 'string'],
            'purchase_invoice' => ['nullable', 'string'],
            'sale_return_invoice' => ['nullable', 'string'],
            'remarks' => ['required'],
...
//            "total_price" => ['required', 'numeric'],
        ...
        foreach ($requested_arrays as $index => $requested_array) {
            ...
            $warehouse=WarehouseModel::where('wh_id','=',$item_warehouse)->pluck('wh_title')->first();
            $item_warehouse_name = $warehouse;
            ...
            if ($request->product_or_service_status[$index] == 0) {
                ...
                $products[] = [
                    ...
                    'product_uom' => $item_unit_measurement,
                    'product_scale_size' => $item_unit_measurement_scale_size, //mustafa
...
//        $products = json_decode($request->products_array, true);
        ...
        $data = $this->AssignWarehouseStocksValues([], $products, 2);
        ...
        $warehouses_summary = [];
        $warehouse_stock_summary = $this->AssignWarehouseStocksSummaryValues($warehouses_summary, $products, 'CLAIM ISSUE');
        ...
        foreach ($products as $product) {
            ...
            $stock_movement_last_entry = StockMovementModels::where('sm_product_code', '=', $product['product_code'])->where('sm_warehouse_id', '=', $product['warehouse'])->orderBy('sm_id', 'desc')
                ->first();
        ...
        $claim_issue = config('global_variables.claim_issue');
        $party_claim_account = $request->party_claim_account;

        $transactions1 = new TransactionModel();
        $transaction1 = $this->AssignTransactionsValues($transactions1, 0, $amount, $stock_in_hand, '', '', 0);
        ...
        if ($transaction1->save()) {
            ...
            $balance1 = $this->AssignAccountBalancesValues($balances1, $transaction_id1, $stock_in_hand, $amount, 'Cr', $request->remarks,
                'CLAIM_ISSUE', '', '', $request->posting_reference);
        ...
        $transactions1 = new TransactionModel();
        $transaction1 = $this->AssignTransactionsValues($transactions1, 0, $amount, $claim_issue, '', '', 0);
        ...
        if ($transaction1->save()) {
            ...
            $balance1 = $this->AssignAccountBalancesValues($balances1, $transaction_id1, $claim_issue, $amount, 'Cr', $request->remarks,
                'CLAIM_ISSUE', '', '', $request->posting_reference);
        ...
        $transactions1 = new TransactionModel();
        $transaction1 = $this->AssignTransactionsValues($transactions1, $party_claim_account, $amount, 0, '', '', 0);
        ...
        if ($transaction1->save()) {
            ...
            $balance1 = $this->AssignAccountBalancesValues($balances1, $transaction_id1, $party_claim_account, $amount, 'Dr', $request->remarks,
                'CLAIM_ISSUE', '', '', $request->posting_reference);
    ...
    private function assignStockMovementValues(Request $request, $product, $user, $day_end, $stock_movement_last_entry)
    {
        ...
        $stock_movement->sm_out_qty = $product['product_qty'];
        $stock_movement->sm_out_bonus = null;
        $stock_movement->sm_out_rate = $product['product_rate'];
        $stock_movement->sm_out_total = $product['product_amount'];

        $product_quantity = -1 * abs($product['product_qty']);

        $stock_movement->sm_internal_hold = null;
        $stock_movement->sm_internal_bonus = null;
        $stock_movement->sm_internal_claim = $product_quantity;

        $stock_movement->sm_bal_qty_for_sale = $stock_movement_last_entry->sm_bal_qty_for_sale - $product['product_qty'] - $product_quantity;
        ...
        $sm_bal_rate_dividend = $stock_movement_last_entry->sm_bal_total_qty_wo_bonus - $stock_movement->sm_out_qty;
        $stock_movement->sm_bal_rate = ($stock_movement_last_entry->sm_bal_total - $stock_movement->sm_out_total) / ($sm_bal_rate_dividend == 0 ? 1 : $sm_bal_rate_dividend);
    ...
    private function assign_party_claim_values($request, $user, $day_end)
    {
        ...
        $partyClaim->pc_type = 'CLAIM ISSUE';
        $partyClaim->pc_party_claim_account = $request->party_claim_account;
        $partyClaim->pc_purchase_invoice_num = $request->purchase_invoice;
        $partyClaim->pc_pr_id = $request->posting_reference;
        $partyClaim->pc_sale_return_invoice_num = $request->sale_return_invoice;
//        $partyClaim->pc_claim_issue_voucher_num = '';
    ...
    public function trade_claim_stock_issue_to_party_list(Request $request, $array = null, $str = null)
    {
        ...
        $prnt_page_dir = 'print.party_claim.issue.claim_stock_issue';
        $pge_title = 'Trade Claim Stock Issued';
        $srch_fltr = [];
        ...
        if (!empty($search) || !empty($search_to) || !empty($search_from)) {
            ...
            $partyClaims = PartyClaimModel::orderBy('pc_id', 'ASC')->where('pc_type', '=', 'CLAIM ISSUE');
        }
        ...
        if (isset($request->array) && !empty($request->array)) {
            return $this->printable($request, $partyClaims, $srch_fltr, $prnt_page_dir, $pge_title,$ttl_amount);
        } else {
            return view('Trade_Invoices/trade_claim_stock_issue_to_party_list', compact('partyClaims', 'ttl_amount','search', 'search_to', 'search_from'));
        }
    }

    private function search(array $searchData)
    {
        $query = PartyClaimModel::orderBy('pc_id', 'DESC')->where('pc_type', '=', 'CLAIM ISSUE');
    ...
    private function printable(Request $request, $partyClaims, $srch_fltr, $prnt_page_dir, $pge_title,$ttl_amount)
    {
        ...
        $pdf->loadView($prnt_page_dir, compact('partyClaims', 'type', 'pge_title','ttl_amount'));
    ...
    public function trade_claim_stock_issue_to_party_items_view_details_SH(Request $request, $id)
    {
        ...
        $type = 'grid';
        $pge_title = 'Trade Claim Stock Issue Items';

        return view('trade_voucher_view.trade_party_claim.issue.trade_party_claim_issue_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'invoice_remarks'));
    }

    public function trade_claim_stock_issue_to_party_items_view_details_pdf_SH(Request $request, $id)
    {
        $jrnl = PartyClaimModel::where('pc_id', $id)->first();
        $items = PartyClaimItemModel::where('pci_pc_id', $id)->get();
        $nbrOfWrds = $this->myCnvrtNbr($jrnl->pc_total_amount);
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('trade_voucher_view.trade_party_claim.issue.trade_party_claim_issue_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'type','invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);

        return $pdf->stream('Party-Claim-Issue-Detail.pdf');
    }
}
