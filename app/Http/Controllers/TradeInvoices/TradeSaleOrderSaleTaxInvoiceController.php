...
class TradeSaleOrderSaleTaxInvoiceController extends Controller
{
    public $actual_stock_price = 0;
    public $product_total_rate = 0;


    public function trade_sale_order_sale_tax_invoice()
    {
...
//        $sale_persons = User::where('user_id', '!=', 1)->orderBy('user_role_id', 'DESC')->orderBy('user_name', 'ASC')->get();
        ...
        return view('Trade_Invoices/trade_sale_order_sale_tax_invoice', compact('pro_code', 'pro_name', 'accounts', 'products', 'packages'));

    }

    public function submit_trade_sale_order_sale_tax_invoice(Request $request)
    {

        $this->delivery_order_sale_invoice_validation($request);
        $cash_discount_per = $request->disc_percentage;
        $order_type = $request->order_sale_type; // delclear the sale order invoice and sale order delivery order sale invoice
...
//        }
        ...
        if ($invoice_type == 1) {
            ...
            $delivery_qty_hold_table = 'financials_delivery_order_qty_hold_log';
            ...
            if ($order_type == 0) {
                $voucher_code = config('global_variables.TRADE_SALE_ORDER_SALE_VOUCHER_CODE');
            } else {
                $voucher_code = config('global_variables.TRADE_SALE_ORDER_DELIVERY_ORDER_SALE_VOUCHER_CODE');
            }
            ...
            if ($order_type == 0) {
                $voucher_code = config('global_variables.TRADE_SALE_ORDER_SALE_TAX_VOUCHER_CODE');
            } else {
                $voucher_code = config('global_variables.TRADE_SALE_ORDER_DELIVERY_ORDER_SALE_TAX_VOUCHER_CODE');
            }
//            $voucher_code = config('global_variables.TRADE_SALE_ORDER_SALE_TAX_VOUCHER_CODE');
...
//            });
            ...
            if ($order_type == 0) {
                $item_qty_hold = $this->AssignSaleOrderSaleQtyHoldValues($sale_invoice_id, $delivery_order, $invoice_type, $sales_array, $sale_qty_hold_prefix, 1);
                ...
                if (!DB::table($delivery_qty_hold_table)->insert($item_qty_hold)) {
                    $rollBack = true;
                    DB::rollBack();
                    return redirect()->back()->with('fail', 'Failed Try Again');
                }
...
//            }
            ...
            if ($order_type == 0) {
                if ($invoice_type == 1) {
                    $invoice_type_summary = 'SALE-ORDER-SALE';
                } else {
                    $invoice_type_summary = 'SALE-ORDER-SALE-TAX';
                }
            } else {
                if ($invoice_type == 1) {
                    $invoice_type_summary = 'SALE-ORDER-DELIVERY-ORDER-SALE';
                } else {
                    $invoice_type_summary = 'SALE-ORDER-DELIVERY-ORDER-SALE-TAX';
                }
            }
            ...
            $invoice_type_movement = '';
            if ($order_type == 0) {
                if ($invoice_type == 1) {
                    $invoice_type_movement = 'SALE-ORDER-SALE-INVOICE';
                } else {
                    $invoice_type_movement = 'SALE-ORDER-SALE-TAX-INVOICE';
                }
            } else {
                if ($invoice_type == 1) {
                    $invoice_type_movement = 'SALE-ORDER-DELIVERY-ORDER-SALE-INVOICE';
                } else {
                    $invoice_type_movement = 'SALE-ORDER-DELIVERY-ORDER-SALE-TAX-INVOICE';
                }
            }

            $stock_movement = $this->stock_movement_module_delivery_order_sale($sales_array, $voucher_code . $sale_invoice_id, $invoice_type_movement, $invoice_type_movement);
            ...
            $stock_movement_child = $this->stock_movement_child($sales_array, $sale_invoice_id, $account_code, $account_name, $invoice_type_movement);
...
//////////////////////////// Details Remarks of service Invoice Insertion ////////////////////////////////////
            ...
            if ($order_type == 0) {
                $item_qty_hold = $this->AssignSaleOrderSaleQtyHoldValues($sale_invoice_id, $delivery_order, $invoice_type, $services_array, $sale_qty_hold_prefix, 2);
                if (!DB::table($qty_hold_table)->insert($item_qty_hold)) {
                    $rollBack = true;
                    DB::rollBack();
                    return redirect()->back()->with('fail', 'Failed Try Again');
                ...
                $item_qty_hold = $this->AssignDeliveryOrderSaleQtyHoldValues($sale_invoice_id, $delivery_order, $invoice_type, $services_array, $delivery_qty_hold_prefix, 2);
                if (!DB::table($delivery_qty_hold_table)->insert($item_qty_hold)) {
                    $rollBack = true;
                    DB::rollBack();
                    return redirect()->back()->with('fail', 'Failed Try Again');
...
//        if credit card machine selected
        ...
        if ($order_type == 0) {
            foreach ($sales_array as $val) {
                ...
                if ($invoice_type == 1) {
                    if ($sale_order->soi_status != null || $sale_order->soi_status != 0) {
                        ...
                        $sale_order->soi_status = $id . ',' . 'SI-' . $sale_invoice_id;
                    } else {
                        $sale_order->soi_status = 'SI-' . $sale_invoice_id;
                    }
                } else {
                    if ($sale_order->soi_status != null || $sale_order->soi_status != 0) {
                        $id = $sale_order->soi_status;
                        $sale_order->soi_status = $id . ',' . 'STSI-' . $sale_invoice_id;
                    } else {
                        $sale_order->soi_status = 'STSI-' . $sale_invoice_id;
                    }
                }
            ...
            foreach ($services_array as $val) {
                ...
                $product_code = $val['service_code'];
                $qty = $val['service_qty'];
                $sale_order = SaleOrderItemsModel::where('soi_product_code', $product_code)->where('soi_invoice_id', '=', $val['delivery_order_id'])
                    ->first();
                if ($invoice_type == 1) {
                    if ($sale_order->soi_status != null || $sale_order->soi_status != 0) {
                        $id = $sale_order->soi_status;
            ...
            foreach ($services_array as $val) {
                ...
                $product_code = $val['service_code'];
                $qty = $val['service_qty'];
                $delivery_order = DeliveryOrderItemsModel::where('doi_product_code', $product_code)->where('doi_invoice_id', '=', $val['delivery_order_id'])->first();
        ...
        if ($rollBack) {
            ...
            DB::commit();
            return redirect()->back()->with(['dosti_id' => $sale_invoice_id]);
//            return redirect()->back()->with(['si_id' => $sale_invoice_id, 'success' => 'Successfully Saved']);
    ...
    {//status 1 for products and 2 for services
        ...
        $sale_invoice->$col_delivery_order_id = 'SO-' . implode(',', $do_ids);
    ...
    public
    function AssignSaleOrderSaleQtyHoldValues($invoice_id, $delivery_order_id, $invoice_type, $array, $prfx, $status)
    {//status 1 for products and 2 for services
        ...
        if ($status == 1) {
            ...
            foreach ($array as $value) {
                ...
                $qty = SaleOrderQuantityHoldModel::where('soqh_product_code', $value['service_code'])->orderBy('soqh_id', 'DESC')->first();
                ...
                $data[] = [
                    ...
                    $product_code => $value['service_code'],
//                    $warehouse => $value['warehouse'],
                    $sale_qty => $value['service_qty'],
                    $balance_qty => $qty->soqh_balance_qty - $value['service_qty'],
    ...
    {//status 1 for products and 2 for services
        ...
        if ($status == 1) {
            ...
            foreach ($array as $value) {
                ...
                $qty = DeliveryOrderQuantityHoldModel::where('doqh_product_code', $value['service_code'])->orderBy('doqh_id', 'DESC')->first();
...
//                    $warehouse => $value['warehouse'],
                    $sale_qty => $value['service_qty'],
                    $balance_qty => $qty->doqh_balance_qty - $value['service_qty'],
    ...
    public function trade_sale_order_delivery_order_sale_tax_invoice()
    {
        $accounts = $this->get_account_query('sale')[0];
...
//        $sale_persons = User::where('user_id', '!=', 1)->orderBy('user_role_id', 'DESC')->orderBy('user_name', 'ASC')->get();
        ...
        return view('Trade_Invoices/trade_sale_order_delivery_order_sale_tax_invoice', compact('pro_code', 'pro_name', 'accounts', 'products', 'packages', 'warehouses'));
