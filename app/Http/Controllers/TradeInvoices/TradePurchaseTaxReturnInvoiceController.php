...
class TradePurchaseTaxReturnInvoiceController extends Controller
{
    public function trade_purchase_tax_return_invoice()
    {
//        $user = Auth::user();
...
//        }
//        $query = AccountRegisterationModel::where('account_delete_status', '!=', 1)->where('account_disabled', '!=', 1)->whereIn('account_parent_code', $heads)
//            ->whereNotIn(
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Trade_Invoices/trade_purchase_sale_tax_return_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_purchase_tax_return_invoice(Request $request)
    {
...
//            return redirect()->back()->with('success', 'Successfully Saved');
    ...
    public function trade_sale_tax_purchase_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Sale Tax Purchase Return Invoice List';
        $srch_fltr = [];
...
//            $query->whereBetween('pi_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_purchase_sale_tax_return_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_name', 'pro_code', 'search_to', 'search_from',
    ...
    public function trade_return_sale_tax_purchase_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0){
            ...
            $pim = DB::table('financials_purchase_return_saletax_invoice')
                ...
                ->where('prsi_id', $id)
                ->select('financials_accounts.account_urdu_name as prsi_party_name','prsi_id','prsi_party_code','prsi_customer_name','prsi_remarks','prsi_total_items','prsi_total_price','prsi_product_disc','prsi_round_off_disc',
                    'prsi_cash_disc_per','prsi_cash_disc_amount','prsi_total_discount','prsi_inclusive_sales_tax','prsi_exclusive_sales_tax','prsi_total_sales_tax','prsi_grand_total','prsi_cash_paid','prsi_day_end_id','prsi_day_end_date','prsi_createdby','financials_posting_reference.pr_name')->first();
        }

        $accnts = AccountRegisterationModel::where('account_uid', $pim->prsi_party_code)->select('account_name')->first();
        if ($urdu_eng->rc_invoice == 0) {
            $piims = PurchaseReturnSaletaxInvoiceItemsModel::where('prsii_purchase_invoice_id', $id)
                ->select('prsii_product_name as name', 'prsii_remarks as remarks', 'prsii_qty as qty', 'prsii_rate as rate', 'prsii_discount_per as discount', 'prsii_discount_amount as discount_amount',
                    'prsii_after_dis_rate as after_discount', 'prsii_saletax_per as sale_tax', 'prsii_saletax_amount as sale_tax_amount', 'prsii_saletax_inclusive as inclu_exclu', 'prsii_amount as amount', 'prsii_scale_size as scale_size','prsii_uom as uom')
        ...
        $type = 'grid';
        $pge_title = 'Trade Sale Tax Purchase Return Invoice';

        return view('trade_invoice_view.trade_purchase_return_sale_tax_invoice.trade_purchase_return_sale_tax_invoice_list_modal', compact('piims', 'pim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date',
            'type', 'pge_title', 'urdu_eng'));


    }

    public function trade_return_sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnSaletaxInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_saletax_invoice.prsi_pr_id')->where('prsi_id', $id)->first();
        ...
        $pdf->loadView('trade_invoice_view.trade_purchase_return_sale_tax_invoice.trade_purchase_return_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr',
            'invoice_date', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);
