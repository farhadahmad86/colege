...
class TradeProductPackagesController extends Controller
{
    public function trade_product_packages()
    {
        ...
        return view('Trade_Invoices/trade_product_packages', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_product_packages(Request $request)
    {
...
//            $product_grand_total += $item_amount;
        ...
        foreach ($sales_array as $key) {

            $data[] = ['ppi_pro_pack_id' => $product_packages_id, 'ppi_product_code' => $key['product_code'], 'ppi_product_name' => ucwords($key['product_name']), 'ppi_remarks' => ucfirst($key['product_remarks']), 'ppi_qty' => $key['product_qty'], 'ppi_scale_size' => $key['product_unit_scale_size'], 'ppi_uom' => $key['product_unit_measurement'], 'ppi_rate' => $key['product_rate'], 'ppi_amount' => $key['product_amount'], 'ppi_ip_adrs' => $ip, 'ppi_brwsr_info' => $browser, 'ppi_update_datetime' => $current_date_time];
        }
    ...
    public function trade_product_packages_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Product Package List';
        $srch_fltr = [];
...
//            ->where('pp_delete_status', '!=', 1)
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_product_packages_list', compact('datas', 'search', 'packages_name', 'search_by_user', 'search_to', 'search_from', 'search_product', 'restore_list'));
        }
    ...
    public function edit_trade_product_packages(Request $request)
    {
        $products = $this->get_all_products();
        ...
        return view('Trade_Invoices/edit_trade_product_packages', compact('product_package', 'pro_code', 'pro_name'));
    }

    public function update_trade_product_packages(Request $request)
    {
        $this->validation($request);
    ...
    public function delete_trade_product_packages(Request $request)
    {
        $this->validate($request, [
...
//        $delete->pp_delete_status = 1;
    ...
    public function get_trade_product_packages_details(Request $request)
    {
