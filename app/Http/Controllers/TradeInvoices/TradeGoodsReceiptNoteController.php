...
class TradeGoodsReceiptNoteController extends Controller
{
    public function trade_goods_receipt_note()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled', '=', 0)->get();
        ...
        return view('Trade_Invoices/trade_goods_receipt_note', compact('po', 'sale_persons', 'machines', 'service_code', 'service_name', 'packages'));
    }

    public function submit_trade_goods_receipt_note(Request $request)
    {
//        dd($request);
...
//            });
            ...
            foreach ($item as $value) {
                ...
                $qty = $value[$grn_items_prefix . '_qty'];
                $scale_size = (float)$value[$grn_items_prefix . '_scale_size'];
                $pack_qty = floor($qty / $scale_size);
                $loose_qty = fmod($qty, $scale_size);

                $detail_remarks .= $value[$grn_items_prefix . '_product_name'] . ', Total QTY = ' . $value[$grn_items_prefix . '_qty'] . ', Pack QTY = ' . $pack_qty . ', Loose QTY = ' .
...
//            return redirect()->back()->with('success', 'Successfully Saved');
            return redirect()->back()->with(['grni_id' => $goods_receipt_note_id]);
        }
    }
...
// update code by mustafa start
    public
    function trade_goods_receipt_note_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Goods Receipt Note List';
        $srch_fltr = [];
...
//            $query->whereBetween('grn_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_goods_receipt_note_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user'));
...
// update code by mustafa end
    ...
    public
    function trade_goods_receipt_note_items_view_details(Request $request)
    {
    ...
    public
    function trade_goods_receipt_note_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice == 0) {
            $siims = DB::table('financials_goods_receipt_note_items')
                ...
                ->select('grni_product_name as name', 'grni_remarks as remarks', 'grni_qty as qty', 'grni_scale_size as scale_size', 'grni_due_qty as due_qty', 'grni_bonus_qty as bonus', 'grni_status as status')
                ->get();
        } else {
            ...
            $siims = DB::table('financials_goods_receipt_note_items')
                ...
                ->where('grni_invoice_id', $id)
                ->select('financials_products.pro_urdu_title as name', 'grni_remarks as remarks', 'grni_qty as qty', 'grni_scale_size as scale_size', 'grni_due_qty as due_qty', 'grni_bonus_qty as bonus', 'grni_status as status')
                ->orderby('grni_product_name', 'ASC')
                ->get();
...
//        $nbrOfWrds = $this->myCnvrtNbr($mainGrndTtl);
        ...
        $type = 'grid';
        $pge_title = 'Trade Goods Receipt Note';

        return view('trade_invoice_view.trade_goods_receipt_note.trade_goods_receipt_note_list_modal', compact('siims', 'sim', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title'));
    }
    ...
    function trade_goods_receipt_note_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('trade_invoice_view.trade_goods_receipt_note.trade_goods_receipt_note_list_modal', compact('siims', 'sim', 'accnts', 'type', 'pge_title','invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);

        return $pdf->stream('Trade-Goods-Receipt-Note.pdf');
    }
