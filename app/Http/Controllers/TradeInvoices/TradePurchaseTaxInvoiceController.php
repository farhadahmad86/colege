...
class TradePurchaseTaxInvoiceController extends Controller
{
    public $invoice_total_value = 0;

    public function trade_purchase_tax_invoice()
    {
//        $user = Auth::user();
...
//        $posting_references = PostingReferenceModel::where('pr_disabled', '=', 0)->get();
        return view('Trade_Invoices/trade_purchase_sale_tax_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_trade_purchase_tax_invoice(Request $request)
    {
...
//            return redirect()->back()->with('success', 'Successfully Saved');
            return redirect()->back()->with(['pti_id' => $purchase_invoice_id]);
        }
    }
    ...
    public function trade_sale_tax_purchase_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Sale Tax Purchase Invoice List';
        $srch_fltr = [];
...
//            $query->whereBetween('pi_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_purchase_sale_tax_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_name', 'pro_code', 'search_to', 'search_from', 'search_by_user','posting_references','search_posting_reference'));
        }
    }

    // update code by shahzaib end

    public function trade_sale_tax_purchase_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0) {
            ...
            $pim = DB::table('financials_purchase_saletax_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as psi_party_name', 'psi_id', 'psi_party_code', 'psi_customer_name', 'psi_remarks', 'psi_total_items', 'psi_total_price',
                    ...
                    'psi_cash_disc_per', 'psi_cash_disc_amount', 'psi_total_discount', 'psi_inclusive_sales_tax', 'psi_exclusive_sales_tax', 'psi_total_sales_tax', 'psi_grand_total',
                    'psi_cash_paid', 'psi_day_end_id', 'psi_day_end_date', 'psi_createdby', 'financials_posting_reference.pr_name')->first();
        }
        ...
        if ($urdu_eng->rc_invoice == 0) {
            $piims = PurchaseSaletaxItemsInvoiceModel::where('psii_purchase_invoice_id', $id)
                ->select('psii_product_name as name', 'psii_remarks as remarks', 'psii_qty as qty', 'psii_rate as rate', 'psii_discount_per as discount', 'psii_discount_amount as discount_amount', 'psii_after_dis_rate as after_discount', 'psii_saletax_per as sale_tax', 'psii_saletax_amount as sale_tax_amount', 'psii_saletax_inclusive as inclu_exclu', 'psii_amount as amount', 'psii_scale_size as scale_size','psii_uom as uom')
...
//                ->join('financials_products','financials_products.pro_p_code','=','financials_purchase_saletax_invoice_items.psii_product_code')
    ...
    public function trade_sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            $pim = PurchaseSaletaxInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_saletax_invoice.psi_pr_id')->where('psi_id', $id)->first();
...
//                ->join('financials_products','financials_products.pro_p_code','=','financials_purchase_saletax_invoice_items.psii_product_code')
        ...
        $pdf->loadView('trade_invoice_view.trade_purchase_sale_tax_invoice.trade_purchase_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);
