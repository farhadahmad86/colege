...
class TradeSaleReturnInvoiceController extends Controller
{
    public function sale_return_invoice()
    {
        ...
        return view('Trade_Invoices/trade_sale_return_invoice', compact('pro_code', 'pro_name', 'accounts', 'products', 'packages', 'detail_remarks'));
    }
    ...
    public function submit_trade_sale_return_invoice(Request $request)
    {
...
//            return redirect()->back()->with('success', 'Successfully Saved');
    ...
    public function trade_sale_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Trade Sale Return Invoice List';
        $srch_fltr = [];
...
//            $query->whereBetween('si_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('Trade_Invoices/trade_sale_return_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'urdu_eng', 'posting_references', 'search_posting_reference'));
        }
    }

    public function trade_sale_return_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
...
//            ->select('srii_product_name as name', 'srii_remarks as remarks', 'srii_qty as qty', 'srii_rate as rate', 'srii_discount as discount', 'srii_saletax as sale_tax', 'srii_amount as amount')
        ...
        $type = 'grid';
        $pge_title = 'Trade Sale Return Invoice';

        return view('trade_invoice_view.trade_sale_return_invoice.trade_sale_return_invoice_list_modal', compact('siims', 'sim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng'));
    }

    public function trade_sale_return_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $sim = SaleReturnInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_return_invoice.sri_pr_id')->where('sri_id', $id)->first();
...
//            ->select('srii_product_name as name', 'srii_remarks as remarks', 'srii_qty as qty', 'srii_rate as rate', 'srii_discount as discount', 'srii_saletax as sale_tax', 'srii_amount as amount')
        ...
        $pdf->loadView('trade_invoice_view.trade_sale_return_invoice.trade_sale_return_invoice_list_modal', compact('siims', 'sim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);
