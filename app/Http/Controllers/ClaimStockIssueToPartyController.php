...
class ClaimStockIssueToPartyController extends Controller
{
    public function add_claim_stock_issue_to_party()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('add_claim_stock_issue_to_party', compact('pro_code', 'pro_name', 'warehouses'));

    }

    public function submit_claim_stock_issue_to_party(Request $request)
    {
...
//dd($stock_movement->sm_bal_qty_for_sale,$stock_movement_last_entry->sm_bal_qty_for_sale,$product['product_qty'], $product_quantity);
...
//        $partyClaim->pc_claim_issue_voucher_num = '';
    ...
    public function claim_stock_issue_to_party_list(Request $request, $array = null, $str = null)
    {
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            $pdf->loadView($prnt_page_dir, compact('partyClaims', 'ttl_amount', 'type', 'pge_title'));
            ...
            if ($type === 'pdf') {
                ...
                return Excel::download(new ExcelFileCusExport($partyClaims, $ttl_amount, $srch_fltr, $type, $prnt_page_dir, $pge_title), $pge_title . '_x.xlsx');
            }
...
//        } else {
            return view('claim_stock_issue_to_party_list', compact('partyClaims', 'ttl_amount', 'search','search_to', 'search_from'));
        }
    }
    ...
    private function printable(Request $request, $partyClaims, $srch_fltr, $prnt_page_dir, $pge_title)
    {
        ...
        $pdf = PDF::loadView($prnt_page_dir, compact('partyClaims', 'type', 'pge_title'));
        $pdf->setOptions($options);
    ...
    public function claim_stock_issue_to_party_items_view_details_SH(Request $request, $id)
    {
        ...
        $type = 'grid';
        $pge_title = 'Claim Stock Issue Items';

        return view('voucher_view.party_claim.issue.party_claim_issue_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'invoice_remarks'));
    }

    public function claim_stock_issue_to_party_items_view_details_pdf_SH(Request $request, $id)
    {
        $jrnl = PartyClaimModel::where('pc_id', $id)->first();
        $items = PartyClaimItemModel::where('pci_pc_id', $id)->get();
        $nbrOfWrds = $this->myCnvrtNbr($jrnl->pc_total_amount);
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('voucher_view.party_claim.issue.party_claim_issue_list_modal', compact('items', 'jrnl', 'nbrOfWrds', 'type', 'invoice_nbr', 'invoice_date','pge_title'));
        // $pdf->setOptions($options);
