...
class ConvertQuantityController extends Controller
{
    public function show_convert_quantity()
    {
        // $warehouses = \App\Models\WarehouseModel::orderBy('wh_title', 'ASC')->get();
//        $products = \App\Models\ProductModel::orderBy('pro_title', 'ASC')->get();
        ...
        return view('_ab.convert_quantity', compact('products'));
    }

    public function submit_convert_quantity(Request $request)
    {
    ...
    private function assignStockMovementConditionsBasedValues(Request $request, StockMovementModels $stock_movement, StockMovementModels $stock_movement_last_entry)
    {
    ...
    public function convert_quantity_list(Request $request)
    {
        ...
        $search_product_title = (!isset($request->product_title) && empty($request->product_title)) ? ((!empty($ar)) ? $ar[3]->{'value'} : '') : $request->product_title;
        ...
        $prnt_page_dir = 'print.convert_quantity.convert_quantity';
        $pge_title = 'Convert Quantity';
        $srch_fltr = [];
...
//        }
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('_ab.convert_quantity_list', compact('convert_quantities', 'products', 'searchData','search_product_code','search_product_title'));
        }
    }
...
//            $query = $query->orWhere('cq_pro_title', '=', $searchData['product_title']);
//        }
