...
class SimpleSaleTaxInvoiceController extends Controller
{
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Simple_Invoices/simple_sale_tax_invoice', compact(  'sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'detail_remarks'));
    }

    public function submit_simple_sale_tax_invoice(Request $request)
    {
...
//                    $after_dis_rate => $value['product_rate'] - ($value['product_discount_amount'] / $value['product_qty']),//hamza
    ...
    // update code by shahzaib start
    public
    function simple_sale_tax_sale_invoice_list(Request $request, $array = null, $str = null)
    {
...
// update code by shahzaib end
    ...
    public
    function simple_sale_sale_tax_items_view_details_SH(Request $request, $id)
    {
...
//            ->select('ssii_product_name as name', 'ssii_remarks as remarks', 'ssii_qty as qty', 'ssii_rate as rate', 'ssii_discount as discount', 'ssii_saletax as sale_tax', 'ssii_amount as amount')
        ...
        return view('invoice_view.sale_sale_tax_invoice.simple_sale_sale_tax_invoice_list_modal', compact('siims', 'sim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'seim',
            'cash_received', 'urdu_eng'));
    }


    public
    function simple_sale_sale_tax_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            $sim = SaleSaletaxInvoiceModel::leftJoin('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_saletax_invoice.ssi_pr_id')->where('ssi_id', $id)->first();
...
//        $invoice_date = $sim->ssi_day_end_date;
        ...
        $pdf = PDF::loadView('invoice_view.sale_sale_tax_invoice.sale_sale_tax_invoice_list_modal', compact('siims', 'sim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'seim',
            'cash_received', 'urdu_eng'));
        $pdf->setOptions($options);
