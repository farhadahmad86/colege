...
class SimplePurchaseReturnInvoiceController extends Controller
{
    public function simple_purchase_return_invoice()
    {
...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('Simple_Invoices/simple_purchase_return_invoice', compact('pro_code', 'pro_name'));
    }

    public function submit_simple_purchase_return_invoice(Request $request)
    {
...
//            return redirect()->back()->with('success', 'Successfully Saved');
    ...
    public function simple_purchase_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Purchase Return Invoice List';
        $srch_fltr = [];
...
//        $query = PurchaseInvoiceModel::query();
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('purchase_return_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_name', 'pro_code', 'search_to', 'search_from', 'posting_references', 'search_posting_reference'));
        }

    }
    // update code by shahzaib end

    public function simple_purchase_return_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0){
            ...
            $pim = DB::table('financials_purchase_return_invoice')
                ...
                ->where('pri_id', $id)
                ->select('financials_accounts.account_urdu_name as pri_party_name','pri_id','pri_party_code','pri_customer_name','pri_remarks','pri_total_items','pri_total_price','pri_product_disc','pri_round_off_disc',
                    'pri_cash_disc_per','pri_cash_disc_amount','pri_total_discount','pri_inclusive_sales_tax','pri_exclusive_sales_tax','pri_total_sales_tax','pri_grand_total','pri_cash_paid','pri_day_end_id','pri_day_end_date','pri_createdby','pr_name')->first();
        }
    ...
    public function simple_purchase_return_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $pim = PurchaseReturnInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_purchase_return_invoice.pri_pr_id')->where('pri_id', $id)->first();
        ...
        $pdf = PDF::loadView('invoice_view.purchase_return_invoice.purchase_return_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'urdu_eng'));
        $pdf->setOptions($options);

        return $pdf->stream('Purchase-Return-Invoice.pdf');
    }
