...
class SimpleSaleInvoiceController extends Controller
{
...
//        return view('Simple_Invoices/practice', compact('accounts', 'sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'warehouses'));
    }

    public function submit_simple_sale_invoice(Request $request)
    {
...
//            $item_quantity $item_unit_measurement_scale_size
            ...
            $item_simple_offer = $request->trade_offer[$index];
...
//                $product_total_price = round($product_total_price + ($item_rate * $item_quantity), 2); // hamza 1
                ...
                $total_simple_offer_amount += $item_simple_offer;
...
//        $product_grand_total = ($product_total_price - $product_total_discount_amount - $total_simple_offer) + $product_exclusive_saletax;  // hamza

        $product_grand_total = ($product_total_price - $product_total_discount_amount - $total_simple_offer_amount) + $product_exclusive_saletax; // mustafa
...
//            $service_round_off_discount_amount = 0;
            ...
            $product_round_off_discount_amount = round($product_grand_total - round($product_grand_total), 2); //hamza
            $service_round_off_discount_amount = round($service_grand_total - round($service_grand_total), 2); // hamza
...
//        }
        ...
        $sale_amount = $product_grand_total - $product_total_saletax + $product_total_discount_amount + $total_simple_offer_amount;
        ...
        $simple_offer_account = config('global_variables.simple_offer_account');
...
//                        $notes, $detail_remarks, $voucher_code . $sale_invoice_id);  // hamza
            ...
            if ($total_simple_offer_amount != 0) {

                //////////////////////////// TRANSACTION EIGHT TRADE OFFER ////////////////////////////////////

                $transactions8 = new TransactionModel();
                $transaction8 = $this->AssignTransactionsValues($transactions8, $simple_offer_account, $total_simple_offer_amount, 0, $notes, $transaction_type, $sale_invoice_id);
                ...
                if ($transaction8->save()) {
                    ...
                    $balance8 = $this->AssignAccountBalancesValues($balances8, $transaction_id8, $simple_offer_account, $total_simple_offer_amount, 'Dr', $request->remarks,
                        $notes, $detail_remarks, $voucher_code . $sale_invoice_id, $request->posting_reference);
...
//////////////////////////// TRANSACTION ONE SERVICE ACCOUNT ////////////////////////////////////
            ...
            if ($transaction1->save()) {
                ...
                $balance1 = $this->AssignAccountBalancesValues($balances1, $transaction_id1, $service_account, $service_amount, 'Cr', $request->remarks,
                    $service_notes, $detail_remarks, $service_voucher_code . $service_invoice_id, $request->posting_reference, $voucher_no='');
...
//            return redirect()->back()->with(['si_id'=>$sale_invoice_id, 'success'=>'Successfully Saved hahaha']);
    ...
    {//status 1 for products and 2 for services
        ...
        $col_clg_id = $prfx . '_clg_id';
        $col_branch_id = $prfx . '_branch_id';
...
//        $sale_invoice->$col_grand_total = $request->grand_total;
        ...
        $sale_invoice->$col_clg_id = $user->user_clg_id;
        $sale_invoice->$col_branch_id = Session::get('branch_id');
...
// update code by shahzaib start
    public
    function simple_sale_invoice_list(Request $request, $array = null, $str = null)
    {
...
// update code by shahzaib end
    public
    function simple_sale_items_view_details(Request $request)
    {
    ...
    public
    function simple_sale_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
            $sim = SaleInvoiceModel::join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_invoice.si_pr_id')->where('si_id', $id)->first();
        } else {
            $sim = DB::table('financials_sale_invoice')
                ...
                ->select('financials_accounts.account_urdu_name as si_party_name', 'si_id', 'si_party_code', 'si_customer_name', 'si_remarks', 'si_total_items', 'si_total_price',
                    ...
                    'si_cash_received', 'si_day_end_id', 'si_day_end_date', 'si_createdby', 'si_sale_person', 'si_service_invoice_id', 'si_local_invoice_id', 'si_local_service_invoice_id', 'si_cash_received_from_customer', 'si_return_amount','pr_name')->first();
        }
...
//            ->select('sseii_service_name as name', 'sseii_remarks as remarks', 'sseii_qty as qty', 'sseii_rate as rate', 'sseii_discount as discount', 'sseii_saletax as sale_tax', 'sseii_amount as amount')
        ...
        return view('invoice_view.sale_invoice.simple_sale_invoice_list_modal', compact('siims', 'sim', 'seim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'cash_received', 'urdu_eng'));
    }

    public
    function simple_sale_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0) {
...
//            ->select('sseii_service_name as name', 'sseii_remarks as remarks', 'sseii_qty as qty', 'sseii_rate as rate', 'sseii_discount as discount', 'sseii_saletax as sale_tax', 'sseii_amount as amount')
        ...
        $pdf = PDF::loadView('invoice_view.sale_invoice.simple_sale_invoice_list_modal', compact('siims', 'sim', 'seim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'cash_received', 'urdu_eng'));
        $pdf->setOptions($options);
