...
//        $posting_references = PostingReferenceModel::where('pr_disabled','=',0)->get();
        return view('product_loss', compact('accounts', 'pro_code', 'pro_name'));
    }

    public function submit_product_loss(Request $request)
    {
        ...
        $notes = 'PRODUCT_LOSS';
        $voucher_code = config('global_variables.PRODUCT_LOSS_VOUCHER_CODE');
...
//        $sale_voucher_code = config('global_variables.SALE_VOUCHER_CODE');
        $sale_voucher_code = config('global_variables.PRODUCT_LOSS_VOUCHER_CODE');//mustafa
        ...
        $sale_invoice = $this->AssignSaleInvoiceValues($sale_invoice, $day_end, $user, 'si', $request->total_items, $amount, $voucher_code . $product_loss_id, $this->detail_remarks,
...
//            $amount = $value['product_qty'] * $rate;
    ...
    public function product_loss_list(Request $request, $array = null, $str = null)
    {
//        $heads = config('global_variables.all_expense_accounts');
//        $heads = explode(',', $heads);
//
...
//        }
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('product_loss_list', compact('datas', 'search', 'product', 'search_products', 'search_product', 'search_to', 'search_from', 'search_by_user'));
        }
    ...
    public function product_loss_recover_items_view_details(Request $request)
    {
    ...
    public function product_loss_recover_items_view_details_SH(Request $request, $id)
    {
        ...
        $type = 'grid';
        $pge_title = 'Product ' . ucfirst(strtolower($pim->plr_status)) . ' Invoice';
    ...
    public function product_loss_recover_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.product_loss_invoice.product_loss_invoice_list_modal', compact('piims', 'pim', 'accnts', 'user', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'invoice_remarks', 'type', 'pge_title'));
        // $pdf->setOptions($options);
...
//    public function assign_product_balances_values($request, $data, $account_uid, $account_name, $invoice_id, $type, $notes)
//    {
//        $user = Auth::user();
//
//        $get_day_end = new DayEndController();
...
////        $products_array = stripslashes($products_array);
...
//
//            $pro_code = $key[0];
//            $pro_name = ucwords($key[1]);
//            $qty = $key[3];
//            $amount = $key[5];
//
//            $product_stock_balance = $purchase_invoice_controller->calculate_product_stock_balance($pro_code);
//            $product_amount_balance = $purchase_invoice_controller->calculate_product_amount_balance($pro_code);
//
//            if ($type == 'LO') {
//
...
//
//                $data[] = ['pb_p_code' => $pro_code, 'pb_p_name' => $pro_name, 'pb_acccount_uid' => $account_uid, 'pb_account_name' => $account_name, 'pb_s_inward' => 0, 'pb_s_outward' => $qty, 'pb_s_balance' => $product_stock_balance, 'pb_amount_inward' => 0, 'pb_amount_outward' => $amount, 'pb_amount_balance' => $product_amount_balance, 'pb_notes' => $notes, 'pb_voucher_number' => $type . '-' . $invoice_id, 'pb_createdby' => $user->user_id, 'pb_datetime' => Carbon::now()->toDateTimeString(), 'pb_dayend_id' => $day_end->de_id, 'pb_dayend_date' => $day_end->de_datetime];
//
//            } else {
//
//                $product_stock_balance += $qty;
//                $product_amount_balance += $amount;
//
//                $data[] = ['pb_p_code' => $pro_code, 'pb_p_name' => $pro_name, 'pb_acccount_uid' => $account_uid, 'pb_account_name' => $account_name, 'pb_s_inward' => $qty, 'pb_s_outward' => 0, 'pb_s_balance' => $product_stock_balance, 'pb_amount_inward' => $amount, 'pb_amount_outward' => 0, 'pb_amount_balance' => $product_amount_balance, 'pb_notes' => $notes, 'pb_voucher_number' => $type . '-' . $invoice_id, 'pb_createdby' => $user->user_id, 'pb_datetime' => Carbon::now()->toDateTimeString(), 'pb_dayend_id' => $day_end->de_id, 'pb_dayend_date' => $day_end->de_datetime];
//
//            }
//
//        }
