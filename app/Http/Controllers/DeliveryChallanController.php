...
//        $posting_references = PostingReferenceModel::where('pr_disabled', '=', 0)->get();
        ...
        return view('delivery_challan', compact('pro_code', 'pro_name', 'accounts', 'products', 'packages'));
...
//        return view('delivery_challan', compact('accounts', 'sale_persons', 'machines', 'service_code', 'service_name', 'packages', 'warehouses'));
    ...
    public function submit_delivery_challan(Request $request)
    {
//        dd($request);
...
//        return response()->json(['array'=>$array,'sale'=>$sale,'items'=>$items]);
    ...
    public
    function delivery_challan_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Delivery Challan List';
        $srch_fltr = [];
...
//            $query->whereBetween('do_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('delivery_challan_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user'));
        }
    }
...
// update code by shahzaib end
    ...
    public
    function delivery_challan_items_view_details(Request $request)
    {
    ...
    public
    function delivery_challan_items_view_details_SH(Request $request, $id)
    {
        ...
        $siims = DB::table('financials_delivery_challan_items')
            ...
            ->select('dci_product_name as name', 'dci_remarks as remarks', 'dci_qty as qty', 'dci_uom as uom', 'dci_bonus_qty as bonus'
...
//        $nbrOfWrds = $this->myCnvrtNbr($mainGrndTtl);
        ...
        $type = 'grid';
        $pge_title = 'Delivery Challan Invoice';

        return view('invoice_view.delivery_challan.delivery_challan_list_modal', compact('siims', 'sim', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title'));
    }

    public
    function delivery_challan_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.delivery_challan.delivery_challan_list_modal', compact('siims', 'sim', 'accnts', 'type', 'pge_title','invoice_nbr','invoice_date'));
        // $pdf->setOptions($options);

        return $pdf->stream('Deliver-Challan-Invoice.pdf');
    }
