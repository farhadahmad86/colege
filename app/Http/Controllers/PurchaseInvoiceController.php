...
//        }
//        //where('account_delete_status', '!=', 1)->where('account_disabled', '!=', 1)->
...
//        return view('purchase_invoice', compact('pro_code', 'pro_name', 'accounts', 'warehouses','posting_references'));
        return view('Simple_Invoices/simple_purchase_invoice', compact('pro_code', 'pro_name'));

    }

    public function submit_purchase_invoice(Request $request)
    {
...
//        $cash_in_hand_account_name = $this->get_account_name($cash_in_hand);
        ...
        if ($notes === 'PURCHASE_INVOICE'):
            $chk_bnk_pymnt = PurchaseInvoiceModel::where('pi_clg_id', '=', $user->user_clg_id)->get();
            $sstm_cnfg_clm = 'sc_purchase_invoice_number';
        elseif ($notes === 'PURCHASE_SALE_TAX_INVOICE'):
            $chk_bnk_pymnt = PurchaseSaletaxInvoiceModel::where('psi_clg_id', '=', $user->user_clg_id)->get();
...
//                $notes, $detail_remarks, $voucher_code . $purchase_invoice_id);//hamza
            $balance1 = $this->AssignAccountBalancesValues($balances1, $transaction_id1, $stock_in_hand, $this->invoice_total_value, 'Dr', $request->remarks, $notes, $detail_remarks, $voucher_code
                . $purchase_invoice_id, $request->posting_reference);
...
//        if party pay cash or select purchaser account
    ...
    public function AssignPurchaseInvoiceValues($request, $purchase_invoice, $day_end, $user, $prfx, $invoice_no = 0)
    {
        ...
        $purchase_invoice->$clg_id = $user->user_clg_id;
    ...
    public function purchase_invoice_list(Request $request, $array = null, $str = null)
    {
...
//            $pdf->setOptions($options);
    ...
    public function purchase_items_view_details(Request $request)
    {
    ...
    public function purchase_items_view_details_SH(Request $request, $id)
    {
...
//                PurchaseInvoiceItemsModel::where('pii_purchase_invoice_id', $id)
    ...
    public function purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice', 'rc_invoice_party')->first();
...
//                PurchaseInvoiceItemsModel::where('pii_purchase_invoice_id', $id)
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.purchase_invoice.purchase_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title','urdu_eng'));
//        $pdf->setOptions($options);
...
//            $pdf->setOptions($options);
    ...
    public function sale_tax_purchase_items_view_details_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0) {
            ...
            $pim = DB::table('financials_purchase_saletax_invoice')
                ...
                ->where('psi_id', $id)
                ->select('financials_accounts.account_urdu_name as psi_party_name', 'psi_id', 'psi_party_code', 'psi_customer_name', 'psi_remarks', 'psi_total_items', 'psi_total_price', 'psi_product_disc', 'psi_round_off_disc', 'psi_cash_disc_per', 'psi_cash_disc_amount', 'psi_total_discount', 'psi_inclusive_sales_tax', 'psi_exclusive_sales_tax', 'psi_total_sales_tax', 'psi_grand_total', 'psi_cash_paid',
                    'psi_day_end_id', 'psi_day_end_date', 'psi_createdby','pr_name')->first();
        }
        ...
        return view('invoice_view.purchase_sale_tax_invoice.purchase_sale_tax_invoice_list_modal', compact('piims', 'pim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title',
        'urdu_eng'));
    ...
    public function sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        $pdf = PDF::setOptions(['isHTML5ParserEnabled'=>true, 'isRemoteEnabled'=>true]);
        $pdf->getDomPDF()->setHttpContext($options,$optionss);
        $pdf->loadView('invoice_view.purchase_sale_tax_invoice.purchase_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);
    ...
    public function return_sale_tax_purchase_items_view_details_pdf_SH(Request $request, $id)
    {
        ...
        if ($urdu_eng->rc_invoice_party == 0) {
            ...
            $pim = DB::table('financials_purchase_return_saletax_invoice')
                ...
                ->where('prsi_id', $id)
                ->select('financials_accounts.account_urdu_name as prsi_party_name', 'prsi_id', 'prsi_party_code', 'prsi_customer_name', 'prsi_remarks', 'prsi_total_items', 'prsi_total_price', 'prsi_product_disc', 'prsi_round_off_disc', 'prsi_cash_disc_per', 'prsi_cash_disc_amount', 'prsi_total_discount', 'prsi_inclusive_sales_tax', 'prsi_exclusive_sales_tax', 'prsi_total_sales_tax', 'prsi_grand_total', 'prsi_cash_paid',
                    'prsi_day_end_id', 'prsi_day_end_date', 'prsi_createdby')->first();
        }
        ...
        $pdf->loadView('invoice_view.purchase_return_sale_tax_invoice.purchase_return_sale_tax_invoice_list_modal', compact('piims', 'pim', 'nbrOfWrds', 'accnts', 'invoice_nbr', 'invoice_date', 'type', 'pge_title','urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);
    ...
    public function purchase_register(Request $request, $array = null, $str = null)
    {
        $heads = config('global_variables.payable_receivable_cash');
        $heads = explode(',', $heads);
        ...
        $search_to = (!isset($request->to) && empty($request->to)) ? ((!empty($ar)) ? $ar[4]->{'value'} : '') : $request->to;
        $search_from = (!isset($request->from) && empty($request->from)) ? ((!empty($ar)) ? $ar[5]->{'value'} : '') : $request->from;
        $posting = (!isset($request->posting) && empty($request->posting)) ? ((!empty($ar)) ? $ar[6]->{'value'} : '') : $request->posting;
        $search_by_user = (isset($request->search_by_user) && !empty($request->search_by_user)) ? $request->search_by_user : '';
        $prnt_page_dir = 'print.purchase_invoice_list.purchase_register';
        $pge_title = 'Purchase Register';
        $srch_fltr = [];
...
//            $pdf->setOptions($options);
            ...
            return view('purchase_register', compact('posting_references','datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'pro_code', 'pro_name', 'search_to', 'search_from', 'search_by_user'));
        }
