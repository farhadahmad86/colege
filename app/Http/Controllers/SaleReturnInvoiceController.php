...
use App\Models\SaleServiceInvoiceItemsModel;
...
//        return view('sale_return_invoice', compact('pro_code', 'pro_name', 'accounts', 'products', 'packages', 'warehouses', 'detail_remarks', 'posting_references'));
    ...
    public function submit_sale_return_invoice(Request $request)
    {
        ...
        foreach ($requested_arrays as $index => $requested_array) {
            ...
            $item_remarks = isset($request->product_remarks[$index]) ? $request->product_remarks[$index] : '';//$request->remarks[$index];
        ...
        if ($invoice_type == 1) {
            ...
            $voucher_code = config('global_variables.SALE_RETURN_VOUCHER_CODE');
            ...
            $voucher_code = config('global_variables.SALE_RETURN_SALE_TAX_VOUCHER_CODE');
...
//            }
    ...
    public function get_sale_items_for_return(Request $request)
    {
        $invoice_type = $request->invoice_type;
    ...
    public function sale_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $prnt_page_dir = 'print.sale_return_invoice_list.sale_return_invoice_list';
        $pge_title = 'Sale Return Invoice List';
        $srch_fltr = [];
        ...
        if (!empty($search_posting_reference)) {
            $query->where('sri_pr_id', $search_posting_reference);
        }

        if (!empty($search_product)) {
            $get_p_id = SaleReturnInvoiceItemsModel::where('srii_product_code', $search_product)->pluck('srii_invoice_id')->all();
...
//            $query->whereBetween('si_day_end_date', [$start, $end]);
        ...
        $datas = $query->orderBy('sri_id', config('global_variables.query_sorting'))
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('sale_return_invoice_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'urdu_eng', 'posting_references', 'search_posting_reference'));
        }
    }

    public function sale_return_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
...
//            ->select('srii_product_name as name', 'srii_remarks as remarks', 'srii_qty as qty', 'srii_rate as rate', 'srii_discount as discount', 'srii_saletax as sale_tax', 'srii_amount as amount')
    ...
    public function sale_return_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
            $sim = SaleReturnInvoiceModel::Join('financials_posting_reference', 'financials_posting_reference.pr_id', 'financials_sale_return_invoice.sri_pr_id')->where('sri_id', $id)->first();
...
//            ->select('srii_product_name as name', 'srii_remarks as remarks', 'srii_qty as qty', 'srii_rate as rate', 'srii_discount as discount', 'srii_saletax as sale_tax', 'srii_amount as amount')
        ...
        $pdf->loadView('invoice_view.sale_return_invoice.sale_return_invoice_list_modal', compact('siims', 'sim', 'nbrOfWrds', 'accnts', 'type', 'pge_title', 'urdu_eng', 'invoice_nbr','invoice_date'));
//        $pdf->setOptions($options);
    ...
    public function sale_tax_sale_return_invoice_list(Request $request, $array = null, $str = null)
    {
        ...
        $pge_title = 'Sale Return Sale TaxInvoice List';
        $srch_fltr = [];
        array_push($srch_fltr, $search, $search_account, $search_product, $search_posting_reference, $search_to, $search_from);
...
//            $query->whereBetween('si_day_end_date', [$start, $end]);
        ...
        if (isset($request->array) && !empty($request->array)) {
            ...
            return view('sale_return_invoice_sale_tax_list', compact('datas', 'search', 'party', 'accounts', 'search_account', 'search_product', 'products', 'search_to', 'search_from', 'check_desktop', 'search_by_user', 'urdu_eng', 'posting_references', 'search_posting_reference'));
        }
    }

    public function sale_return_saletax_items_view_details_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
...
//            ->select('srsii_product_name as name', 'srsii_remarks as remarks', 'srsii_qty as qty', 'srsii_rate as rate', 'srsii_discount as discount', 'srsii_saletax as sale_tax', 'srsii_amount as amount')
        ...
        return view('invoice_view.sale_return_sale_tax_invoice.sale_return_sale_tax_invoice_list_modal', compact('siims', 'sim', 'accnts', 'nbrOfWrds', 'invoice_nbr', 'invoice_date', 'type', 'pge_title', 'urdu_eng'));

    }

    public function sale_return_saletax_items_view_details_pdf_SH(Request $request, $id)
    {
        $urdu_eng = ReportConfigModel::where('rc_id', '=', 1)->select('rc_invoice','rc_invoice_party')->first();
        if ($urdu_eng->rc_invoice_party == 0){
...
//            ->select('srsii_product_name as name', 'srsii_remarks as remarks', 'srsii_qty as qty', 'srsii_rate as rate', 'srsii_discount as discount', 'srsii_saletax as sale_tax', 'srsii_amount as amount')
                ->select('srsii_product_name as name', 'srsii_remarks as remarks', 'srsii_qty as qty', 'srsii_rate as rate', 'srsii_discount_per as discount', 'srsii_discount_amount as discount_amount',
                    'srsii_after_dis_rate as after_discount', 'srsii_saletax_per as sale_tax', 'srsii_saletax_amount as sale_tax_amount', 'srsii_saletax_inclusive as inclu_exclu', 'srsii_amount as amount', 'srsii_saletax_inclusive as type', 'srsii_scale_size as scale_size')
                ->get();
        } else {
            $siims = DB::table('financials_sale_return_saletax_invoice_items')
                ->join('financials_products', 'financials_products.pro_p_code', '=', 'financials_sale_return_saletax_invoice_items.srsii_product_code')
